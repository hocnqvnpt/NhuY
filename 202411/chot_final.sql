...
create table ttin_chungtu_t10 as 
...
--
                        ...
                        from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt a 
                                            ...
                                            left join ttin_Chungtu_t10 b on a.ma_Gd = b.ma_gd
                        ...
                        where  A.THANG = 202410-- and a.thuebao_id in (select thuebao_id from temp_chot)--and a.thang = 202403 and to_number(to_char(ngay_tt,'yyyymm'))  = 202403---        CHANGE
...
--            join ttkd_bsc.nhanvien_202404 b on a.MANV_THUYETPHUC = b.ma_nv 
            ...
            and not exists (select 1 from ttkd_Bsc.ct_dongia_Tratruoc where thang in  (202409) and tien_khop >0 and thuebao_id = a.thuebao_id and ma_nv = a.MANV_THUYETPHUC and
...
where thang = 202410 and tien_khop = 1 and loai_tinh = 'DONGIATRA_OB' and exists (
select 1-- distinct c.ma_ct,c.nd_ct,c.nhandien_thanhtoan ID600_nhandien,b.ma_gd
from ttkd_Bsc.nhuy_ct_bsc_ipcc_obghtt a
    join ttkd_bct.bangiao_chungtu_tinhbsc b on a.ma_Chungtu = b.ma_Ct
where
 a.thang = f.thang and a.thang = 202410 and a.thuebao_id = f.thuebao_id
 and f.ma_Nv = a.manv_thuyetphuc
    and b.xuly_tudong = 1
);
select* from ttk ;
commit;
select* from ttkd_Bsc.ct_dongia_Tratruoc where tien_khop = 3 and thang=202405;
update ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt set tien_khop = 3 where ma_Tb in (Select ma_tb from phithu_Tainha where thang = 202410);
update ttkd_Bsc.nhuy_ct_Bsc_ip  cc_obghtt set ipcc = 1 where ma_Nd_ob is not null and thang = 202410; and ipcc !=1;
select* from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202410 and ma_Tb='dulich63';
commit;
...
using (
  ...
  WHERE THANG = 202410 AND OB_ID IS NOT NULL
) b
...
;
update ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt set ipcc = 1 where ma_nd_ob is not null and thang = 202410 and ipcc is null;
commit;
select* from  ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt a where thang = 202410 and ma_nd_ob is null and ipcc is null; and exists (select 1 from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202410
and khachhang_id = a.khachhang_id and nvtuvan_id = a.nvtuvan_id and ma_gd = a.ma_Gd and ma_nd_ob is not null);
select thuebao_id, manv_Thuyetphuc from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt a where thang = 202410 and tien_khop = 1 
...
-- update 
update ttkd_Bsc.ct_dongia_tratruoc set tien_thuyetphuc = dongia where dongia!= 10000 and loai_tinh = 'DONGIATRA_OB' AND THANG = 202410;
update ttkd_Bsc.ct_dongia_tratruoc set tien_thuyetphuc = (dongia-tien_xuathd) where dongia= 10000 and loai_tinh = 'DONGIATRA_OB' AND THANG = 202410;
commit;
select distinct tien_khop, tien_xuathd, tien_thuyetphuc from  ttkd_Bsc.ct_dongia_tratruoc WHERE loai_tinh = 'DONGIATRA_OB' AND THANG = 202410 ; and dongia != 7500 ;
rollback;
select* from  ttkd_Bsc.ct_dongia_tratruoc WHERE loai_tinh = 'DONGIATRA_OB' AND THANG = 202410 and tien_khop!= 1;
...
select* from ttkd_Bsc.ct_Dongia_Tratruoc where thang = 202410 and loai_tinh = 'DONGIA_TS_TP_TT';
...
from  ttkd_Bsc.tl_giahan_Tratruoc WHERE THANG = 202410 AND LOAI_TINH IN ('DONGIATRA_OB','DONGIA_TS_TP_TT')
GROUP BY THANG,MA_NV,MA_TO,MA_PB;
rollback;
select* from ttkd_Bsc.ct_dongia_tratruoc where thang = 202410 and ma_Tb ='bht210726';
COMMIT;
select* from  ttkd_Bsc.tl_giahan_tratruoc where thang = 202409 and ma_nv = 'CTV021917';
delete from ttkd_Bsc.tl_giahan_tratruoc where thang = 202410 and ma_pb in (select ma_pb from ttkd_Bsc.nhanvien where thang = 202410 and ten_pb like '%Khu%') and loai_Tinh = 'DONGIATRA_OB';
...
with tien as (
    ...
    where thang = 202410 and loai_tinh = 'DONGIATRA_OB_BS_T9' --and ipcc = 1 --and ghichu = 'bo sung TB co chot, chua chi don gia, nhan vien xuat hoa don la nhan vien cap nhat chung tu, ty le thanh cong la ty le tien don gia xuat HD (TTCK)'
--    and ma_nv = 'VNP017247'
--   DONGIATRA_OB_BS_T92410 and loai_tinh = 'DONGIATRA_OB' --and ghichu= 'bo sung TB co chot, chua chi don gia, nhan vien xuat hoa don la nhan vien cap nhat chung tu, ty le thanh cong la ty le tien don gia xuat HD (TTCK)'
)
select a.thang,a.ma_nv, b.ma_to, b.ma_pb, a.loai_tinh, a.ma_kpi, sum(tien) tien --,b.ten_to, b.ten_pb, b.ma_Vtcv, b.ten_vtcv--, ten_pb
from tien a
    join ttkd_bsc.nhanvien b on a.ma_nv = b.ma_nv and 202409 = b.thang and b.donvi = 'TTKD'
--WHERE  a.ma_pb != 'VNP0700600' --(select ma_pb from ttkd_Bsc.nhanvien where thang = 202410 and ten_pb like '%Khu%') 
