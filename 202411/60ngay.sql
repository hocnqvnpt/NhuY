select* from ttkd_bsc.ct_bsc_tratruoc_moi where thang = 202411;
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
            ...
            , ds as (select ten_pbh, thuebao_id,202410 thang_kt
                    from giao_oneb where thang_kt = 202410 and thang = 202411
                    )
    ...
    select 202411 thang, 0, 0 pbh_ql_id, 0 pbh_giao_id, 0 tbh_giao_id
    ...
    where  a.thang_kt = 202410 --and rkm_id != 3369696 --a.ma_tb  in ('') -- change
           --      and a.gh_id is null
...
set ma_pb = (select distinct ma_pb from ttkd_Bsc.nhanvien where thang = 202411 and ten_pb = phong_cs)
where thang = 202411 and ma_pb is null;
-- khdn
update ttkd_Bsc.ct_Bsc_Tratruoc_moi a 
...
where  ht_tra_id in (2,4,5,207) and tien_khop = 0  and thang = 202411 --and rkm_id in (select rkm_id from tmp3_30ngay where lan = 308)
and exists 
    (
...
delete from  ttkd_bsc.ct_Bsc_Tratruoc_moi where thang = 202411 and khachhang_id in (2842255,9802705, 8921108);
commit;
-- bang cha du tien
update ttkd_bsc.ct_Bsc_Tratruoc_moi a set tien_khop = 1
-- select* from ttkd_bsc.ct_Bsc_Tratruoc_moi a
...
commit;
select distinct diachi from userld_202411_goc;
select* from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang = 202411 and ma_pb = 'VNP0702300' and tien_khop = 0;
select count(distinct rkm_id) from ttkd_Bsc.ct_Bsc_Tratruoc_moi where thang = 202411 and tien_khop is null and ht_Tra_id is not null;
update ttkd_Bsc.ct_Bsc_Tratruoc_moi set tien_khop = 1 where  thang = 202411 and ht_Tra_id = 207 and kenhthu_id in (25,26);
...
where thang = 202411 and rkm_id is null and 
not exists (select 1 from tmp3_60ngay where a.thuebao_id = thuebao_id)
and  exists (select 1 from tt where a.thuebao_id = thuebao_id and to_number(to_char(ngay_kt_mg,'yyyymm'))>202411);
commit;
rollback;
delete from ttkd_bsc.tl_giahan_Tratruoc where thang = 202411 and ma_kpi ='HCM_TB_GIAHA_023';
...
and ma_kpi in ('HCM_TB_GIAHA_023') and thang = 202411 and MA_VTCV = a.MA_VTCV)
and ma_kpi = 'HCM_TB_GIAHA_023' and tytrong is not null;
select* from TTKD_BSC.bangluong_kpi a where thang = 202411 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where giamdoc_phogiamdoc = 1 
and ma_kpi in ('HCM_TB_GIAHA_023') and thang = 202411 and MA_VTCV = a.MA_VTCV)
and ma_kpi = 'HCM_TB_GIAHA_023' ;and tytrong is not null and chitieu_giao is not null;
delete from  TTKD_BSC.bangluong_kpi a where thang = 202411 and  ma_kpi = 'HCM_TB_GIAHA_023' and tytrong is not null and chitieu_giao is  null;
commit;
--
...
where ma_kpi ='HCM_TB_GIAHA_023' AND THANG = 202411;
...
--                            and dbkh.khachhang_id  <> 9798610          ---khong tinh giao 30 ngay, chi tinh 60 ngay theo vb 3116/TTr-NVC-KHDN2 (folder loai tru ghtt) cho het nam 2023
                 ...
                 --  and a.ma_tb in ('hcmnhp183')
                  -- and a.thuebao_id in (1307757,
                and not exists (select 1 from ttkd_bsc.ct_Bsc_Tratruoc_moi where thang = 202411 and c.rkm_id = rkm_id )
;
commit;
select* from ttkd_Bsc.ct_Bsc_Tratruoc_moi where thang = 202411 and thuebao_id in (
select thuebao_id from ttkd_Bsc.ct_Bsc_Tratruoc_moi where thang = 202411 having count(distinct nvl(tien_khop,0))>1 group by thuebao_id);
delete from ttkd_Bsc.ct_Bsc_Tratruoc_moi where thang = 202411 and thuebao_id in (
select  thuebao_id from ttkd_Bsc.ct_Bsc_Tratruoc_moi where thang = 202411 and rkm_id in (select rkm_id from tmp3_60ngay_bs)) and tien_khop is null
;group by thuebao_id having count(nvl(tien_khop,0))=2;
update ttkd_Bsc.ct_Bsc_Tratruoc_moi set tien_khop = 1 where thang = 202411 and ht_Tra_id = 208;