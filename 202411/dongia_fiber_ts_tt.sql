...
--													   left join ct on p.phieutt_id = ct.phieu_id
                     ...
                     hdtb.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224) and a.tien > 0 --and hdtb.thuebao_id = 8811198
                                      and  to_number(to_char(p.ngay_tt, 'yyyymm')) = 202411                 ----change--2 thang- ngay 02
...
select 202411 thang,  -1 gh_id, dbtb.ma_tb, a.thuebao_id
...
--                left join c on a.thuebao_id = c.thuebao_id
            left join ttkd_bsc.nhanvien nv on a.MANV_THUYETPHUC = nv.ma_nv and thang = 202411 ; 
 commit;
 --- update chung tu
...
--                                            left join  km on a.rkm_id = km.rkm_id
                        where a.thang = 202411-- and --a.rkm_id is not null AND -- don gia TP 
...
commit;
delete from ttkd_bsc.tl_giahan_Tratruoc where thang = 202411 and loai_tinh=  'DONGIA_TS_TP_TT';
...
where ma_kpi = 'DONGIA' --and loaitb_id in (58,59)
            and loai_tinh in ('DONGIA_TS_TP_TT') and a.thang = 202411 and b.ma_pb not in ('VNP0702300','VNP0702400','VNP0702500')
