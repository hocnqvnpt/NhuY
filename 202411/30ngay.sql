...
--                                            left join ct on a.phieutt_id = ct.phieu_id
                         ...
                         where a.kenhthu_id not in (6) and a.trangthai = 1
                                        and to_number(to_char(a.ngay_tt, 'yyyymmdd')) between 20240801 and 20241201                  ----change--3 thang- ngay 2
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
            ...
            , ds as (select ten_pbh, thuebao_id, thang_kt
                    from giao_oneb where thang_kt =202411 and thang = 202411
                    )
    ...
    from ds a
                                ...
                                left join ttkd_bsc.nhanvien nvtp on c.MANV_THUYETPHUC = nvtp.ma_nv and nvtp.thang = 202411 -- change
...
commit;
select* from ttkdhcm_ktnv.ds_Chungtu_Nganhang_oneb where ma_ct='VCB_20241102_438486';
...
rollback;
select distinct phong_cs , ma_pb from ttkd_Bsc.ct_Bsc_Tratruoc_moi where thang = 202411; and ma_pb is null;
...
where thang = 202411 and tien_khop = 0 and ht_Tra_id in (207) and kenhthu_id in (25,26);
select* from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang = 202411 and tien_khop = 0;
select* from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang = 202411 and tien_khop is null and rkm_id is  not null;
...
update ttkd_bsc.ct_Bsc_tratruoc_moi_30day a set tien_khop = 1
--select thuebao_id from ttkd_bsc.ct_Bsc_tratruoc_moi_30day a 
...
delete from ttkd_Bsc.tl_giahan_Tratruoc where thang = 202411 and ma_Kpi = 'HCM_TB_GIAHA_022';
...
where thang = 202411 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where-- giamdoc_phogiamdoc = 1 
 ma_kpi in ('HCM_TB_GIAHA_022') and thang = 202411 and MA_VTCV = a.MA_VTCV)
...
where ma_kpi ='HCM_TB_GIAHA_022' AND THANG = 202411 and ma_pb != 'VNP0703000';
commit;
--- bhol 
update ttkd_Bsc.bangluong_kpi  
set mucdo_hoanthanh = case when tyle_Thuchien >= 80 then 120
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
    ...
    from ds a
                                ...
                                left join ttkd_bsc.nhanvien nvtp 
                                    on c.MANV_THUYETPHUC = nvtp.ma_nv and nvtp.thang = 202411 -- change
                                left join goi on a.thuebao_id = goi.thuebao_id and rn = 1
                        
    where  a.thang_kt = 202411 and a.thuebao_id in (select thuebao_id from tmp3_60ngay_Bs)
     and not exists (select 1 from ttkd_Bsc.ct_Bsc_tratruoc_moi where thang = 202411 and tien_khop =1 and thuebao_id = a.thuebao_id);   and dbtb.ma_Tb='co29042019' --and rkm_id != 3369696 --a.ma_tb  in ('') -- change
           --      and a.gh_id is null
                            and dbkh.khachhang_id  <> 9798610          ---khong tinh giao 30 ngay, chi tinh 60 ngay theo vb 3116/TTr-NVC-KHDN2 (folder loai tru ghtt) cho het nam 2023
           ;       --   and a.thangdc < 3 and c.so_thangdc>= 3 
                 --  and a.ma_tb in ('hcmnhp183')
...
;
commit;
select* from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang= 202411 ;and thuebao_id in (
select thuebao_id from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang= 202411 group by thuebao_id having count(distinct nvl(tien_khop,0) )>1);
and rkm_id is null;
