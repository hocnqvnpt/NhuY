     ...
     a.thuebao_id, a.ma_tb 
                ,case 
                   ...
                   round((select sum(da_giahan_dung_hen)*100/sum(tong) 
                            from ttkd_Bsc.tl_giahan_tratruoc where thang = 202411 and ma_kpi = 'HCM_TB_GIAHA_024' and loai_tinh = 'KPI_NV'  and ma_nv = a.manv_giao),2) tyle
              from ttkd_bsc.ct_bsc_giahan_cntt a
                    where a.thang = 202411  and thang_ktdc_cu = 202411  and loaitb_id  in (55 ,80 ,116 ,117,132,140,154,181,288,318 ) -- and ma_Tb in ('hcm_ca_00039226','hcm_ca_00055400')     ------------n------------
                 ...
                 left   join ttkd_bsc.nhanvien nv on a.manv_giao = nv.ma_nv and nv.thang = 202411 and nv.donvi = 'TTKD';
...
where thang = 202411 AND LOAI_TINH = 'DONGIATRU_CA' AND MA_KPI = 'DONGIA' and a.ma_nv in 
(Select ma_nv from ttkd_bsc.nhanvien where ma_vtcv not in ('VNP-HNHCM_BHKV_6','VNP-HNHCM_KHDN_3','VNP-HNHCM_KHDN_18', 'VNP-HNHCM_BHKV_41') and thang=202411 and donvi = 'TTKD' )
    and ghichu is  null;

update TTKD_bSC.ct_dongia_tratruoc set dongia = 0 where ghichu is not null and thang = 202411 and loai_tinh = 'DONGIATRU_CA';
rollback;
select* from ttkd_Bsc.ct_dongia_tratruoc where thang = 202411 and loai_Tinh='DONGIATRU_CA';
commit;
--- boi hoan

insert into ttkd_Bsc.ct_dongia_tratruoc
select 202411 THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, PHONG_QL, THUEBAO_ID, MA_TB, TIEN_DC_CU, MANV_GIAO, MA_NV_CN, MANV_THUYETPHUC, SOTHANG_DC, HT_TRA_ONLINE, 
...
commit;
select* from ttkd_bsc.ct_dongia_tratruoc where tien_Dc_Cu is  null and loai_tinh ='DONGIATRU_CA' AND thang = 202411;
delete from ttkd_Bsc.tl_giahan_tratruoc where thang = 202411 and loai_tinh = 'DONGIATRU_CA';
commit;
...
where ma_kpi = 'DONGIA' 
        and loai_tinh = 'DONGIATRU_CA' and thang = 202411
...
where thang = 202411 and  loai_tinh = 'DONGIATRU_CA'  and tien_khop = 0 and ghichu is null and tien_Dc_cu is null;
update  ttkd_Bsc.ct_dongia_Tratruoc a
set dongia=tien_Dc_Cu /10
where thang = 202411 and  loai_tinh = 'DONGIATRU_CA'  and tien_khop = 0 and ghichu is null and dongia is null;

select* from ttkd_Bsc.ct_dongia_Tratruoc where thang = 202411 and  loai_tinh = 'DONGIATRU_CA'  ; and ma_Nv= 'VNP017796';
select* from ttkd_Bsc.ct_Bsc_Giahan_cntt where thang = 202411 and manv_Giao= 'VNP017796';
delete from ttkd_bsc.tl_giahan_tratruoc a
where loai_tinh = 'DONGIATRU_CA' and a.thang = 202411 ;
select * from ttkd_bsc.tl_giahan_tratruoc a
where loai_tinh = 'DONGIATRU_CA' and a.thang = 202411 ;and tien != 0;
select* from ct_Bsc_chungtu where thang = 202411 and tra_truoc  =1 and ma_gd like 'HCM-LD%';
update  ttkd_bsc.tl_giahan_tratruoc set tien = 0  where ma_nv ='VNP029065' and loai_tinh = 'DONGIATRU_CA' and thang = 202411;
...
delete;
update  ttkd_Bsc.tl_giahan_tratruoc set tien = 0 where thang = 202411  and loai_tinh = 'DONGIATRU_CA' and ma_Nv = 'VNP029065';;
commit;
select distinct loai_tinh, ma_kpi from ttkd_Bsc.tl_giahan_tratruoc where thang = 202411  and loai_tinh = 'DONGIATRU_CA' ;
select* from  ttkd_Bsc.tl_giahan_tratruoc where thang = 202411 and ma_kpi = 'THUHOI_BSC_DONGIA';
...
select distinct ma_kpi, loai_tinh from ttkd_Bsc.tl_giahan_tratruoc where thang = 202411;
...
select SUM( TIEN )
from tl_giahan_Tratruoc where loai_tinh = 'DONGIATRU' AND MA_KPI = 'DONGIA_CA';    
SELECT DISTINCT TEN_vTCV FROM NHANVIEN_202411 WHERE MA_nV IN (SELECT MA_nV FROM TTKD_BSC.TL_GIAHAN_TRATRUOC WHERE THANG = 202411 AND loai_Tinh ='DONGIATRU_CA' AND TIEN!=0);