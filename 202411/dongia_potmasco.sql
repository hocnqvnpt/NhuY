...
with ds as (
    ...
    HT_TRA_ONLINE, KHUVUC, case when tien_khop = 1 then   DONGIA end DONGIA, DTHU, NGAY_TT, HESO_GIAO, KHDN, NHOMTB_ID, KHACHHANG_ID, HESO_DICHVU
    , TIEN_KHOP, GHICHU, TYLE_THANHCONG, HESO_CHUKY,
    NHANVIEN_XUATHD, null TIEN_XUATHD, case when tien_khop = 1 then   TIEN_THUYETPHUC end TIEN_THUYETPHUC, IPCC
    from ttkd_bsc.ct_dongia_tratruoc a where thang =  202411 and loai_tinh = 'DONGIATRA_OB' and 
        (exists (select 1 from ttkd_bsc.nhanvien where thang = 202411 and donvi = 'POT' and ma_nv = a.ma_nv) or 
        exists (select 1 from ttkd_bsc.nhanvien where thang = 202411  and donvi = 'POT' and ma_nv = a.nhanvien_xuathd))
)
...
;
select* from ttkd_Bsc.dongia_Vttp where thang = 202411;
--- TRA SAU CHUYEN TRA TRUOC
insert into ttkd_Bsc.dongia_Vttp(THANG, LOAI_TINH, MA_KPI, MA_NV, THUEBAO_ID, MA_TB, MANV_THUYETPHUC, SOTHANG_DC,DONGIA, DTHU, NGAY_TT,KHACHHANG_ID, HESO, TIEN_KHOP, TIEN_DONGIA, 
TEN_NV, DV_CAP1, DV_CAP2, DONVI_ID, HESO_CHUKY, MANV_XHDON, TIEN_XHDON)
with ds as (
    select* from ttkd_bsc.ct_dongia_tratruoc a where thang =  202411 and loai_tinh = 'DONGIA_TS_TP_TT' and 
        (exists (select 1 from ttkd_bsc.nhanvien where thang = 202411 and donvi = 'POT' and ma_nv = a.ma_nv) or 
        exists (select 1 from ttkd_bsc.nhanvien where thang = 202411 and donvi = 'POT' and ma_nv = a.nhanvien_xuathd))
)
...
DELETE from
--select*  from
ttkd_Bsc.dongia_Vttp A where loai_Tinh = 'DONGIA_POTMASCO' and thang = 202411 and exists (select 1 from ttkd_bsc.dongia_vttp where thang = a.thang and ma_nv = a.ma_nv
...
set TIEN_DONGIA = 0
where ma_nv not in (Select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202411 ) and thang = 202411 and loai_tinh = 'DONGIA_POTMASCO';
commit;
update ttkd_Bsc.dongia_vttp 
set TIEN_XHDON = 0
where nvl(MANV_XHDON,'a') not in (Select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang =  202411) and thang = 202411 and TIEN_XHDON > 0 and loai_tinh = 'DONGIA_POTMASCO';
...
select * from ttkd_Bsc.dongia_vttp where thang = 202411 and donvi = 'TTKD'  ;
COMMIT;
select * from ttkd_bsc.bldg_danhmuc_vtcv_p1 where thang = 202408;
select * from ttkd_Bsc.nhanvien where thang = 202411 and  donvi = 'TTKD' AND TEN_VTCV ='Nhân viên qu?n lý ?i?m bán';
