...
update ct_Bsc_Chungtu set thang = -202412 where thang = 202412;
delete from ct_Bsc_Chungtu where thang = 202412;
commit;
select* from ttkdhcm_ktnv.ds_chungtu_nganhang_tinh_bsc_202412_1 where thang  = 202412;
...
from ttkdhcm_ktnv.ds_chungtu_nganhang_tinh_bsc_202412_1 a
...
where thang = 202412 and tra_truoc = 1 and exists (select 1 from ttkd_bsc.ct_dongia_tratruoc a 
...
commit;
---- tinh toan
select* from ct_Bsc_chungtu where ma_Chungtu='VCB_20241212_457066';

update ct_Bsc_Chungtu set tinh_Bsc = 0, tinh_dongia = 0
--select distinct chungtu_id from ct_Bsc_Chungtu
where tra_truoc = 0 and thang = 202412 and (chungtu_id, ma_tt) in (
...
select a.chungtu_id,
                ...
                where a.chungtu_id = b.chungtu_id 
                            ...
                            and to_number(to_char(b.ngay_tt, 'yyyymm')) = 202412 and tongtra> 0 and  c.httt_id  in (170, 171)
                            )
...
, final as (
    select a.*, case when nv_Gach like 'dl_mailinh%' then 2
                ...
                ,case when nv_Gach like 'dl_mailinh%' then 2
                    ...
                    else null end heso_bsc, 202412 thang
    from chot a
)
select* from final;
commit;
update final_Ctu_tien set thang= -202412 where thang = 202412;
delete from final_Ctu_tien  where thang = 202412;
...
from tt 
    join ttkd_Bsc.nhanvien a ON TT.MA_nV = A.MA_nV AND thang = 202412 and donvi = 'TTKD'
where ma_to = 'VNP0700940'
GROUP BY TT.MA_NV, MA_PB, MA_TO,TEN_NV, TEN_TO,TEN_PB, a.thang
having SUM(TIEN) > 0;
rollback;
commit;
select sum(tien) from final_Ctu_tien where thang =202412;in ( 202412 ,112024);
delete from ttkd_Bsc.tl_Giahan_Tratruoc where thang = 202412 and loai_Tinh = 'DONGIA_CHUNG_TU';
...
FROM final_Ctu_tien WHERE THANG = 202412;
delete from  ttkd_Bsc.tl_Giahan_Tratruoc where thang =202412 and loai_tinh= 'DONGIA_CHUNG_TU'