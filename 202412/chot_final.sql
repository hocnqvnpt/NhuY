...
select* from ttkd_bsc.nhuy_ct_bsc_ipcc_obghtt where thanb;
...
delete from ttkd_Bsc.ct_Dongia_Tratruoc where thang = 202412 and loai_tinh = 'DONGIATRA_OB';
...
--
                        ...
                        from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt a 
                                            ...
                                            left join ttin_Chungtu_t12 b on a.ma_Gd = b.ma_gd
                        ...
                        where  A.THANG = 202412-- and a.thuebao_id in (select thuebao_id from temp_chot)--and a.thang = 202403 and to_number(to_char(ngay_tt,'yyyymm'))  = 202403---        CHANGE
...
--            join ttkd_bsc.nhanvien_202404 b on a.MANV_THUYETPHUC = b.ma_nv 
            ...
            and not exists (select 1 from ttkd_Bsc.ct_dongia_Tratruoc where thang in  (202411) and tien_khop >0 and thuebao_id = a.thuebao_id and ma_nv = a.MANV_THUYETPHUC and
...
set ma_Chungtu = (select   distinct ma_Ct from ttkdhcm_ktnv.ds_chungtu_nganhang_oneb a 
                            join ttkdhcm_ktnv.ds_chungtu_Nganhang_phieutt_hd_1 b on a.chungtu_id = b.chungtu_id
--                            where ma in (select ma_Gd from ttkd_Bsc.nhuy_ct_bsc_ipcc_obghtt where thang  = 202412)
            where b.ma = x.ma_Gd)
where thang = 202412 and ma_Gd !='C';
commit;
select* from ttkd_Bsc.nhuy_ct_bsc_ipcc_obghtt x where ma_Gd ='C';
...
where thang = 202412 and tien_khop = 1 and loai_tinh = 'DONGIATRA_OB' and exists (
...
-- update 
update ttkd_Bsc.ct_dongia_tratruoc set tien_thuyetphuc = dongia where dongia!= 10000 and loai_tinh = 'DONGIATRA_OB' AND THANG = 202412;
update ttkd_Bsc.ct_dongia_tratruoc set tien_thuyetphuc = (dongia-tien_xuathd) where dongia= 10000 and loai_tinh = 'DONGIATRA_OB' AND THANG = 202412;
commit;
...
delete from ttkd_Bsc.ct_dongia_Tratruoc where thang = 202412
and ma_to in (select distinct ma_to from ttkd_Bsc.nhanvien where thang = 202412 and ten_to = 'T? Bán Hàng Online');
...
WHERE a.THANG = 202412 AND LOAI_TINH IN ('DONGIATRA_OB','DONGIA_TS_TP_TT') and b.ma_Vtcv = 'VNP-HNHCM_KDOL_17'
...
delete from ttkd_Bsc.ct_Dongia_Tratruoc where thang = 202412 and loai_tinh = 'DONGIATRA_OB' and ma_To in 
(select DISTINCT ma_to from ttkd_Bsc.nhanvien where thang = 202412 and UPPER(ten_to) = 'T? BÁN HÀNG ONLINE');
COMMIT;
delete from ttkd_Bsc.tl_giahan_tratruoc where thang = 202412 and loai_tinh = 'DONGIATRA_OB' and ma_pb != 'VNP0700900'; and ma_pb in (select ma_pb from ttkd_Bsc.nhanvien where thang = 202410 and ten_pb like '%Khu%') and loai_Tinh = 'DONGIATRA_OB';
...
with tien as (
    ...
    where thang = 202412 and loai_tinh = 'DONGIATRA_OB_BS'  --and ghichu = 'bo sung TB co chot, chua chi don gia, nhan vien xuat hoa don la nhan vien cap nhat chung tu, ty le thanh cong la ty le tien don gia xuat HD (TTCK)'
--    and ma_nv = 'VNP017247'
    union all
...
commit;
select distinct loai_Tinh, ma_kpi from ttkd_Bsc.tl_giahan_Tratruoc where thang = 202412;
delete from  ttkd_Bsc.CT_DONGIA_TRATRUOC where thang = 202412 and loai_tinh in ('DOANHTHU','DONGIATRA_OB');
