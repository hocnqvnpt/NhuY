...
commit;
select* from nhuy.tmp3_ob ;where thuebao_id in (select thuebao_id from nhuy.tmp3_ob group by thuebao_id having count(distinct ngay_bd_moi)>1);
create table tmp3_ob as;
--commit;
insert into tmp3_ob;
select* from css_hcm.db_Thuebao_ttk where ma_Tb='khanhdu89';
WITH db as 
(
    select a.thuebao_id, a.loaitb_id, ma_tb
    from css_hcm.db_Thuebao  a
--        join css_hcm.db_adsl  b on a.thuebao_id =b.thuebao_id
    where a.trangthaitb_id not in (7,8,9)  and a.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224) and to_number(to_char(a.ngay_sd,'yyyymm')) <= 202407
) 
...
--        group by bb.phieu_id) 
    , kq_ghtt as (select 
              ...
              , p.thungan_tt_id, p.kenhthu_id, p.ht_tra_id, hdtb.tthd_id, hdkh.ngay_yc, p.thungan_hd_id,
               kt.kenhthu , nh.ten_nh ten_nganhang ,ht.ht_tra ten_ht_tra, 888 lan
...
--													   left join ct on p.phieutt_id = ct.phieu_id
                     ...
                     hdtb.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224,136) and a.tien > 0 
                                      and ( to_number(to_char(hdtb.ngay_ht, 'yyyymmdd')) between 20241001 and 20241031)  --    and p.ma_Gd = 'HCM-TT/02814791'            
                                      and ( nvl(to_number(to_char(p.ngay_tt, 'yyyymmdd')),20241025) between 20241001 and 20241031)  --    and p.ma_Gd = 'HCM-TT/02814791'            
)  
select *
from kq_ghtt a where ma_Tb='khanhdu89';
...
-- tao bang chot
create table tmp3_ob as select* from TTKD_HCM.TMP3_OB ; where ma_Tb = 'vanday5389166';
...
(
    ...
    from css_hcm.db_Thuebao a
        join css_hcm.db_adsl b on a.thuebao_id =b.thuebao_id
    where a.trangthaitb_id not in (7,8,9) and b.chuquan_id = 145 and a.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224) and to_number(to_char(a.ngay_sd,'yyyymm')) <= 202407
)
,km as (select rkm_id, thuebao_id, ma_tb, loaitb_id, ngay_bddc, ngay_ktdc, ngay_kt, ngay_huy, ngay_thoai
                            , thang_bddc, thang_ktdc, thang_kt_mg, thang_huy, thang_kt_dc                                                            
                            , case when ngay_huy is not null then ngay_huy
...
--													   left join ct on p.phieutt_id = ct.phieu_id
                     ...
                     hdtb.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224) and a.tien > 0 --and hdtb.thuebao_id = 8811198
                                      and  to_number(to_char(p.ngay_tt, 'yyyymm')) = 202410                  ----change--2 thang- ngay 02
...
select* from admin_hcm.nhanvien_onebss where nhanvien_id =859;
select* from ttkd_Bsc.nhanvien where ma_nv='HCM011780';
create table tmp3_ts_ as select* from ttkd_hcm.tmp3_ts ;
...
select *
from kq_Ghtt a where ngay_bd_moi is not null and ma_Tb='khanhdu89'; not exists (select 1 from tmp3_ob where rkm_id = a.rkm_id); and ma_tb in ('giaquang_f3','an133_14','tri-1963','');nd ma_tb = 'hongand12'
;
select* from v_Thongtinkm_All where ma_Tb='khanhdu89';
----- XOA BOT
