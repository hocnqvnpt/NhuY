...
COMMIT;
select* from sldh_bhol where thang = 202412;where ht_tra_id in(214,216);
create table sldh_bhol as;
select* from giao_oneb c where thang_kt = 202412 and c.thang = 202412;
select* from ttkd_bsc.blkpi_danhmuc_kpi where thang = 202412 and ma_kpi = 'HCM_SL_BHOL_009';
insert into sldh_bhol
select
    202412 thang, b.thuebao_id, b.ma_tb, a.ngay_yc, c.ngay_Ktdc, nv.ma_nv, nv.ten_nv, nv.ma_to, nv.ma_pb, p.ngay_tt, a.ma_gd, p.ht_Tra_id, p.kenhthu_id, p.trangthai, p.tien, p.vat, 
...
from css_Hcm.hd_khachhang a
    ...
    join giao_oneb c on b.ma_Tb = c.ma_Tb and c.thang_kt = 202412 and c.thang = 202412
...
where to_number(to_char(a.ngay_yc,'yyyymm')) between 202410 and 202412 and 
nv.ma_pb ='VNP0703000' ;
-- update chung tu
update sldh_bhol a set tien_khop = 1
-- select* from ttkd_bsc.nhuy_Ct_bsc_ipcc_obghtt a
where  ht_tra_id in (2,4,5,207) and tien_khop = 0  and thang = 202412 --and rkm_id in (select rkm_id from tmp3_30ngay where lan = 22)
and exists 
    (
...
where b.kieuld_id in (551, 550, 24, 13080) and b.thuebao_id = 2350321 and  to_number(to_char(a.ngay_yc,'yyyymm')) between 202409 and 202412 ;

update sldh_bhol set tien_khop = 1-- where ht_Tra_id = 216 and thang = 202412;
where tien_khop = 0 and ma_gd in (select ma from ttkdhcm_ktnv.ds_chungtu_nganhang_phieutt_hd_1 ) and thang = 202412;
select* from sldh_bhol where  thang = 202412; and ht_Tra_id is not null and tien_khop is null;
...
COMMIT;
delete from sldh_bhol where thang = 202412;
ROLLBACK;
...
SET TYLE_THUCHIEN = (select tyle from  ttkd_Bsc.tl_giahan_Tratruoc where a.thang = thang and a.ma_kpi = ma_kpi ),
    giao = (select tong from  ttkd_Bsc.tl_giahan_Tratruoc where a.thang = thang and a.ma_kpi = ma_kpi ),
    thuchien = (select da_giahan_dung_Hen from  ttkd_Bsc.tl_giahan_Tratruoc where a.thang = thang and a.ma_kpi = ma_kpi )
WHERE 'HCM_SL_BHOL_002' =MA_KPI AND THANG = 202412;
select* from  TTKD_bSC.BANGLUONG_KPI  a WHERE 'HCM_SL_BHOL_002' =MA_KPI AND THANG = 202412;

delete from sldh_Bhol where thang = 202412 and ht_Tra_id = 208;
commit;

delete from ttkd_Bsc.tl_giahan_Tratruoc where thang = 202412 and ma_Kpi in( 'HCM_SL_BHOL_002','HCM_SL_BHOL_009');
commit;
...
--****C.4(BHKV), C.4(KHDN) Ty le thue bao ghtt khong thanh cong tren tap KH thuoc BHKV, BHDN giao ds het han----
                ...
                from        (select thang, a.ma_to, a.ma_pb,a.ma_nv , sum(tien) DTHU, max(ngay_tt) ngay_tt
                                         ...
                                         from SLDH_BHOL a
                                        where thang = 202412 and ma_nv is not null
...
insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TONG, DA_GIAHAN_DUNG_HEN
            ...
            where thang = 202412 and MA_KPI in ('HCM_SL_BHOL_002') --and  ma_to ='VNP0702302'  --, 'HCM_TB_GIAHA_025') 
...
WHERE 'HCM_SL_BHOL_002' =MA_KPI AND THANG = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_SL_BHOL_002')  
...
, tyle_thuchien  = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
                                                            where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_TO' 
                                                                                and ma_to = a.ma_to and ma_pb = a.ma_pb and ma_kpi = 'HCM_SL_BHOL_002')
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where to_truong_pho = 1 and ma_kpi in ( 'HCM_SL_BHOL_002') 
                                and thang = 202412 and MA_VTCV = a.MA_VTCV) and ma_kpi = 'HCM_SL_BHOL_002';
commit;
----
UPDATE TTKD_bSC.BANGLUONG_KPI  a
...
WHERE 'HCM_SL_BHOL_002' =MA_KPI AND THANG = 202412;
select * from ttkd_Bsc.BANGLUONG_KPI  WHERE 'HCM_SL_BHOL_009' =MA_KPI AND THANG = 202412;
---- 009
insert into ttkd_Bsc.tl_giahan_Tratruoc(THANG, MA_PB,MA_TO,MA_NV,ma_kpi, LOAI_TINH, TONG, DA_GIAHAN_dung_hen,  DTHU_THANHCONG_DUNG_HEN, TYLE)
select a.*
from (
...
SET TYLE_THUCHIEN = (select tyle from  ttkd_Bsc.tl_giahan_Tratruoc where a.thang = thang and a.ma_kpi = ma_kpi and ma_Nv = a.ma_nv ),
    giao = (select tong from  ttkd_Bsc.tl_giahan_Tratruoc where a.thang = thang and a.ma_kpi = ma_kpi and ma_Nv = a.ma_nv),
    thuchien = (select da_giahan_dung_Hen from  ttkd_Bsc.tl_giahan_Tratruoc where a.thang = thang and a.ma_kpi = ma_kpi and ma_Nv = a.ma_nv)
WHERE 'HCM_SL_BHOL_009' =MA_KPI AND THANG = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_SL_BHOL_009')  
                                            and thang = 202412 and MA_VTCV = a.MA_VTCV);
...
WHERE 'HCM_SL_BHOL_009' =MA_KPI AND THANG = 202412;
commit;
select* from TTKD_bSC.BANGLUONG_KPI  a WHERE 'HCM_SL_BHOL_009' =MA_KPI AND THANG = 202412;
...
delete from ttkd_bsc.tl_giahan_Tratruoc where thang = 202412 and ma_kpi in ('HCM_SL_BHOL_009','HCM_SL_BHOL_002');
---- code cu
insert into ttkd_Bsc.tl_giahan_Tratruoc(THANG, MA_PB,MA_TO,MA_NV,ma_kpi, LOAI_TINH, TONG, DA_GIAHAN_dung_hen,  DTHU_THANHCONG_DUNG_HEN, TYLE)
select a.*
from (
--****C.4(BHKV), C.4(KHDN) Ty le thue bao ghtt khong thanh cong tren tap KH thuoc BHKV, BHDN giao ds het han----
                ...
                from        (select thang, a.ma_to, a.ma_pb, a.thuebao_id, a.ma_tb, sum(tien) DTHU, max(ngay_tt) ngay_tt
                                         ...
                                         from SLDH_BHOL a
                                        where thang = 202412 and ma_To = 'VNP0703003'
