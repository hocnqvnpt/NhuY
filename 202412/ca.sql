delete from ttkd_bsc.ct_bsc_giahan_cntt where thang = 202412;
commit;
----- TAO BANG CHI TIET BSC
insert into ttkd_bsc.ct_bsc_giahan_cntt
select
202412 thang, 0 GH_ID, null PBH_QL_ID, a.DONVI_GIAO PBH_GIAO_ID, null, a.PBH_ID_TH PBH_TH_ID, null, a.ma_tb, null, null
...
from ttkdhcm_ktnv.ghtt_chotngay_271 a
            left join ttkd_bsc.nhanvien b on a.MANV_HRM = b.ma_nv and b.thang = 202412 and b.donvi='TTKD' -- CHANGE
            ...
            left join ttkd_bsc.nhanvien nv1 on nv.ma_nv = nv1.ma_nv AND NV1.THANG = 202412 AND NV1.DONVI = 'TTKD'-- CHANGE
...
where ngay_chot =to_date('20250102','yyyymmdd') and thang_kt = 202412 and a.loaitb_id in (147,148);
and a.thang_kt in (202412)   and (a.loaitb_id in  (55 ,80 ,116 ,117,132,140,154,181,318 ) 
...
;  00965446
select * from ttkd_Bsc.ct_bsc_giahan_cntt where thang = 202412 and tien_khop = 1 and ht_Tra_id = 2 and kenhthu_id = 23 and ma_gd not in (select ma from ttkdhcm_ktnv.ds_chungtu_Nganhang_phieutt_Hd_1)
...
where thang = 202412 and a.phieutt_id is not null and TIEN_KHOP = 0 and  a.loaitb_id in  (55 ,80 ,116 ,117,132,140,154,181,288,318,147,148 ) 
...
select* from ttkd_Bsc.ct_bsc_giahan_Cntt where thang = 202412 and tien_khop = 0;
update ttkd_Bsc.ct_bsc_giahan_Cntt set tien_khop = 1 where thang = 202412 and tien_khop = 0 and kenhthu_id = 10; 
--- btcn
update ttkd_Bsc.ct_bsc_giahan_Cntt set tien_khop = 1 where thang = 202412 and ma_gd in ('01041766','01023980');
--- id600

update ttkd_bsc.CT_bSC_GIAHAN_CNTT a set tien_khop = 1
-- select* from ttkd_bsc.CT_bSC_GIAHAN_CNTT a
where  ht_tra_id in (2,4,5,207) and tien_khop = 0  and thang = 202412-- and loaitb_id in (147,148)--and rkm_id in (select rkm_id from tmp3_30ngay where lan = 22)
and exists 
    (
...
delete from ttkd_Bsc.tl_giahan_tratruoc where thang = 202412 and ma_kpi in ('HCM_TB_GIAHA_024','HCM_TB_GIAHA_026');
commit;
--- cap nhat tay
update ttkd_Bsc.ct_bsc_giahan_cntt set tien_khop = 1
where thang = 202412 and ma_tb='hcm_tmvn_00002032';
commit;
update ttkd_Bsc.ct_bsc_giahan_cntt set tien_khop = 1
where thang = 202412 and ma_tb in ('hcm_ca_ivan_00019983','hcm_ivan_00029682');
--- kiem tra du lieu
select* from ttkd_bsc.ct_bsc_giahan_cntt x
where thang = 202412 and tien_khop = 0 and  exists (select 1 from ttkdhcm_ktnv.ghtt_chotngay_271 a
                                                where  ngay_chot =to_date('20241202','yyyymmdd') --and thang_kt = 202412 and a.loaitb_id in (147,148)
and a.thang_kt in (202412)   and (a.loaitb_id in  (55 ,80 ,116 ,117,132,140,154,181,318 ) 
                   or (a.loaitb_id in (288) and tocdo_id not in (17187) ) )
                   and ma_tb = x.ma_tb and tien_khop=1);
-- vet
update ttkd_Bsc.ct_Bsc_giahan_cntt set tien_khop = 1 where thang = 202412 and ma_gd in ('HCM-LD/01669720','HCM-LD/01805418');
commit;
select * from ttkd_bsc.ct_bsc_giahan_cntt x
where thang = 202412 and tien_khop = 0 and  exists (select 1 from ttkdhcm_ktnv.ghtt_chotngay_271 a
                                                where  ngay_chot =to_date('20241202','yyyymmdd') --and thang_kt = 202412 and a.loaitb_id in (147,148)
...
and a.thang_kt in (202412)   and (a.loaitb_id in  (55 ,80 ,116 ,117,132,140,154,181,318 ) 
                   or (a.loaitb_id in (288) and tocdo_id not in (17187) ) )
                   and ma_tb = x.ma_tb and tien_khop=1));
update ttkd_Bsc.ct_Bsc_giahan_cntt set tien_khop = 1 where ma_gd in ('00862640','HCM-TT/02814473')  and thang=202412;
   commit; 

 -- CA 
 delete from ttkd_Bsc.tl_Giahan_Tratruoc where thang = 202412 and ma_kpi ='HCM_TB_GIAHA_024';
...
from       (select thang, THANG_KTDC_CU, a.pbh_giao_id, a.phong_giao, a.manv_giao ma_nv, a.ma_to, a.ma_pb, a.thuebao_id, a.ma_tb, tien_dc_cu, sum(tien_thanhtoan) DTHU, max(ngay_tt) ngay_tt
                ...
                where a.thang = 202412 and loaitb_id in (55 ,80 ,116 ,117,132,140,154,181,288,318 ) and thang_ktdc_cu in (202412)--(147,148 )  and manv_GIAO = 'VNP017400' ------------n------------ 
...
insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TONG, DA_GIAHAN_DUNG_HEN
            ...
            where thang = 202412 and MA_KPI in ('HCM_TB_GIAHA_024') --and  ma_to ='VNP0702302'  --, 'HCM_TB_GIAHA_025') 
            group by THANG, MA_KPI, MA_TO, MA_PB 
;
commit;
rollback;
select* from bk_ca;
SELECT* from ttkd_Bsc.tl_giahan_tratruoc where thang = 202412 and ma_kpi = 'HCM_TB_GIAHA_024' and loai_tinh in  ('KPI_PB','KPI_TO') ;
select* from ttkd_bsc.tl_giahan_tratruoc a
    join ttkd_Bsc.nhanvien b on a.ma_nv = b.ma_Nv and b.thang = 202412 and b.donvi = 'TTKD'
where a.thang = 202412 and ma_kpi = 'HCM_TB_GIAHA_024' and loai_tinh = 'KPI_PB' 
ORDER BY TEN_NV;
ROLLBACK;
select sum(da_giahan_dung_hen) from TTKD_BSC.tl_giahan_tratruoc where thang = 202412  and ma_kpi = 'HCM_TB_GIAHA_024' and loai_tinh = 'KPI_PB'; 
select * from TTKD_BSC.tl_giahan_tratruoc where thang = 202412  and ma_kpi = 'HCM_TB_GIAHA_024' and loai_tinh = 'KPI_PB'; 

select * FROM  ttkd_bsc.blkpi_dm_to_pgd WHERE MA_KPI = 'HCM_TB_GIAHA_024';
commit;
ROLLBACK;
delete from TTKD_BSC.tl_giahan_tratruoc where thang = 202412  and ma_kpi = 'HCM_TB_GIAHA_024' and loai_tinh = 'KPI_PB';
--- 9 PHONG BHKV
insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TONG, DA_GIAHAN_DUNG_HEN
...
from ttkd_bsc.tl_giahan_tratruoc a left join (select distinct ma_kpi, ma_to,ma_pb, ma_nv, thang from ttkd_bsc.blkpi_dm_to_pgd 
                       WHERE ma_kpi = 'HCM_TB_GIAHA_024' AND PHUTRACH = 'Phong'  AND THANG = 202412  )
            b on a.thang = b.thang 
and a.ma_pb = b.ma_pb
where a.thang = 202412 and loai_tinh = 'KPI_TO' and a.MA_KPI in ('HCM_TB_GIAHA_024') AND A.MA_PB NOT IN ('VNP0702300','VNP0702400','VNP0702500') --, 'HCM_TB_GIAHA_025') --and b.ma_nv = 'VNP017819'
group by a.THANG, a.MA_KPI, a.MA_PB, b.ma_nv
;
COMMIT;
SELECT* FROM ttkd_bsc.blkpi_dm_to_pgd 
                       WHERE ma_kpi = 'HCM_TB_GIAHA_024' AND  THANG = 202412;
...
where a.thang = 202412 and loai_tinh = 'KPI_TO' and a.MA_KPI in ('HCM_TB_GIAHA_024') AND A.MA_PB IN ('VNP0702300','VNP0702400','VNP0702500') --, 'HCM_TB_GIAHA_025') --and b.ma_nv = 'VNP017819'
group by a.THANG, a.MA_KPI, a.MA_PB, b.ma_nv
ORDER BY A.MA_PB
;
commit;
...
from (select thang, THANG_KTDC_CU, a.pbh_giao_id, a.phong_giao, a.manv_giao ma_nv, a.ma_to, a.ma_pb, a.thuebao_id, a.ma_tb, tien_dc_cu, sum(tien_thanhtoan) DTHU, max(ngay_tt) ngay_tt
                                    , decode(sum(a.tien_khop), 0, 0, null, 0, 1) tien_khop
                                                    , sum(a.tien_khop)
                                    from ttkd_bsc.ct_bsc_giahan_cntt a
                                    where a.thang = 202412 and loaitb_id in (147,148) and thang_ktdc_cu = 202412 --and ma_pb ='VNP0702400'--  AND MANV_GIAO = 'VNP017400'------------n------------ 
                                   group by thang, THANG_KTDC_CU, a.pbh_giao_id, a.phong_giao, a.manv_giao, a.ma_to, a.ma_pb, a.thuebao_id, a.ma_tb, tien_dc_cu
                    ) a 
where ma_pb is not null 
group by a.thang, a.THANG_KTDC_CU, a.ma_nv, a.ma_to, a.ma_pb ;
...
insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TONG, DA_GIAHAN_DUNG_HEN
            ...
            select THANG, 'KPI_TO', MA_KPI, null, MA_TO, MA_PB, sum(TONG), sum(DA_GIAHAN_DUNG_HEN), sum(DTHU_DUYTRI), sum(DTHU_THANHCONG_DUNG_HEN)
                                ...
                                , sum(heso_giao) heso
            from ttkd_bsc.tl_giahan_tratruoc
            where thang = 202412 and MA_KPI in ('HCM_TB_GIAHA_026') --and ma_pb ='VNP0702400'-- AND MA_TO = 'VNP0701603'
            group by THANG, MA_KPI, MA_TO, MA_PB
;

delete FROM ttkd_bsc.tl_giahan_tratruoc a where a.thang = 202412 and a.MA_KPI in ('HCM_TB_GIAHA_026') and loai_tinh = 'KPI_PB'
...
select a.THANG, 'KPI_PB', a.MA_KPI,  b.ma_nv, null MA_TO, a.MA_PB, sum(TONG) tong , sum(DA_GIAHAN_DUNG_HEN) DA_GIAHAN_DUNG_HEN,  sum(DTHU_DUYTRI)DTHU_DUYTRI, 
        sum(DTHU_THANHCONG_DUNG_HEN) DTHU_THANHCONG_DUNG_HEN, round(sum(DA_GIAHAN_DUNG_HEN)*100/sum(TONG), 2) tyle, b.ma_nv, sum(heso_giao)heso_giao
from ttkd_bsc.tl_giahan_tratruoc a left join (select distinct ma_kpi, ma_to,ma_pb, ma_nv, thang from ttkd_bsc.blkpi_dm_to_pgd 
                       WHERE ma_kpi = 'HCM_TB_GIAHA_026' AND phutrach = 'Phong'    )
            b on a.thang = b.thang 
and a.ma_pb = b.ma_pb
where a.thang = 202412 and loai_tinh = 'KPI_TO' and a.MA_KPI in ('HCM_TB_GIAHA_026') AND A.MA_PB NOT IN ('VNP0702300','VNP0702400','VNP0702500') --, 'HCM_TB_GIAHA_025') --and b.ma_nv = 'VNP017819'
group by a.THANG, a.MA_KPI, a.MA_PB, b.ma_nv
;
COMMIT;
--- 3 PHONG DOANH NGHIEP
...
from ttkd_bsc.tl_giahan_tratruoc a left join (select distinct ma_kpi, ma_to,ma_pb, ma_nv, thang from ttkd_bsc.blkpi_dm_to_pgd 
            ...
            b on a.thang = b.thang and a.ma_pb = b.ma_pb AND A.MA_TO = B.MA_TO
where a.thang = 202412 and loai_tinh = 'KPI_TO' and a.MA_KPI in ('HCM_TB_GIAHA_026') AND A.MA_PB IN ('VNP0702300','VNP0702400','VNP0702500') --, 'HCM_TB_GIAHA_025') --and b.ma_nv = 'VNP017819'
group by a.THANG, a.MA_KPI, a.MA_PB, b.ma_nv
ORDER BY A.MA_PB
;
;
...
set 
 ...
 ,tyle_thuchien = (select round(sum(DA_GIAHAN_DUNG_HEN)*100/sum(TONG), 2) 
                                                    from ttkd_bsc.tl_giahan_tratruoc where thang = to_char(trunc(sysdate, 'month')-1, 'yyyymm') 
                                                                    and ma_nv = a.ma_nv and ma_kpi = 'HCM_TB_GIAHA_024')
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_TB_GIAHA_024') and thang = 202412 and MA_VTCV = a.MA_VTCV)
...
, tyle_thuchien  = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
                                                            where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_TO' 
                                                                                and ma_to = a.ma_to and ma_pb = a.ma_pb and ma_kpi = 'HCM_TB_GIAHA_024')
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where to_truong_pho = 1 and ma_kpi in ( 'HCM_TB_GIAHA_024') 
                                and thang = 202412 and MA_VTCV = a.MA_VTCV) and ma_kpi = 'HCM_TB_GIAHA_024';
--- pho giam doc
...
set thuchien = (select da_Giahan_dung_Hen from ttkd_bsc.tl_giahan_tratruoc 
    ...
    ,tyle_thuchien = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
     ...
     , giao = (select tong from ttkd_bsc.tl_giahan_tratruoc 
                                                            where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_TO' 
                                                                                and ma_to = a.ma_to and ma_pb = a.ma_pb and ma_kpi = 'HCM_TB_GIAHA_024')                                                                 
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where to_truong_pho = 1 and ma_kpi in ( 'HCM_TB_GIAHA_027') 
                                and thang = 202412 and MA_VTCV = a.MA_VTCV) and ma_kpi = 'HCM_TB_GIAHA_027';
commit;
select* from ttkd_Bsc.tl_Giahan_Tratruoc where thang = 202412 and ma_kpi = 'HCM_TB_GIAHA_024' and loai_tinh = 'KPI_PB';
update TTKD_BSC.bangluong_kpi a 
set tyle_thuchien = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
...
where thang = 202412 and ma_kpi ='HCM_TB_GIAHA_027' ;
COMMIT;
...
set 
 ...
 ,tyle_thuchien = (select round(sum(DA_GIAHAN_DUNG_HEN)*100/sum(TONG), 2) 
                                                    from ttkd_bsc.tl_giahan_tratruoc where thang = to_char(trunc(sysdate, 'month')-1, 'yyyymm') 
                                                                    and ma_nv = a.ma_nv and ma_kpi = 'HCM_TB_GIAHA_024')
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_TB_GIAHA_028') and thang = 202412 and MA_VTCV = a.MA_VTCV)
...
, tyle_thuchien  = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
                                                            where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_TO' 
                                                                                and ma_to = a.ma_to and ma_pb = a.ma_pb and ma_kpi = 'HCM_TB_GIAHA_024')
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where to_truong_pho = 1 and ma_kpi in ( 'HCM_TB_GIAHA_028') 
                                and thang = 202412 and MA_VTCV = a.MA_VTCV) and ma_kpi = 'HCM_TB_GIAHA_028' and ten_nv like '%Kiên';
--- DIEM CONG DIEM TRU
update ttkd_Bsc.bangluong_kpi  
...
where ma_kpi ='HCM_TB_GIAHA_028' AND THANG = 202412 ;
...
where thang = 202412 and ma_kpi ='HCM_TB_GIAHA_028' and ma_vtcv = 'VNP-HNHCM_KHDN_4' and ma_nv != 'VNP017445';
delete from ttkd_Bsc.bangluong_kpi where thang = 202412 and ma_kpi ='HCM_TB_GIAHA_028' and ma_vtcv = 'VNP-HNHCM_KHDN_4' and ma_nv != 'VNP017445';
--- TEN MIEN
--- NHAN VIEN
update TTKD_BSC.bangluong_kpi a 
set 
...
, giao =  (select round(sum(tong))
                   from ttkd_bsc.tl_giahan_tratruoc where thang = to_char(trunc(sysdate, 'month')-1, 'yyyymm') 
                    and ma_nv = a.ma_nv and ma_kpi = 'HCM_TB_GIAHA_026')
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_TB_GIAHA_026') and thang = 202412 and MA_VTCV = a.MA_VTCV) 
...
, giao =  (select round(sum(tong))
 ...
 , tyle_thuchien= (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
                                                            where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_TO' 
                                                                                and ma_to = a.ma_to and ma_kpi = 'HCM_TB_GIAHA_026')                                                                               
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where to_truong_pho = 1 and ma_kpi in ( 'HCM_TB_GIAHA_026') 
                                and thang = 202412 and MA_VTCV = a.MA_VTCV) and ma_kpi = 'HCM_TB_GIAHA_026' 
;
--- PHO GIAM DOC
update TTKD_BSC.bangluong_kpi a 
set tyle_thuchien = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
...
where ma_kpi ='HCM_TB_GIAHA_026' AND THANG = 202412; 
;
...
where ma_kpi ='HCM_TB_GIAHA_026' AND THANG = 202412 ;
  ;
COMMIT;
SELECT* FROM TTKD_bSC.BANGLUONG_KPI A WHERE THANG = 202412 and ma_kpi = 'HCM_TB_GIAHA_024';
