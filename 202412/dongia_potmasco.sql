...
--- thang 11: tien thuyet phuc cong doan tra truoc la 6k
with ds as (
    select THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, PHONG_QL, THUEBAO_ID, MA_TB, TIEN_DC_CU, MANV_GIAO, MA_NV_CN, MANV_THUYETPHUC, SOTHANG_DC, 
    HT_TRA_ONLINE, KHUVUC,  DONGIA, DTHU, NGAY_TT, HESO_GIAO, KHDN, NHOMTB_ID, KHACHHANG_ID, HESO_DICHVU
    , TIEN_KHOP, GHICHU, TYLE_THANHCONG, HESO_CHUKY,
    NHANVIEN_XUATHD, null TIEN_XUATHD, case when tien_khop = 1 and dongia =10000 then 6000 else  TIEN_THUYETPHUC end TIEN_THUYETPHUC, IPCC
    from ttkd_bsc.ct_dongia_tratruoc a where thang =  202412 and loai_tinh = 'DONGIATRA_OB' and 
        (exists (select 1 from ttkd_bsc.nhanvien where thang = 202412 and donvi = 'POT' and ma_nv = a.ma_nv) or 
        exists (select 1 from ttkd_bsc.nhanvien where thang = 202412  and donvi = 'POT' and ma_nv = a.nhanvien_xuathd))
)
    ...
    from ds
        left join ttkd_bsc.nhanvien b on ds.ma_nv= b.ma_Nv and ds.thang = b.thang and b.donvi = 'POT'
--        left join admin_hcm.nhanvien_onebss nv on ds.ma_nv = nv.ma_nv
--        left join admin_hcm.donvi dv1 on nv.donvi_id = dv1.donvi_id
;
select distinct loai_tinh from ttkd_Bsc.dongia_Vttp where thang = 202412;
--- TRA SAU CHUYEN TRA TRUOC
insert into ttkd_Bsc.dongia_Vttp(THANG, LOAI_TINH, MA_KPI, MA_NV, THUEBAO_ID, MA_TB, MANV_THUYETPHUC, SOTHANG_DC,DONGIA, DTHU, NGAY_TT,KHACHHANG_ID, HESO, TIEN_KHOP, TIEN_DONGIA, 
TEN_NV, DV_CAP1, DV_CAP2, DONVI_ID, HESO_CHUKY, MANV_XHDON, TIEN_XHDON)
with ds as (
    select* from ttkd_bsc.ct_dongia_tratruoc a where thang =  202412 and loai_tinh = 'DONGIA_TS_TP_TT' and 
        (exists (select 1 from ttkd_bsc.nhanvien where thang = 202412 and donvi = 'POT' and ma_nv = a.ma_nv) or 
        exists (select 1 from ttkd_bsc.nhanvien where thang = 202412 and donvi = 'POT' and ma_nv = a.nhanvien_xuathd))
)
...
rollback;
select ma_tb, 
SELECT * FROM  ttkd_Bsc.dongia_Vttp A where loai_Tinh IN ('DONGIA_TS_TP_TT', 'DONGIA_POTMASCO' ) ;and ROUND(DONGIA*HESO_CHUKY*HESO)!= TIEN_DONGIA; GROUP BY THUEBAO_ID HAVING  COUNT(MA_nV)>1;
------
DELETE from
--select*  from
ttkd_Bsc.dongia_Vttp A where loai_Tinh = 'DONGIA_POTMASCO' and thang = 202412 and exists (select 1 from ttkd_bsc.dongia_vttp where thang = a.thang and ma_nv = a.ma_nv
...
set TIEN_DONGIA = 0
where ma_nv not in (Select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202412 ) and thang = 202412 and loai_tinh = 'DONGIA_POTMASCO';
commit;
update ttkd_Bsc.dongia_vttp 
set TIEN_XHDON = 0
where nvl(MANV_XHDON,'a') not in (Select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang =  202412) and thang = 202412 and TIEN_XHDON > 0 and loai_tinh = 'DONGIA_POTMASCO';
...
delete ttkd_Bsc.dongia_vttp where thang = 202412 and loai_tinh = 'DONGIATRA_OB' and ;
delete from ttkd_Bsc.dongia_vttp where thang = 202412 and loai_Tinh  ='DONGIATRA_OB' and ( ma_Tb,ma_Nv) in (
select ma_Tb, ma_Nv from ttkd_Bsc.dongia_vttp where thang = 202412 and loai_tinh in ( 'DONGIATRA_OB','DONGIA_POTMASCO')  
GROUP BY MA_tB , ma_nv HAVING COUNT(MA_NV) > 1) ;
--delete from
--select* from 
ttkd_Bsc.dongia_vttp a 
where thang=202412 and  loai_tinh in ( 'DONGIA_TS_TP_TT','DONGIA_POTMASCO') ; and ma_nv not in (select ma_Nv from ttkd_Bsc.nhanvien_vttp_potmasco);
...
select * from ttkd_Bsc.dongia_vttp where thang = 202412 and donvi = 'TTKD'  ;
COMMIT;
select * from ttkd_bsc.bldg_danhmuc_vtcv_p1 where thang = 202408;
select * from ttkd_Bsc.nhanvien where thang = 202412 and  donvi = 'TTKD' AND TEN_VTCV ='Nhân viên qu?n lý ?i?m bán';
select* from ttkd_Bsc.dongia_vttp where thang = 202412 and loai_tinh= 'DONGIATRA_OB'
