...
(
    ...
    where a.trangthaitb_id not in (7,8,9) and b.chuquan_id = 145 and a.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224) and to_number(to_char(a.ngay_sd,'yyyymm')) <= 202408
)
...
--													   left join ct on p.phieutt_id = ct.phieu_id
                     ...
                     hdtb.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224) and a.tien > 0 --and hdtb.thuebao_id = 8811198
                                      and  to_number(to_char(p.ngay_tt, 'yyyymm')) = 202412                 ----change--2 thang- ngay 02
...
, PHIEUTT_ID, TIEN_KHOP,  HT_TRA_ID, KENHTHU_ID, loaitb_id)
...
select 202412 thang,  -1 gh_id, dbtb.ma_tb, a.thuebao_id
...
--                left join c on a.thuebao_id = c.thuebao_id
            left join ttkd_bsc.nhanvien nv on a.MANV_THUYETPHUC = nv.ma_nv and thang = 202412 ; 
 commit;
 --- update chung tu
-- 4. TRA SAU THUYET PHUC TRA TRUOC
rollback;
UPDATE TTKD_bSC.ct_bsc_trasau_tp_tratruoc set tien_khop = 1 WHERE THANG = 202412 AND tien_khop = 0 and ma_Gd in (
...
select tien_Ct,tien_tt from ttkdhcm_ktnv.ds_chungtu_Nganhang_oneb where chungtu_id = 484294;
 delete from  TTKD_BSC.CT_DONGIA_TRATRUOC where thang = 202411 and loai_tinh=  'DONGIA_TS_TP_TT';
...
--                                            left join  km on a.rkm_id = km.rkm_id
                        where a.thang = 202412-- and --a.rkm_id is not null AND -- don gia TP 
...
commit;
delete from ttkd_bsc.tl_giahan_Tratruoc where thang = 202412 and loai_tinh=  'DONGIA_TS_TP_TT';
...
where ma_kpi = 'DONGIA' --and loaitb_id in (58,59)
            and loai_tinh in ('DONGIA_TS_TP_TT') and a.thang = 202412 and b.ma_pb not in ('VNP0702300','VNP0702400','VNP0702500')
