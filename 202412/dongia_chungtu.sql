...
rollback;
commit;
create table xxx_chungtu_202410 as select* from xx_chungtu_202410;
create table ct_trsau as ;
update xx_chungtu_202410 set tinh_Bsc = 0, tinh_dongia = 0
--select distinct chungtu_id from ct_Bsc_Chungtu
where tratruoc = 0 and thang = 202410 and (chungtu_id, ma_tt) in (
-- lay danh sach chung tu id600, bo hinh thuc UNC/UNT theo file
select a.chungtu_id,
--                a.ma_nh nganhang,to_char(a.ngay_ct,'dd/mm/yyyy') ngay_nh,a.ma_ct ma_chungtu,a.tien_ct tien_chungtu,c.hinhthuc hinhthuc_tt,to_char(b.ngay_tt,'dd.mm/yyyy') ngay_tt
                b.ma_tt--,b.ma_tb
...
--                ,b.tragoc,b.trathue,b.tongtra, c.httt_id
                ...
                where a.chungtu_id = b.chungtu_id and ma_ct ='VCB_20240826_315992'
                            and b.httt_id = c.httt_id
                            and to_number(to_char(b.ngay_tt, 'yyyymm')) = 202410 and tongtra> 0 and  c.httt_id  in (170, 171)
                            )
                            
  -- doan lay httt_id = 192 KBNN
;
select* from qltn_Hcm.bangphieutra where ma_tt='HCM004682190' and ky_cuoc = 20240901;
select* from ttkd_Bsc.nhanvien where ma_nv ='CTV041777';
SELECT* FROM ct_trsau WHERE THANG = 202410;
commit;
