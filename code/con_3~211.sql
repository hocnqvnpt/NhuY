                    ...
                    with 
                      ...
                      --  select * from dc1 where rownum = 1
                        select dc.*, ts.sl_datcoc, pkg.nhomtb_id, pkg.goi_id, db.trangthaitb_id, db.khachhang_id--, pttt.ht_tra_id, pttt.kenhthu_id
                                    ...
                                    where  to_number(to_char(ngay_kt_mg, 'yyyymm')) >= 202401 ----thang n, n thang chay du lieu
                                                       -- and rownum = 1
--                                                    and dc.thuebao_id in ('11736290')
                                             
        ;
        
        select RKM_ID||',', thuebao_id, ma_tb from ds_giahan_tratruoc2 group by  RKM_ID, thuebao_id, ma_tb having count(*)>1;
