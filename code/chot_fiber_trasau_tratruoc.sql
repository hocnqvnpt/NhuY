...
select * from hocnq_ttkd.tmp3_ts;
create table tmp3_ts as  select * from hocnq_ttkd.tmp3_ts where 1 =2
select* from tmp3_ts
select rkm_id from tmp3_ts
group by rkm_id having count(rkm_id) > 1
select* from ttkd_bsc.ct_bsc_Tratruoc_moi_30day where tien_thanhtoan < 20000 and thang = 202310
...
from (select  --gh.khachhang_id, gh.thuebao_id, gh.duan_id, gh.ma_tb, gh.ma_tt, gh.loaitb_id, gh.thang_kt
         ...
         from css_hcm.phieutt_hd a, css_hcm.ct_phieutt b
                    ...
                    , (select distinct thuebao_id, ma_tb 
                          ...
                          where thang_kt=0 and to_number(to_char(ngay_cn, 'yyyymm')) = 202311   ---change n
                                        ) gh                                             
         ...
         where a.kenhthu_id not in (6) and a.trangthai = 1
               ...
               --         and to_number(to_char(a.ngay_tt, 'yyyymm')) = 202308                      ----change thang n---
                        and ((a.ht_tra_id <> 2 and to_number(to_char(a.ngay_tt, 'yyyymm')) = 202311)                     ----change thang n---
                                        or (a.ht_tra_id in (2,4,5,9) and to_number(to_char(ct.ngay_nganhang, 'yyyymm')) =202311))                     ----change thang n---
...
select *   from ttkd_bct.ds_ketqua_capnhat_dai a where thang_kt = 0 and to_number(to_char(ngay_cn, 'yyyymm')) = 202203 ;
                                        max idno
                                        ;MA_TT_DHSXKD
select *  from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202310; and rkm_id  in (select rkm_id from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202111);
alter table ttkd_bsc.ct_bsc_trasau_tp_tratruoc rename column NGAY_HD to ngay_nganhang;
create table ct_bsc_trasau_tp_tratruoc as select* from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where 1= 2
        ...
        , PHIEUTT_ID, TIEN_KHOP, MA_CHUNGTU, HT_TRA_ID, KENHTHU_ID)
            ...
            with t0 as (select c0.thuebao_id, c0.phieutt_id, c0.ma_gd, c0.rkm_id, c0.ngay_bd_moi, c0.tien_thanhtoan, c0.vat
                                       ...
                                       from tmp3_ts c0
                                                    ...
                                                    left join admin_hcm.nhanvien nv2 on nv2.nhanvien_id = c0.nhanviengt_id
                ...
                , ds as (select a.*, w.ma_tt, rank() over (partition by a.ma_tb ORDER BY a.idno  desc) rnk  
                                      ...
                                      where a.thang_kt=0 and to_number(to_char(a.ngay_cn, 'yyyymm')) = 202311 ---thang n--change anh Huyen, Duc Dung moi thang -------
                                    )
...
--                                , (select * from ttkd_bsc.dm_to where hieuluc =1 and tbh_id = dbtb.tbh_ql_id) mato_ql
--                                , (select * from ttkd_bsc.dm_phongban where active =1 and pbh_id = dbtb.pbh_ql_id) mapb_ql
                 ...
                 from ds  a  
                           --     join css_hcm.db_thuebao dbtb on a.thuebao_id = dbtb.thuebao_id and dbtb.loaitb_id in (58, 59)
                            ...
                            left join ttkd_bsc.nhanvien_202311 nv on c.MANV_THUYETPHUC = nv.ma_nv   ---change thang n----
            where a.rnk = 1 
...
;
commit;
select * from ct_bsc_trasau_tp_tratruoc where thang = 202310; --and ma_pb = 'VNP0701100'; --fiber40_2019999

select thuebao_id from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202310 group by thuebao_id having count(*) >1
;
commit;
     select * from  ct_bsc_tratruoc_moi_30day a  where thang = 202311 and TIEN_THANHTOAN < 20000
;


---check-
    ---- loai tru  ptm n-1, n
delete from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202310 and thuebao_id in (
        select a.thuebao_id from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a join css_hcm.db_thuebao b on a.thuebao_id = b.thuebao_id where thang = 202310 and to_number(to_char(b.ngay_sd, 'yyyymm')) >= 202309);
select b.ngay_sd, a.* from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a join css_hcm.db_thuebao b on a.thuebao_id = b.thuebao_id where thang = 202310 and to_number(to_char(b.ngay_sd, 'yyyymm')) >= 202309;
...
where rkm_id is not null and thang = 202310 and exists (select 1 from v_thongtinkm_all where cuoc_dc > 0 
                                                                              ...
                                                                              --      and to_date('01/07/2023', 'dd/mm/yyyy') between ngay_bddc and least(NGAY_ktdc, nvl(NGAY_THOAI -1, sysdate + INTERVAL '50' YEAR), nvl(NGAY_huy -1, sysdate + INTERVAL '50' YEAR))
                                                                                    and ngay_bddc <= to_date('30/09/2023', 'dd/mm/yyyy')
...
--delete
--select* 
from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202312 and thuebao_id in (
select a.thuebao_id from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a join css_hcm.db_thuebao b on a.thuebao_id = b.thuebao_id where a.thang = 202312 and to_number(to_char(b.ngay_sd, 'yyyymm')) >= 202311);

select b.ngay_sd, a.* from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a join css_hcm.db_thuebao b on a.thuebao_id = b.thuebao_id where thang = 202310 and to_number(to_char(b.ngay_sd, 'yyyymm')) >= 202309;
commit;
----loai tru tbao co tra truoc n-1
select b.* from abcdef a join v_thongtinkm_all b on a.thuebao_id = b.thuebao_id and a.thuebao_id in (4653624,5932610,8645295,9388950)
order by ngay_bd
select* from ttkd_bsc.tl_giahan_Tratruoc where thang = 202312 and loai_tinh = 'DONGLUCTS'
insert into abcdef
select a.* from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a 
-- delete from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a
where rkm_id is not null and thang = 202311 and exists (select 1 from v_thongtinkm_all where cuoc_dc > 0 
                                                       -- and 202306 between thang_bddc and least (thang_ktdc, nvl(thang_kt_dc, 999999), nvl(thang_huy, 999999))
                                                  --      and to_date('01/07/2023', 'dd/mm/yyyy') between ngay_bddc and least(NGAY_ktdc, nvl(NGAY_THOAI -1, sysdate + INTERVAL '50' YEAR), nvl(NGAY_huy -1, sysdate + INTERVAL '50' YEAR))
                                                        and ngay_bddc <= to_date('30/09/2023', 'dd/mm/yyyy')
                                                        and least(NGAY_ktdc, nvl(NGAY_THOAI -1, sysdate + INTERVAL '50' YEAR), nvl(NGAY_huy -1, sysdate + INTERVAL '50' YEAR))
        ...
        --     and a.thuebao_id  in (select thuebao_id from v_thongtinkm_all where cuoc_dc > 0 
           --                                             and 202208 between thang_bddc and least (thang_ktdc, nvl(thang_kt_dc, 999999), nvl(thang_huy, 999999)))
;
select distinct manv_thuyetphuc from abcdef
commit;      
