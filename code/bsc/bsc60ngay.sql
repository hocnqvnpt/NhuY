...
select * from hocnq_ttkd.v_thongtinkm_all where thang_bddc = 202310 and ma_tb = 'hiepem'
 ...
 select* from ttkd_bsc.tl_giahan_tratruoc where thang = 202405 and ma_kpi = 'HCM_TB_GIAHA_027';
...
select* from ttkd_bsc.tl_giahan_tratruoc where thang = 202408 and ma_kpi = 'HCM_TB_GIAHA_023' and loai_Tinh = 'KPI_NV';
rollback;
...
delete from ttkd_bsc.tl_giahan_tratruoc  where thang = 202408 and ma_kpi in ('HCM_TB_GIAHA_023');, 'HCM_TB_GIAHA_022','DONGIATRA','DONGIATRU')
...
-- BSC 60 NGAY: TO
-- select* from ttkd_bsc.ct_Bsc_Trasau_tp_tratruoc where thang = 202404
select distinct loai_tinh, ma_kpi from ttkd_bsc.tl_giahan_tratruoc where thang = 202404 and ma_kpi in ('HCM_TB_GIAHA_023','HCM_TB_GIAHA_022')
delete
...
where thang = 202408 and MA_KPI in ('HCM_TB_GIAHA_023')  and loai_tinh = 'KPI_NV' --and ma_TO = 'VNP0702219'
group by THANG, MA_KPI, MA_TO, MA_PB;  
update  ttkd_bsc.tl_giahan_tratruoc A set ma_to = (Select DISTINCT ma_To from ttkd_Bsc.nhanvien where thang = 202408 and ma_pb = a.ma_pb and ten_to = 'T? Sau Bán Hàng')
where thang = 202408 and  MA_KPI in ('HCM_TB_GIAHA_023')  and loai_tinh = 'KPI_TO' ;
SELECT* FROM TTKD_BSC.blkpi_dm_to_pgd WHERE THANG = 202408 AND DICHVU IS NULL; Select DISTINCT ma_To from ttkd_Bsc.nhanvien where thang = 202408 and ma_pb = a.ma_pb and ten_to = 'T? Sau Bán Hàng'
select distinct ma_to from ttkd_bsc.ct_bsc_tratruoc_moi where thang = 202310;
-- BSC 60 NGAY: PHONG BAN
    commit;
select SUM(DA_GIAHAN_DUNG_HEN)  from ttkd_bsc.tl_giahan_tratruoc a where a.MA_KPI in ('HCM_TB_GIAHA_023')and  thang = 202408 and loai_tinh ='KPI_TO';
delete  from ttkd_bsc.tl_giahan_tratruoc a where a.MA_KPI in ('HCM_TB_GIAHA_023')and  thang = 202408 and loai_tinh ='KPI_PB';
ROLLBACK;
commit;
select distinct ten_to, ma_to from ttkd_bsc.nhanvien where ma_pb = 'VNP0701100' and thang = 202408;
select *  
...
select a.THANG, 'KPI_PB', a.MA_KPI, b.ma_nv, null, a.MA_PB, sum(TONG), sum(DA_GIAHAN_DUNG_HEN), sum(DTHU_DUYTRI), sum(DTHU_THANHCONG_DUNG_HEN)
                    , round(sum(DA_GIAHAN_DUNG_HEN)*100/sum(TONG), 2)
                    , sum(heso_giao) heso
from ttkd_bsc.tl_giahan_tratruoc a left join (select DISTINCT ma_nv, ma_to, ma_kpi, thang, ma_pb  from ttkd_bsc.blkpi_dm_to_pgd where thang = 202408
                                                and dichvu is null) b on a.ma_to = b.ma_to --and a.ma_kpi = b.ma_kpi
where a.thang = 202408 and loai_tinh = 'KPI_TO' and a.MA_KPI in ('HCM_TB_GIAHA_023') -- and b.ma_nv = 'VNP017585'
