...
;
ROLLBACK;
select * from ttkd_Bsc.ct_dongia_Tratruoc where thang = 202403 and loai_tinh = 'DONGLUCTT'-- AND TIEN_KHOP =1 and (manv_Giao = manv_thuyetphuc or manv_thuyetphuc = 'khongco') 
commit;
...
and thang = 202403 --and ma_pb = 'VNP0701100'-- and ma_nv = 'VNP017802' --and ngay_tt < add_months(trunc(sysdate, 'month'), -1)
group by thang, LOAI_TINH, ma_kpi, ma_nv, ma_to, ma_pb
order by 2
--                  )
;
select* from ttkd_bsc.tl_giahan_tratruoc where thang = 202403 and ma_kpi = 'HCM_TB_GIAHA_023'
commit;