...
and not exists (select 1 from ttkd_bsc.ct_dongia_tratruoc where thang = 202401 and loai_tinh = 'DONGIATRA30D' and DONGIA > 0 and tien_khop = 1 and ma_nv is not null and a.thuebao_id = thuebao_id)
...
commit;
select sum(dongia) from ct_dongia_Tratruoc where thang = 202312 and loai_tinh = 'DONGIATRA30D'
---- TRA LUONG DON GIA
...
from ttkd_bsc.ct_dongia_tratruoc  a
where ma_kpi = 'DONGIA' 
            and loai_tinh in ('DONGIATRA') and thang = 202402
...
select* from ttkd_bsc.ct_Dongia_tratruoc where thang = 202309 