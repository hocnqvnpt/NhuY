select* from ttkd_bsc.ct_dongia_tratruoc where thang = 202403 and 'DONGLUCTS' = LOAI_TINH
...
--insert into ct_dongia_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, THUEBAO_ID, MA_TB, ma_nv_cn, manv_thuyetphuc, SOTHANG_DC, HT_TRA_ONLINE, dongia, DTHU, NGAY_TT, tien_khop)
        ...
        from (select thang, manv_thuyetphuc, ma_to, ma_pb, thuebao_id, ma_tb, manv_cn
                                ...
                                , sum(case when a.ht_tra_id in (1, 7,204) then 0
                                                                                when a.ht_tra_id in (2,4) then 1 end) ht_tra_online
                                , case
                                            when max(so_thangdc) >= 3 and max(so_thangdc) < 6 
                                                            and sum(case when a.ht_tra_id in (1, 7,204) then 0
                ...
                where rkm_id is not null and thang = 202403 and ma_Tb = 'ctylekhuong34'
...
delete 
--SELECT* 
from ttkd_Bsc.ct_dongia_tratruoc where thang = 202403 and loai_tinh = 'DONGLUCTS' AND ma_tb = 'ctylekhuong34'  --'THUEBAO_ID IN (SELECT * FROM ABCDEF)
commit;
...
insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TONG, DA_GIAHAN_DUNG_HEN, DTHU_DUYTRI, DTHU_THANHCONG_DUNG_HEN, TYLE, TIEN)
        select thang, LOAI_TINH, ma_kpi, ma_nv, ma_to, ma_pb
            ...
            --    select *
                ...
                where ma_kpi = 'DONGLUC' and loai_tinh = 'DONGLUCTS' and thang = 202403 AND MA_NV ='VNP016582'--IN (SELECT DISTINCT MANV_THUYETPHUC FROM ABCDEF) --and ma_nv = manv_thuyetphuc 
                 group by thang, LOAI_TINH, ma_kpi, ma_nv, ma_to, ma_pb
commit;

select distinct ma_kpi, loai_tinh from ttkd_bsc.ct_Dongia_Tratruoc where thang = 202402
select sum(tien) from ttkd_bsc.tl_giahan_tratruoc where thang = 202401 and ma_kpi = 'DONGIA';
