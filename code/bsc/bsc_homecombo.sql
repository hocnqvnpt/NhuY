...
select * from vietanhvh.PL4_0401@ttkddb where thang = 2024101;

select* from ttkd_bsc.nhanvien where (thang = 202410 and donvi = 'VTTP') OR (THANG = 202410 AND DONVI = 'TTKD');
commit;

select distinct loai_goi from ct_homecombo_2024 where thang = 202410; and ma_kpi = 'HCM_TB_GIAHA_024' AND MA_NV = 'VNP020231';
select* from ttkd_Bsc.nhuy_ct_bsc_ipcc_obghtt where thang ;
rollback;
    
update ttkd_bsc.ct_bsc_homecombo set ma_Nv = 'CTV021936',ma_to='VNP0701890', ma_pb ='VNP0701800' where thang = 202410 and ma_Tb = 'ng_minhhoang59';
commit;
select* from ct_homecombo_2024 where thang = 202410 and loai_kpi like 'Fiber_%';-- and account_fiber in ('p105d_ktxdhyd','buivantuan' )
insert into ttkd_bsc.ct_bsc_homecombo
select 202410 thang, 'Fiber_moi' loai_KPI, account_fiber, loai_goi, a.ngay_dk ngay_dk_goi, hd.ngay_sd ngay_sd_fiber, hd.thuonghieu, mnv_tt, ma_to, ma_pb, hd.hdtb_id,
hd.thuebao_id, hd.ctv_id, hd.hdkh_id, hd.tocdo_id, hd.goi_id
...
select* from ttkd_bsc.ct_bsc_homecombo where thang = 202410 and ma_Tb = 'ng_minhhoang59';
insert into ttkd_bsc.ct_bsc_homecombo;
select 202410 thang, 'Fiber_hh' loai_KPI, account_fiber, loai_goi, a.ngay_dk ngay_dk_goi, hd.ngay_sd ngay_sd_fiber, hd.thuonghieu, mnv_tt, ma_to, ma_pb, hd.hdtb_id, hd.thuebao_id, hd.ctv_id, hd.hdkh_id, hd.tocdo_id, hd.goi_id
    , null ghichu                                                                                           ----1-change----
--    , 'bsung n-1 ket goi, hoan cong n' ghichu                                                                                           ---or 2-change----
from ct_homecombo_2024 a
, (select hdgoi.ctv_id, hdgoi.goi_id, hdtb.hdtb_id, hdtb.thuebao_id, hdtb.ma_tb, hdtb.hdkh_id, td.tocdo_id, td.thuonghieu, db.ngay_sd
...
where a.thang = 202409 and trangthai_goi <> 'Huy' and a.account_fiber = hd.ma_tb      --change n, n-1------
        and trangthai_fiber != 'Moi' and loai_goi in  ('HOMECHAT4','HOMECHAT2','HOMECHAT6','HOMESANH1','HOMESANH2','HOMESANH3','HOMESANH4')
        and not exists (select * from ttkd_bsc.ct_bsc_homecombo where thuebao_id = hd.thuebao_id and thang >=202409)     ---change n-1------
;
commit;
select ma_tb from ttkd_bsc.ct_bsc_homecombo where thang = 202410 and loai_kpi like 'Fiber%' group by ma_tb having count(ma_Tb) > 1; --and ma_tb not in 
...
select* from ttkd_Bsc.ct_bsc_homecombo where thang = 202410; and ma_nv = 'VNP019514'
...
update dongia_Chungtu set thang = 202410;
...
select* from ttkd_bsc.tl_giahan_tratruoc where thang = 202410 and ma_kpi =  'HCM_SL_COMBO_006';
...
;
select* from admin_hcm.nguoidung where ma_Nd like '%daily_bocau%';
select* from admin_Hcm.nhanvien where nhanvien_id in (94471,94472,94473,5092);
select* from tmp3_ob where nvtuvan_id  in (94471,94472,94473,5092);
select* from ttkd_Bsc.ct_Dongia_Tratruoc where ma_Nv in ('daily_bocau','daily_bocau3','daily_bocau2') and thang in (202406,202407,202407,202409,202410);
select distinct manv_Thuyetphuc from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where nvtuvan_id  in (94471,94472,94473,5092);;
...
insert into ttkd_bsc.tl_giahan_tratruoc (thang, loai_tinh, ma_kpi, ma_nv, ma_to, ma_pb, DA_GIAHAN_DUNG_HEN)
    ...
    where thang = 202410 and LOAI_TINH = 'KPI_NV' and  ma_kpi = 'HCM_SL_COMBO_006' 
...
select  distinct ma_kpi, loai_tinh from ttkd_Bsc.tl_giahan_tratruoc where thang = 202410;-- and loai_tinh = 'KPI_PB'
commit;
select SUM(DA_GIAHAN_DUNG_HEN) from ttkd_Bsc.tl_giahan_tratruoc where thang = 202410 and ma_kpi = 'HCM_SL_COMBO_006'  and loai_tinh = 'KPI_TO';
delete from ttkd_bsc.tl_giahan_tratruoc where thang = 202410 and loai_tinh = 'KPI_PB' and ma_kpi = 'HCM_SL_COMBO_006' ;
commit;
ROLLBACK;
insert into ttkd_bsc.tl_giahan_tratruoc (thang, loai_tinh, ma_kpi, ma_nv, ma_to, ma_pb, DA_GIAHAN_DUNG_HEN)
    ...
    where a.thang = 202410 and LOAI_TINH = 'KPI_TO'  and  a.ma_kpi = 'HCM_SL_COMBO_006' 
    group by a.thang, a.ma_kpi, a.ma_pb, b.ma_nv
;
select* from ttkd_Bsc.tonghop_giao_372 where thang = 202410;
select* from ttkd_bsc.nhanvien where thang = 202410 and ma_pb ='VNP0703000';
select* from ttkd_Bsc.blkpi_danhmuc_kpi_vtcv where  ma_kpi = 'HCM_SL_COMBO_006' and thang = 202410 ;
...
and thang= 202410 and tien_khop = 0 ;
commit;
...
select distinct loai_tinh from ttkd_Bsc.tl_giahan_tratruoc where thang = 202410;
		select* from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi ='HCM_SL_COMBO_006' and thang_kt is null;
select distinct ma_nv, ma_to, ma_kpi, thang, ma_pb from ttkd_bsc.blkpi_dm_to_pgd where thang = 202410 and ma_kpi = 'HCM_SL_COMBO_006' and ma_to ='VNP07015A0';
select * from ttkd_bsc.blkpi_dm_to_pgd where thang = 202410 and ma_kpi = 'HCM_SL_COMBO_006' ;
...
rollback;
select* from ttkd_Bsc.bangluong_kpi_202410 where hcm_Tb_giaha_024 is not null; 
