...
select 202411 THANG, LOAI_TINH, MA_KPI, MA_NV, THUEBAO_ID, MA_TB, TIEN_DC_CU, MANV_GIAO, MA_NV_CN, MANV_THUYETPHUC, SOTHANG_DC, HT_TRA_ONLINE, KHUVUC, -DONGIA,
...
with ds as (
    select* from ttkd_bsc.ct_dongia_tratruoc a where thang =  202411 and loai_tinh = 'DONGIATRA_OB' and 
        (exists (select 1 from ttkd_bsc.nhanvien where thang = 202411 and donvi = 'VTTP' and ma_nv = a.ma_nv) or 
        exists (select 1 from ttkd_bsc.nhanvien where thang = 202411 and donvi = 'VTTP' and ma_nv = a.nhanvien_xuathd))
)
...
set TIEN_DONGIA = 0
where ma_nv not in (Select ma_nv from ttkd_bsc.nhanvien where thang = 202411 and donvi = 'VTTP') and thang = 202411;
commit;
update ttkd_Bsc.dongia_vttp 
set TIEN_XHDON = 0
where nvl(MANV_XHDON,'a') not in (Select ma_nv from ttkd_bsc.nhanvien where thang =  202411 and donvi = 'VTTP') and thang = 202411 and TIEN_XHDON > 0;
...
with ds as(
     select b.* from thuhoi_Bsc_dongia a 
            ...
            where a.thang = 202410 and a.ma_pb is null
)
...
select* from ttkd_bsc.dongia_Vttp a where thang = 202411; and tien_dongia > 0;
...
with tien as (
    ...
    from ttkd_bsc.dongia_vttp where thang = 202411
--    and tien_dongia > 0
    union all
...
,sl as (
    ...
    from  ttkd_Bsc.dongia_vttp 
    where thang = 202411 and manv_xhdon is not null and manv_xhdon != ma_nv
)
,abc as (
...
with tien as (
    select thang,ma_Nv, tien_dongia
    from ttkd_Bsc.dongia_vttp 
    where thang = 202411  --and tien_dongia > 0
    union all
    select thang,MANV_XHDON,  TIEN_XHDON
    from ttkd_Bsc.dongia_vttp 
    where thang = 202411 
