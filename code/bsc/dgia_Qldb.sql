select sum(hcm_luong_qldb_062) from ttkd_bsc.bangluong_dongia_qldb where thang = 202311
...
;
-- Table backup_tonghop_dongia
drop table backup_tonghop_dongia purge;

CREATE TABLE "backup_tonghop_dongia"(
  "THANG" Number,
...
;
select* from  TTKD_BSC.blkpi_danhmuc_kpi_vtcv where donvi = 'tyle' and loai='dongia' and thang_kt is null and ma_kpi = 'HCM_LUONG_QLDB_053'
...
rollback;
    ----tao du lieu view tong quan cac gia KPI
            ...
            delete from backup_tonghop_dongia where thang = 202403;
            commit;
          select* from backup_tonghop_dongia  where thang = 202401 and ma_nv = 'VNP016534'
          insert into ttkd_bsc.bangluong_dongia_qldb (thang) values (-10)
          delete from backup_tonghop_dongia where thang = 202311
              select* from backup_tonghop_dongia--- where thang = 202311
          create table backup_tonghop_dongia as           select* from backup_tonghop_dongia where thang = 202311
select* from backup_tonghop_dongia  where thang = 202401-- and manv = 'VNP016534'
insert into backup_tonghop_dongia 
with dm_kpi as (select * from ttkd_bsc.blkpi_danhmuc_kpi where loai='dongia' and thang_kt is null)-- and thuchien = 'Hoc')
    ...
    , nv as (select a.ma_nv, a.ten_nv, a.ma_vtcv ma_vtcv_old, a.ten_vtcv, a.ma_to, a.ten_to, a.ma_pb, a.ten_pb 
                        ...
                        from ttkd_bsc.nhanvien_202403 a) 
select 202403 thang
...
select ma_nv, ma_kpi, count(*) from backup_tonghop_dongia WHERE thang = 202401 group by ma_nv, ma_kpi; and ma_nv = 'CTV043447';
select * from ttkd_bsc.nhanvien_202308 where ma_vtcv in (select ma_vtcv from m_kpi_vtcv);
select* from backup_tonghop_dongia where thang = 202401 and ma_kpi = 'HCM_LUONG_QLDB_053'
commit;
select * from ttkd_bsc.bangluong_dongia_qldb WHERE thang = 202401 --and ma_nv in ('CTV074982') and ten_kpi like 'TC5%';
commit;
rollback;
select * from backup_tonghop_dongia
        -- 
        select giatri from backup_tonghop_dongia where thang  = 202311 and ma_kpi = 'HCM_LUONG_QLDB_056'
---Update gia HCM_LUONG_QLDB_054 = p1 * dongia * tytrong tien luong
;
update backup_tonghop_dongia a 
set giatri = 
    case when a.ma_vtcv not in ('VNP-HNHCM_BHKV_2','VNP-HNHCM_BHKV_2.2') then
            (select DIEM_P1 * DONGIA * nvl(giatri, 0)/100 from backup_tonghop_dongia where thang = a.thang and ma_kpi = 'HCM_LUONG_QLDB_053' and a.ma_nv = ma_nv)
        when  a.ma_vtcv in ('VNP-HNHCM_BHKV_2','VNP-HNHCM_BHKV_2.2') then
            round((select LUONG_DHANH_KHOANH from ttkd_Bsc.bldg_danhmuc_qldb_luong_dhanh_kh where a.ma_pb= ma_pb)*
            (select  nvl(giatri, 0)/100 from backup_tonghop_dongia where thang = a.thang and ma_kpi = 'HCM_LUONG_QLDB_053' and a.ma_nv = ma_nv),0)
    end
WHERE a.ma_kpi = 'HCM_LUONG_QLDB_054' and thang = 202403
;
                    rollback;
                    commit;
;  
...
WHERE a.ma_kpi = 'HCM_LUONG_QLDB_056' and thang = 202403
;
                     ----TO TRUONG
update backup_tonghop_dongia a 
set giatri = (select round(sum(heso_giao), 0) from ttkd_bsc.tl_giahan_tratruoc
                                    where MA_KPI in ('HCM_TB_GIAHA_022') and loai_tinh = 'KPI_TO' and thang = 202403 and a.ma_to = ma_to)
--select * from backup_tonghop_dongia a 
WHERE a.ma_kpi = 'HCM_LUONG_QLDB_056' and thang = 202403
...
select* from ttkd_bsc.bldg_danhmuc_qldb_pgd_phutrach;
                 ----PGD
update backup_tonghop_dongia a1
set giatri = ( select round(sum(a.heso_giao), 0) from ttkd_bsc.tl_giahan_tratruoc a
                            ...
                            where a.MA_KPI in ('HCM_TB_GIAHA_022') and a.loai_tinh = 'KPI_NV'
                                                and a.thang = 202403 and b.manv_pgd = a1.ma_nv
                     )
-- select * from backup_tonghop_dongia a1
WHERE a1.ma_kpi = 'HCM_LUONG_QLDB_056' and a1.thang = 202403
...
;                            
select* from TTKD_BSC.bldg_danhmuc_qldb_pgd_phutrach where thang = 202403-- where ma_kpi = 'HCM_LUONG_QLDB_056' 
...
set giatri = 
    case when a.ma_Vtcv not in ('VNP-HNHCM_BHKV_2','VNP-HNHCM_BHKV_2.2') then
    round((select giatri from backup_tonghop_dongia where thang = a.thang and ma_kpi = 'HCM_LUONG_QLDB_056' and a.ma_nv = ma_nv)
                            / (select giatri from backup_tonghop_dongia where thang = a.thang and ma_kpi = 'HCM_LUONG_QLDB_055' and a.ma_nv = ma_nv), 4)
     when a.ma_vtcv in ('VNP-HNHCM_BHKV_2','VNP-HNHCM_BHKV_2.2') then 1
    end
WHERE a.ma_kpi = 'HCM_LUONG_QLDB_057' and thang = 202403
;
...
WHERE a1.ma_kpi = 'HCM_LUONG_QLDB_058' and a1.thang = 202403
; 
select* from backup_tonghop_dongia;
                    ----TO TRUONG
update backup_tonghop_dongia a1
...
set a.giatri = (select giatri
            from backup_tonghop_dongia where thang = a.thang and ma_kpi = 'HCM_LUONG_QLDB_058' and a.ma_nv = ma_nv)
...
WHERE a.ma_kpi = 'HCM_LUONG_QLDB_059' and thang = 202403
;
commit;
...
WHERE a1.ma_kpi = 'HCM_LUONG_QLDB_060' and a1.thang = 202403
;
                    
---To truong                          
update backup_tonghop_dongia a1
set giatri = (SELECT sum(GIATRI)
                      FROM backup_tonghop_dongia a
...
WHERE a1.ma_kpi = 'HCM_LUONG_QLDB_062' and a1.thang = 202403
;
                commit;
select *
                                            from ttkd_bsc.ct_dongia_tratruoc  a
...
FROM nhuy.backup_tonghop_dongia
...
;
commit;
select* from hocnq_ttkd.backup_tonghop_dongia where thang = 202401;
------10        
...
join backup_tonghop_dongia b on a.thang = b.thang and a.manv = b.ma_nv
...
select ma_Nv, ma_vtcv from backup_tonghop_dongia where  ma_nv =  'VNP016534' and thang = 202311
select ma_Nv, ma_vtcv from ttkd_Bsc.nhanvien_202311 where  ma_nv =  'VNP016534' and thang = 202311
SELECT
commit;
select manv, HCM_LUONG_QLDB_053, HCM_LUONG_QLDB_054, HCM_LUONG_QLDB_055, HCM_LUONG_QLDB_056, HCM_LUONG_QLDB_057,
HCM_LUONG_QLDB_058, HCM_LUONG_QLDB_059, HCM_LUONG_QLDB_060, HCM_LUONG_QLDB_061, HCM_LUONG_QLDB_062 
from TEMP_QLDB where thang = 202401 and ma_vtcv in ('VNP-HNHCM_BHKV_2','VNP-HNHCM_BHKV_2.2');
select manv, HCM_LUONG_QLDB_053, HCM_LUONG_QLDB_054, HCM_LUONG_QLDB_055, HCM_LUONG_QLDB_056, HCM_LUONG_QLDB_057,
HCM_LUONG_QLDB_058, HCM_LUONG_QLDB_059, HCM_LUONG_QLDB_060, HCM_LUONG_QLDB_061, HCM_LUONG_QLDB_062 
from  ttkd_bsc.bangluong_dongia_qldb  where thang = 202401 and ma_vtcv in ('VNP-HNHCM_BHKV_2','VNP-HNHCM_BHKV_2.2');
...
update bangluong_dongia_qldb
 ...
 where thang=202401;
select* from ttkd_bsc.bangluong_dongia_qldb  where thang=202403;
select sum(tien) from ttkd_bsc.tl_giahan_tratruoc where thang = 202403 and ma_to = 'VNP0701402' and loai_Tinh like 'DONGIA%'
select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where loai='dongia' and thang_kt is null
commit;
update bangluong_dongia_qldb
