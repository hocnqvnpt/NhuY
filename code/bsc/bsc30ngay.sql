...
;
update ttkd_bsc.tl_giahan_tratruoc A set ma_to = (select ma_to,ma_vtcv, ten_pb from ttkd_bsc.nhanvien where thang = 202408 AND TEN_VTCV = 'T? Tr??ng T? Sau Bán Hàng' AND MA_PB = A.MA_PB)
...
select* from ttkd_Bsc.blkpi_dm_to_pgd where ma_to in (select distinct ma_to from ttkd_Bsc.nhanvien where thang = 202408 and ten_to = 'T? Sau Bán Hàng');
update ttkd_bsc.tl_giahan_tratruoc  a set ma_To = (select distinct ma_to from ttkd_Bsc.nhanvien where thang = 202408 and ten_to = 'T? Sau Bán Hàng' and ma_pb = a.ma_pb)
...
SELECT* FROM TTKD_bSC.NHANVIEN_202403 WHERE TEN_NV = 'Nguy?n Ng?c Nhiên'
