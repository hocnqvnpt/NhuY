...
create table thu_t1_V2 as 
WITH
--hddc as (select distinct hdtb_id,h.rkm_id b, g.rkm_id a, nvl(h.rkm_id, g.rkm_id) rkm_id, nvl(h.ngay_bddc, g.ngay_bddc) ngay_bddc, nvl(h.ngay_ktdc, g.ngay_ktdc) ngay_ktdc , g.thang_thoaitra,g.ngay_thoai
...
--                                                        , to_number(to_char(nvl(kmtb.ngay_ktdc, hddc.ngay_ktdc), 'yyyymmdd')) ngay_kt
                                         ...
                                         from css.v_phieutt_hd@dataguard a
                                                                join css.v_ct_phieutt@dataguard b on a.phieutt_id = b.phieutt_id and b.khoanmuctt_id in (11,5,52) and b.tien > 0
...
SELECT * FROM tien_Thu ;where ma_Tb= 'hcmbinhduong2024';
select* from THoai_T1; where ten_kmtt = 'Mua Thêm Thi?t B?' ;
select* from css_hcm.ct_phieutt where phieutt_id = 8046125; like 'Ti?n Tr? Tr??c%';
with t_thu as 
(select ma_Tb , sum(tien) tien from thoai  group by ma_Tb),
tt_thu as 
(select ma_Tb , sum(tien) tien_thoai from THoai_T1  group by ma_Tb)

select a.*, b.tien tien_Baocao from tt_thu a join t_thu b on a.ma_Tb = b.ma_tb where  abs(a.tien_thoai-b.tien)>1 ;and a.tien_thu*2!=b.tien  ; = 'vantai041023';
select* from thoai where ma_Tb not in (select ma_tb  from THoai_T1);
select* from THoai_T1 where ma_Tb not in (select ma_tb  from thoai);
update ttkd_Bsc.tl_Giahan_Tratruoc set loai_Tinh = 'DONGIATRA_OB_BO' where thang = 202405 and  loai_Tinh = 'DONGIATRA_OB';
COMMIT;
select* from thu;
...
;
select* from ttkd_bsc.nhanvien_202405 where ma_nv = 'VNP027795';
update ttkd_bsc.bangluong_kpi_202405 set HCM_SL_ORDER_001 = 452 where   ma_nv = 'VNP027795';
update ttkd_Bsc.nhanvien set ma_vtcv = 'VNP-HNHCM_BHKV_47' , ten_vtcv ='Nhân viên outbound Gia h?n tr? tr??c Phòng bán hàng khu v?c' where ma_nv = 'VNP027795' and thang = 202405;