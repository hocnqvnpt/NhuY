select* from ttkd_bsc.nhanvien where thang = 202408 and ten_nv like '%Thu?n';

select* from ttkd_Bsc.ct_dongia_tratruoc where thang = 202408 and ma_tb in ('netnam_dbp561a',
'netnam_280nkkn',
'netnam573',
'netnam_76lelai',
'netnam_trambom',
...
;insert into tmp3_60ngay(KHACHHANG_ID, THUEBAO_ID, MA_TB,  LOAITB_ID, RKM_ID, NGAY_BD_MOI, NGAY_KT_MOI, PHIEUTT_ID, TRANGTHAI, MA_GD, NGAY_HD, NGAY_TT, 
SOSERI, SERI,  TIEN_THANHTOAN, vat, HDTB_ID, HDKH_ID, NVGIAOPHIEU_ID, DVGIAOPHIEU_ID, NVTUVAN_ID, NVTHU_ID, THUNGAN_TT_ID, 
KENHTHU_ID, HT_TRA_ID,  KENHTHU, TEN_NGANHANG, TEN_HT_TRA, LAN)
WITH db as 
(
    select a.thuebao_id, a.loaitb_id, ma_tb
    from css_hcm.db_Thuebao a
--        join css.db_adsl  b on a.thuebao_id =b.thuebao_id
...
--        group by bb.phieu_id) 
    , kq_ghtt as (select 
                hdkh.khachhang_id, hdtb.thuebao_id, hdtb.ma_tb, hdtb.loaitb_id, nvl(kmtb.rkm_id, hddc.rkm_id) rkm_id
                ...
                , p.ma_gd, p.ngay_hd, p.ngay_tt, p.soseri, p.seri,  b.tien tien_thanhtoan,b.vat 
              ...
              , p.thungan_tt_id, p.kenhthu_id, p.ht_tra_id, 
               kt.kenhthu , nh.ten_nh ten_nganhang ,ht.ht_tra ten_ht_tra, 11 lan
                     from  css_hcm.hd_khachhang  hdkh
													...
													join ttkd_Bsc.ct_Bsc_tratruoc_moi tt  on hdtb.thuebao_id = tt.thuebao_id and tt.thang = 202409-- and gh.thang >= 202403
...
--													   left join ct on p.phieutt_id = ct.phieu_id
                     ...
                     hdtb.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224,136) and a.tien > 0 
                                      and ( to_number(to_char(p.ngay_tt, 'yyyymmdd')) between 20240701 and 20241001)  --    and p.ma_Gd = 'HCM-TT/02814791'            
)  
select *
from kq_Ghtt a where ngay_bd_moi is not null --and not exists (select 1 from tmp3_30ngay where rkm_id = a.rkm_id) 
 and not exists (select 1 from  ttkd_Bsc.ct_Bsc_tratruoc_moi where rkm_id is not  null and thuebao_id = a.thuebao_id and thang = 202409)
;
select* from ct_Bsc_ghtt_bhol;
select* from tmp3_60ngay where lan = 11;
commit;