...
with xgspon as (
select  'HCM' ma_tinh,  c.ma_tb,c.thuebao_id, cn.congnghe,vt.ten_Vt thietbi, tb.trangthaitb_id,  kh.loaihd_id,lhd.ten_loaihd,c.kieuld_id,kld.ten_kieuld , c.ngay_ht, 
...
create table cte as
with cte as (
    select thuebao_id,ma_tb,hdtb_id, row_number() over (partition by   ma_Tb order by  hdtb_id desc) rn , dv_rp, ten_Nv, ma_Nv
                from cdcn_td where  
...
--- 5
 with goi as
(select thuebao_id, nhomtb_id ,goi_id,thang_bd, row_number() over (partition by thuebao_id order by nhomtb_id desc) rn
...
, goi_cu as (select nhomtb_id,goi_id, thuebao_id, row_number() over (partition by thuebao_id order by nhomtb_id desc) rnk, kyhoadon
								from tinhcuoc_hcm.sd_goi_dadv
								where trangthai = 0
...
select a.*
       , case when hd.loaihd_id = 1 then nvl(dv_thuyetphuc ,dv_rp)
         ...
         , hd.ten_Nv, hd.ma_Nv, c.muccuoc muccuoc_cu, c.cuoc_tg cuoc_Cu,  e.muccuoc muccuoc_moi, e.cuoc_tg cuoc_moi,
         gm.ten_goi goi_moi, gc.ten_goi goi_cu
from xgspon a
    join hdong hd on a.ma_tb = hd.ma_Tb and hd.rn = 1
    left join css_hcm.bd_Thuebao b on hd.hdtb_id = b.hdtb_id
    ...
    left join goi_cu g on a.thuebao_id = g.thuebao_id and g.kyhoadon = TO_CHAR(ADD_MONTHS(TO_DATE(goi.thang_bd, 'YYYYMM'), -1), 'YYYYMMDD')
    left join css_Hcm.goi_dadv gm on goi.goi_id = gm.goi_id
    left join css_hcm.goi_dadv gc on g.goi_id = gc.goi_id;
where gc.ten_goi is not null;
