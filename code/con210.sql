...
, kq_ghtt as (select hdtb.thuebao_id, hdtb.ma_tb, hdtb.kieuld_id, nvl(kmtb.rkm_id, hddc.rkm_id) rkm_id
 ...
 where a.kenhthu_id not in (6) and a.trangthai = 1 and hdtb.ma_tb = 'hcmphuoclong090220'                                                                
                and to_number(to_char(nvl(a.ngay_tt, hdtb.ngay_ht), 'yyyymm')) between 202312 and 202312
)
             select * from kq_ghtt;-- where ma_tb = 'phuoclong090220';
  select* from css_hcm.hd_thuebao where hdtb_id = 23306230