	...
	select DISTINCT * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi ='HCM_TB_GIAHA_027' and thang_kt is null;
    SELECT * FROM TTKD_BSC.BANGLUONG_KPI_202405 WHERE HCM_TB_GIAHA_027 IS  NULL AND MA_vTCV IN 
    (	select DISTINCT MA_VTCV from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi ='HCM_TB_GIAHA_027' and thang_kt is null);
    SELECT* FROM TTKD_BSC.TL_GIAHAN_tRATRUOC WHERE THANG = 202405 AND MA_NV IN (SELECT MA_nV FROM TTKD_BSC.BANGLUONG_KPI_202405 WHERE HCM_TB_GIAHA_027 IS  NULL
     AND MA_vTCV IN 
    (	select DISTINCT MA_VTCV from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi ='HCM_TB_GIAHA_027' and thang_kt is null))  AND  ma_kpi ='HCM_TB_GIAHA_027';
;
select * from css_Hcm.loaihinh_Tb where loaitb_id = 11;
SELECT* FROM TTKD_BSC.TL_GIAHAN_tRATRUOC WHERE THANG = 202405 AND   AND  ma_kpi ='HCM_TB_GIAHA_026';
UPDATE TTKD_bSC.BANGLUONG_KPI_202405 SET HCM_TB_GIAHA_026 = 50 WHERE MA_NV = ''
select a.ma_vtcv, a.ten_vtcv, a.ten_pb, a.ma_pb, nvl(b.ma_vtcv, 'khongco') VTCV
    from ttkd_bsc.nhanvien a
					left join ttkd_bsc.bangluong_kpi_202405 b on a.ma_nv = b.ma_nv and a.thang = 202405 
    where thang = 202405 and a.donvi = 'TTKD' and b.ma_vtcv is null
					and ma_pb not in ('VNP0700900', 'VNP0700400', 'VNP0700500', 'VNP0700600', 'VNP0700100', 'VNP0700200', 'VNP0700700', 'VNP0700300');
--- Ngay quet:
...
---GHTC: doi voi congvan_id = 190 (tra truoc tru dan) xet tien AVG thang moi >= AVG thang cu
	select* from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_Vtcv ='VNP-HNHCM_KDOL_17' and thang_kt is null;
select distinct ma_kpi from ttkd_Bsc.tl_Giahan_Tratruoc where thang = 202405;
...
select* from ttkd_Bsc.nhanvien where sdt = '0915470606';
...
, gh as (select khachhang_id, thuebao_id, duan_id, ma_tb, ma_tt, loaitb_id, thang_kt , row_number() over (partition by thuebao_id order by thuebao_id desc) rnk
                from ttkdhcm_ktnv.ghtt_giao_688 where tratruoc = 1 and km = 1 and loaibo = 0 and thang_kt = 202406)             ---change n-1
...
--                                            left join ct2 on a.phieutt_id = ct2.phieu_id
                         where a.kenhthu_id not in (6) and a.trangthai = 1
                                        and to_number(to_char(a.ngay_tt, 'yyyymmdd')) between 20240501 and 20240602                  ----change--1 thang- ngay 2
...
where  a.ngay_bd_moi is NOT null 
                  ...
                  and not exists (select rkm_id from tmp3_tr30ngay where rkm_id = a.rkm_id) 
            ;
            select rkm_id from tmp3_ts group by rkm_id having count(rkm_id) > 1
...
-- kiem tra trung 
SELECT * FROM ttkd_bsc.ct_bsc_tratruoc_moi_tr30day; where thang = 202404 and tien_khop is  null and ht_Tra_id is not null;
select * from tmp3_tr30ngay; group by rkm_id having count(rkm_id) > 1
update nhuy.ct_bsc_tratruoc_moi set tien_khop = null where thuebao_id in (4568396,9710800,9350934,9434808,9195836);
---Buoc 2-----tao ds chot
insert into  ttkd_bsc.ct_bsc_tratruoc_moi_tr30day
--  insert into nhuy.ct_bsc_tratruoc_moi
        (THANG, GH_ID, PBH_QL_ID, PBH_GIAO_ID, TBH_GIAO_ID, PBH_TH_ID, PBH_CN_ID, MA_TB, MANV_ql, PHONG_ql, MA_TO, MA_PB, MANV_GIAO, PHONG_GIAO
        , MANV_TH, PHONG_TH, MA_NV_CN, PHONG_CN, manv_thuyetphuc, mapb_thphuc,  MANV_THUNGAN, KHDN, HESO_GIAO, THANG_KTDC_CU, TIEN_DC_CU
...
with t0 as (select c0.thang_kt, c0.thuebao_id, c0.phieutt_id, c0.ma_tt, c0.ma_gd, c0.rkm_id, c0.ngay_bd_moi, c0.tien_thanhtoan, c0.vat
                                                            ...
                                                            --    , round(cuoc_dc/so_thangdc, 0) avg_thang, so_thangdc
                                                                ...
                                                                , c0.nvtuvan_id, c0.thungan_tt_id, c0.kenhthu_id, c0.ht_tra_id, nv.ma_nv manv_cn, dv.ten_dv phong_cn, nv1.ma_nv manv_thuyetphuc
--                                                                , nv2.ma_nv manv_gt,
                                                             ,   nv3.ma_nv manv_thungan
                                               from tmp3_tr30ngay c0
...
, c as (select t0.thang_kt, t0.thuebao_id, t0.phieutt_id, t0.ma_tt, t0.ma_gd, t0.rkm_id, t0.ngay_bd_moi, t0.tien_thanhtoan, t0.vat, t0.ngay_tt, t0.ngay_hd, t0.ngay_nganhang, t0.soseri, t0.seri
                            , t0.kenhthu, t0.ten_nganhang, t0.ten_ht_tra, t0.hdkh_id, t0.hdtb_id, t0.nvgiaophieu_id, t0.dvgiaophieu_id, t0.nvtuvan_id,
--                            t0.nhanviengt_id, 
                            t0.thungan_tt_id                            , t0.kenhthu_id, t0.ht_tra_id, t0.manv_cn, t0.phong_cn, t0.manv_thuyetphuc,
--                            t0.manv_gt,
                            t0.manv_thungan, km0.so_thangdc, km0.avg_thang
                    from t0
...
select 202405 thang, a.gh_id, a.pbh_ql_id, a.donvi_giao pbh_giao_id, a.tbh_giao_id
...
from ds a
                 --           left join ttkd_bct.db_thuebao_ttkd db on a.thuebao_id = db.thuebao_id and db.trangthaitb_id is not null     ---n-1---
                           ...
                           left join ttkd_bsc.nhanvien_202405 nvtp on c.MANV_THUYETPHUC = nvtp.ma_nv
                           left join goi on a.thuebao_id = goi.thuebao_id and goi.rn = 1
                                    -- join km on c.rkm_id = km.rkm_id 
          
where  a.thang_kt = 202406 --and rkm_id != 3369696 --a.ma_tb  in ('')
         ...
         */     --   and a.thangdc < 3 and c.so_thangdc>= 3 
          ...
          --    and a.ma_tb like 'nutifood%'
               and exists (select * from tmp3_tr30ngay where lan = 22 and thuebao_id = a.thuebao_id )
             --   and rownum < 3
--select * from css_hcm.toanha where duan_id in (345, 1382)
; 
select rkm_id from tmp3_60ngay
rollback;
update ttkd_Bsc.ct_bsc_tratruoc_moi_tr30day set tien_khop = 1 where thang  = 202405 and thuebao_id in (select thuebao_id from tmp3_tr30ngay where lan = 22) and
ma_gd in (select ma_Gd from chungtu where ketqua in ( 'OK' ,'THANH TOAN QUA QR CODE'));
...
commit;
select rkm_id from ttkd_bsc.ct_bsc_tratruoc_moi_30day where thang = 202404 group by rkm_id having count(rkm_id) > 1
