...
select* from admin_hcm.nhanvien_ONEBSS where ma_Nv = 'HCM021261';
...
;
select * from tinhcuoc_hcm.dbkh_20220401;
select * from css_hcm.db_thuebao where ma_tb = 'nquanghoc_2015';--thuebao_id = '8139916';
select * from css_hcm.db_khachhang where khachhang_id = 4747021
...
insert into ds_giahan_tratruoc1
with ks as (
                select THUEBAO_ID, knv.NHANVIEN_ID, kvtb.khuvuc_id, ma_kv, nv.donvi_id tovt, dv.donvi_cha_id ttvt, dv.ten_dv TEN_TO--, dv.ten_dvql ten_TTVT
...
select --count(*) a
  ...
  --   , ts.sl_datcoc    , pkg.nhomtb_id, pkg.goi_id
       ...
       , case when lkh.khdn = 0 and a.loaitb_id in (58, 59) then 1 else 0 end fiber_donle
                ---Check du an
            -- , nv l((select distinct 1 from dattdt.duan_thuebao_mapping where thuebao_id = a.thuebao_id), 0) duan   ---bo ngay 25/7/22 dk duan_id=345 and 
...
from css_hcm.db_thuebao a
            ...
            left join  ks on a.thuebao_id = ks.thuebao_id                                             -----change
    ...
    where a.trangthaitb_id not in (7, 8, 9) and to_number(to_char(dc.ngay_kt_mg, 'yyyymm')) = 202402 --and a.thuebao_id  in( 9049183)
...
;
insert into ds_giahan_tratruoc
with kv as (
...
from css_hcm.db_thuebao a --on a.thuebao_id = dc.thuebao_id                   --thang n
            join css_hcm.db_adsl a1 on a.thuebao_id = a1.thuebao_id and a1.chuquan_id = 145
            join ttkd_bct.db_thuebao_ttkd a2 on a.thuebao_id = a2.thuebao_id and a2.pbh_ql_id = 8
    --part 1----        
  ...
  ;
  select* from ds_giahan_tratruoc where thang = 202408
  commit;
  delete from ds_giahan_tratruoc
  commit;
  ---update fiber_donle theo MATB_TN, MADOICAP cua MYTV
...
select  a.thang, a.ma_tb, b.loaihinh_tb,c.trangthai_tb, a.ngay_bddc, a.ngay_ktdc , a.ngay_kt_mg, a.ngay_Huy, a.ngay_thoai, a.tien_td, a.cuoc_Dc, a.so_Thangdc, a.so_thangkm,
    d.ten_cv ten_congvan, f.ten_ctkm chitiet_khuyenmai, a.sl_datcoc solan_datcoc, e.ma_goi,e.ten_goi, g.ten_nv nv_ttvt, x.ten_dv to_vt, y.ten_dv ttvt, a.khdn,
    z.ten_nv nhanvien_cs, aa.tento to_cs, bb.ten_pb phong_cs, a.heso heso_Giao
from  ds_giahan_tratruoc a
    left join css_hcm.loaihinh_Tb b on a.loaitb_id = b.loaitb_id
    left join css_hcm.trangthai_tb c on a.trangthaitb_id = c.trangthaitb_id
    left join css_hcm.socongvan d on a.congvan_id = d.congvan_id 
    left join css_hcm.ct_khuyenmai f on a.chitietkm_id = f.chitietkm_id
    left join css_hcm.goi_dadv e on a.goi_id = e.goi_id
    left join admin_hcm.nhanvien_onebss g on a.nvttvt_id = g.nhanvien_id
    left join admin_hcm.donvi x on a.tovt_id = x.donvi_id
    left join admin_hcm.donvi y on a.ttvt_id = y.donvi_id
    left join ttkd_bsc.nhanvien_202312 z on a.ma_nv = z.ma_nv 
    left join ttkd_bct.tobanhang aa on a.tbh_ql_id = aa.tbh_id and a.pbh_ql_id = aa.pbh_id and aa.hieuluc = 1
    left join ttkd_bsc.dm_phongban bb on a.pbh_ql_id = bb.pbh_id and bb.active = 1
where a.tratruoc = 1
select* from   ttkd_bsc.dm_phongban
select* from ds_giahan_tratruoc-- css_hcm.ct_khuyenmai
    
ttkdhcm_ktnv.ghtt_giao_688
select table_name
from all_Tables
where owner = 'TTKD_BCT'
select* from ttkd_bsc.ct_Bsc_Tratruoc_moi where phong_cs like '%Gia%'  and phong_cs != phong_giao
