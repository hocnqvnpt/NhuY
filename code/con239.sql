 select 202404 thang,
                         ...
                         36 tong,;
                      select   round(sum(case when dthu > 0 and tien_khop > 0 then heso_chuky*heso_dichvu else 0 end)) da_giahan
...
--             )
             select     
             a.thang, a.thuebao_id, a.loaitb_id,  a.thanhtoan_id , a.ma_to, a.ma_pb
...
--                                            left join  km on a.rkm_id = km.rkm_id
                        where a.rkm_id is not null and  manv_thuyetphuc = 'CTV021886' and to_number(to_char(ngay_Tt,'yyyymm')) = 202404-- and to_number(to_char(ngay_tt,'yyyymm'))  = 202403---        CHANGE
                        group by a.thang, a.thuebao_id, a.loaitb_id,  a.thanhtoan_id ,  a.ma_to, a.ma_pb
...
--            join ttkd_bsc.nhanvien_202404 b on a.manv_thuyetphuc = b.ma_nv
        where rnk = 1 and dthu > 0 and manv_thuyetphuc = 'CTV021886'
        group by a.thang, a.manv_thuyetphuc, a.ma_to, a.ma_pb;
        
        select* from ttkd_Bsc.nhanvien_202404 where ten_Nv like '%Trang';