...
--                                            left join ct on a.phieutt_id = ct.phieu_id
                         ...
                         where a.kenhthu_id not in (6) and a.trangthai = 1 and hdtb.ma_Tb in ('ha10032018','dinhngocho','ctynamson18_05','ntpl944','tdlong18')
                                         and ( ( to_number(to_char(a.ngay_tt, 'yyyymmdd')) between 20240801 and 20250221)  --    and p.ma_Gd = 'HCM-TT/02814791'            
                                      or ( to_number(to_char(hdtb.ngay_ht, 'yyyymmdd')) between 20240801 and 20250221) and a.ngay_tt is null ) --    and p.ma_Gd = 'HCM-TT/02814791'                    ----change--3 thang- ngay 2
...
;
delete from  ttkd_bsc.ct_bsc_tratruoc_moi_30day where thang = 202501;
...
-- insert into nhuy.ct_bsc_tratruoc_moi_30day-create table t4 as
    ...
    with t0 as (select 202501 thang_kt, c0.thuebao_id, c0.phieutt_id, 'matt' ma_tt, c0.ma_gd, c0.rkm_id, c0.ngay_bd_moi, c0.tien_thanhtoan, c0.vat
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
            ...
            , ds as (select ten_pbh, thuebao_id,202501 thang_kt, ngay_bh, luonggiao_ttvt,ngay_ktdc
                    from REPORT.BC_GHTT_BIEU1_CHOT@dataguard where phanvung_id = 28 and to_char(ngay_ktdc,'yyyymm') = '202501' and (ngay_Bh is  null  or to_number(to_char(ngay_bh,'yyyymmdd'))> 20250207) 
                    )
    ...
    from ds a
                                ...
                                left join ttkd_bsc.nhanvien nvtp on c.MANV_THUYETPHUC = nvtp.ma_nv and nvtp.thang = 202501 -- change
                                left join goi on a.thuebao_id = goi.thuebao_id and rn = 1
                        
    where  a.thang_kt = 202501 --and rkm_id != 3369696 --a.ma_tb  in ('') -- change
           --      and a.gh_id is null
                ...
                and not exists  (select 1 from ttkd_Bsc.ct_Bsc_Tratruoc_moi_30day a where thang = 202501 and rkm_id = c.rkm_id);
;
select count(1) from ds_giahan_Tratruoc_t9;
select* from ttkd_Bsc.blkpi_dm_to_pgd where thang = 202409;
select* from admin_hcm.donvi where donvi_cha_id =2941; like '%nlin%';
update ttkd_Bsc.ct_Bsc_Tratruoc_moi_30day set ma_pb = 'VNP0703000' WHERE THANG = 202501;
;
COMMIT;
delete from ttkd_Bsc.ct_Bsc_Tratruoc_moi_30day x
where thang = 202501 and rkm_id is null 
    and exists (select max(a.hdkh_id) 
        from css_hcm.hd_khachhang a 
         join css_hcm.hd_thuebao b on a.hdkh_id = b.hdkh_id
         join admin_hcm.donvi c on a.donvi_id = c.donvi_id
         join css_hcm.ct_tienhd d on b.hdtb_id = d.hdtb_id and d.khoanmuctt_id = 11 and d.tien>0
         left join css_hcm.phieutt_hd e on a.hdkh_id = e.hdkh_id
        where c.donvi_cha_id  != 2941 and b.thuebao_id = x.thuebao_id and tthd_id not in (6, 7 )
        group by b.thuebao_id);
        commit;
        and  a.khachhang_id  = 9798610  ;
update  ttkd_Bsc.ct_Bsc_Tratruoc_moi_30day a 
set ma_pb = 'VNP0703000'
where thang = 202501 and ma_pb is null;
commit;
select* from ttkdhcm_ktnv.ds_Chungtu_Nganhang_oneb where ma_ct='VCB_20250102_438486';
...
delete from ;
UPDATE TTKD_bSC.ct_Bsc_Tratruoc_moi_30day set tien_khop = 1 WHERE THANG = 202501 AND tien_khop = 0 and ma_Gd in (
select ma from ttkdhcm_ktnv.ds_chungtu_Nganhang_phieutt_Hd_1);
rollback;
delete from ttkd_Bsc.ct_Bsc_Tratruoc_moi_30day where thang = 202501; and ma_pb is null;

-- update ma_pb
-- bhkv
update ttkd_Bsc.ct_Bsc_Tratruoc_moi_30day a 
set ma_pb = (select distinct ma_pb from ttkd_Bsc.nhanvien where thang = 202501 and ten_pb = phong_cs)
where thang = 202501 and ma_pb is null;
-- khdn
update ttkd_Bsc.ct_Bsc_Tratruoc_moi_30day a 
...
where  ht_tra_id in (2,4,5,207) and tien_khop = 0  and thang = 202501 --and rkm_id in (select rkm_id from tmp3_30ngay where lan = 308)
and exists 
    (
...
where thang = 202501 and tien_khop = 0 and ht_Tra_id in (207) and kenhthu_id in (25,26);
select* from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang = 202501 and tien_khop = 0;
select* from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang = 202501 and tien_khop is not null and rkm_id is   null;
select* from ttkd_Bsc.bangluong_kpi where thang = 202501 and ma_kpi = 'HCM_SL_COMBO_006';
select* from ttkd_bsc.blkpi_Dm_to_pgd where thang >=202501;
...
update ttkd_bsc.ct_Bsc_tratruoc_moi_30day a set tien_khop = 1
--select * from ttkd_bsc.ct_Bsc_tratruoc_moi_30day a 
where thang = 202501 and tien_khop = 0; rkm_id is null and 
...
where thang = 202501 and rkm_id is not null  
and not exists (select 1 from ds_Giahan_Tratruoc2_v2 where a.thuebao_id = thuebao_id and to_number(to_char(ngay_kt_mg,'yyyymm'))>202501);
update ttkd_bsc.ct_Bsc_tratruoc_moi_30day a
set tien_khop = 1 where thang = 202501 and tien_khop = 0 and ma_gd in (select ma from ttkdhcm_ktnv.ds_Chungtu_Nganhang_phieutt_Hd_1);
commit;
select distinct tien_khop from  ttkd_bsc.ct_Bsc_tratruoc_moi_30day  where thang = 202501;
select distinct ma_kpi, loai_tinh from  ttkd_Bsc.tl_giahan_Tratruoc where thang = 202501;
delete from ttkd_Bsc.tl_giahan_Tratruoc where thang = 202501 and ma_Kpi = 'HCM_TB_GIAHA_022' and ma_pb = 'VNP0703000';
...
where thang = 202501 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where-- giamdoc_phogiamdoc = 1 
 ma_kpi in ('HCM_TB_GIAHA_022') and thang = 202501 and MA_VTCV = a.MA_VTCV)
...
set mucdo_hoanthanh = case when tyle_Thuchien >= 85 then 120
                        when tyle_thuchien >= 55 and tyle_thuchien < 85 then 30 + 2.8*(tyle_thuchien - 55)
                        when tyle_thuchien < 55 then 0 end
where ma_kpi ='HCM_TB_GIAHA_022' AND THANG = 202501 and ma_pb = 'VNP0703000';
commit;
--- bhol 
update ttkd_Bsc.bangluong_kpi  
...
where ma_kpi ='HCM_TB_GIAHA_022' AND THANG = 202501 and ma_pb = 'VNP0703000';
select* from ttkd_Bsc.bangluong_kpi where thang = 202501 and ma_kpi ='HCM_TB_GIAHA_022';
commit;
...
--                                            left join ct on a.phieutt_id = ct.phieu_id
                         ...
                         where a.kenhthu_id not in (6) and a.trangthai = 1
                                        and to_number(to_char(hdtb.ngay_ht, 'yyyymmdd')) between 20240901 and 20250101   
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
            ...
            , ds as (select ten_pbh, thuebao_id, thang_kt
                    from giao_oneb where thang_kt =202501 and thang = 202501
                    )
            , c as (select t0.thang_kt, t0.thuebao_id, t0.phieutt_id, t0.ma_tt, t0.ma_gd, t0.rkm_id, t0.ngay_bd_moi, t0.tien_thanhtoan, t0.vat, t0.ngay_tt, t0.ngay_hd, null ngay_nganhang, t0.soseri, t0.seri
                                , t0.kenhthu, t0.ten_nganhang, t0.ten_ht_tra, t0.hdkh_id, t0.hdtb_id, t0.nvgiaophieu_id, t0.dvgiaophieu_id, t0.nvtuvan_id, t0.nvthu_id, t0.thungan_tt_id
                                , t0.kenhthu_id, t0.ht_tra_id, t0.manv_cn, t0.phong_cn, t0.manv_thuyetphuc, t0.manv_gt, t0.manv_thungan, km0.so_thangdc, km0.avg_thang
    ...
    from ds a
                                ...
                                left join ttkd_bsc.nhanvien nvtp 
                                    on c.MANV_THUYETPHUC = nvtp.ma_nv and nvtp.thang = 202501 -- change
                                left join goi on a.thuebao_id = goi.thuebao_id and rn = 1
                        
    where  a.thang_kt = 202501 and a.thuebao_id in (select thuebao_id from tmp3_60ngay_Bs)
     and not exists (select 1 from ttkd_Bsc.ct_Bsc_tratruoc_moi where thang = 202501 and tien_khop =1 and thuebao_id = a.thuebao_id);   and dbtb.ma_Tb='co29042019' --and rkm_id != 3369696 --a.ma_tb  in ('') -- change
           --      and a.gh_id is null
                            and dbkh.khachhang_id  <> 9798610          ---khong tinh giao 30 ngay, chi tinh 60 ngay theo vb 3116/TTr-NVC-KHDN2 (folder loai tru ghtt) cho het nam 2023
           ;       --   and a.thangdc < 3 and c.so_thangdc>= 3 
                 --  and a.ma_tb in ('hcmnhp183')
...
;
commit;
select* from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang= 202501 ;and thuebao_id in (
select thuebao_id from ttkd_Bsc.ct_Bsc_tratruoc_moi_30day where thang= 202501 group by thuebao_id having count(distinct nvl(tien_khop,0) )>1);
and rkm_id is null;
