...
where thang = 202406 and donvi = 'VTTP'
;
rollback;
commit;
...
/* -------------- UP USER_IPCC ----------------- */
update TTKD_BSC.nhanvien set user_ipcc='' where thang = 202406 and donvi = 'VTTP';
update  TTKD_BSC.nhanvien  set user_ipcc=REPLACE(MAIL_VNPT,'@vnpt.vn','') where user_ipcc is null AND  thang = 202406 and donvi = 'VTTP';
commit;
SELECT MA_NV FROM TTKD_bSC.NHANVIEN WHERE THANG = 202405 GROUP BY MA_nV HAVING COUNT(MA_nV) > 1;
SELECT* FROM  TTKD_bSC.NHANVIEN WHERE MA_NV IN ('HCM013863','HCM014912');
...
update  TTKD_BSC.nhanvien a set (a.nhanvien_id,a.user_dhsxkd)=(select nhanvien_id, user_dhsxkd 
                                                            from ttkd_bsc.nhanvien where ma_nv=a.ma_nv AND THANG = 202405 AND donvi = 'VTTP'
) 
WHERE thang = 202406 and donvi = 'VTTP';
commit ;
...
update nhanvien_202407_lan1 a set a.nhanvien_id=(select nhanvien_id from admin_hcm.nhanvien where ma_nv=a.manv_hrm) 
...
update nhanvien_202407_lan1 a set a.user_dhsxkd=(select distinct cast(ma_nd as varchar(100)) from admin_hcm.nguoidung
...
-- update user ccos
update dm_ccos set thang = 202406 where thang is null;
/* ------------------ UP USER CCOS TU ANH KHANG CUNG CAP ----------------- */
update nhanvien_202407_lan1 A SET (a.user_ccos,trangthai_ccos)=(select user_ccos, trangthai from dm_ccos where manv_hrm=a.ma_nv and thang='202406' ) 
...
commit ;
/* ------------ UP USER_CCBS ------------- */
update nhanvien_202407_lan1 a SET (a.USER_CCBS,a.tr_thai)=(SELECT USER_CCBS,tr_thai FROM ttkd_bsc.nhanvien_202406 WHERE MANV_HRM=A.MANV_HRM) where a.user_ccbs is null ;
COMMIT ;

/* ------------------ UP BO SUNG USER_CCBS TU FILE NHAN VIEN TU ONE ----------------- */
update nhanvien_202407_lan1 a set user_ccbs=(select nd.user_neo from admin.v_nguoidung@dataguard nd where a.nhanvien_id=nd.nhanvien_id and nd.user_neo is not null)
...
update nhanvien_202407_lan1 A SET (A.USER_CCBS,a.tr_thai,a.user_ccos,a.trangthai_ccos)=(SELECT USER_CCBS,tr_thai,user_ccos,trangthai_ccos 
                                                                                        FROM ttkd_bsc.nhanvien_202406 WHERE MANV_HRM=A.MANV_HRM) ;
COMMIT ;

update nhanvien_202407_lan1 a set a.tr_thai=(select tr_thai from userld_202406_goc b where a.user_ccbs=b.user_ld) ;
commit;
select* from userld_202403_goc;
select manv_hrm, mail_vnpt from nhanvien_202406_lan2 where mail_vnpt not like '%@vnpt.vn' ;

/* KIEM TRA TRUNG MANV */
select manv_hrm,count(*)sl from nhanvien_202407_lan1 group by manv_hrm having count(*) > 1 ;
select ma_pb,ten_pb from nhanvien_202407_lan1 group by ma_pb,ten_pb order by ma_pb,ten_pb  ; -- ;
select ma_to,ten_to,ten_pb from nhanvien_202406 group by ma_to,ten_to,ten_pb ; -- ;

select ma_vtcv,ten_vtcv from nhanvien_202406_lan2 a group by ma_vtcv,ten_vtcv order by ma_vtcv,ten_vtcv ; -- ;
...
rename nhanvien_202407_lan1 to nhanvien_202407 ;
...
;
insert into nhanvien(THANG, DONVI, MANV_NNL, MA_NV, TEN_NV, MA_VTCV, TEN_VTCV, MA_TO, TEN_TO, MA_PB, TEN_PB, LOAI_LD, USER_CCBS, TR_THAI, USER_CCBS2, MAIL_VNPT, SDT,
NGAYSINH, NHANVIEN_ID, GIOITINH, CMND, USER_DHSXKD, MANV_HRM, USER_CCOS, TRANGTHAI_CCOS, USER_IPCC, PHAN_LOAI, LOAI_LAODONG, NHOMLD_ID)
