...
, gh as (select khachhang_id, thuebao_id, duan_id, ma_tb, ma_tt, loaitb_id, thang_kt 
    from ttkdhcm_ktnv.ghtt_giao_688 where tratruoc = 1 and km = 1 and loaibo = 0  and thang_kt = 202403 ) -- change
...
, kq_ghtt as (select  gh.khachhang_id, gh.thuebao_id, gh.duan_id, gh.ma_tb, gh.ma_tt, gh.loaitb_id, gh.thang_kt, 05 lan
        --, dbdc.thang_bd b, hdttdc.thang_bd c, hdttdc.*
                     ...
                     where a.kenhthu_id not in (6) and a.trangthai = 1                                                                        
                                    and ((a.ht_tra_id <> 2 and to_number(to_char(a.ngay_tt, 'yyyymmdd')) between 20240201 and 20240402)                     ----change--2 thang- ngay 02
                                                        or (a.ht_tra_id in (2,7, 4) and to_number(to_char(ct.ngay_nganhang, 'yyyymmdd')) between 20240201 and 20240402))                     ----change--2 thang- ngay 02
...
where ngay_bd_moi is not null --and thuebao_id in (9866366,9866341)
                        and not exists (select rkm_id from ttkd_bsc.ct_bsc_tratruoc_moi_30day where rkm_id = a.rkm_id and thang >=202402)
                        and not exists (select rkm_id from ttkd_bsc.ct_bsc_tratruoc_moi where  rkm_id = a.rkm_id and thang >=202402)
...
select * 
delete 
from nhuy.ct_bsc_tratruoc_moi_30day where thang = 202312; and ma_tb = 'vitnam6165925';
...
---truncate table hocnq_ttkd.ct_bsc_tratruoc_moi_30day;
        --select * from hocnq_ttkd.ct_bsc_tratruoc_moi_30day;
insert into ttkd_bsc.ct_bsc_tratruoc_moi_30day
-- insert into nhuy.ct_bsc_tratruoc_moi_30day
...
--                    ) 
    ...
    from ds a
                                ...
                                left join ttkd_bsc.nhanvien_202402 nvtp on c.MANV_THUYETPHUC = nvtp.ma_nv -- change
                        
    where  a.thang_kt = 202403--and rkm_id != 3369696 --a.ma_tb  in ('') -- change
           --      and a.gh_id is null
--                            and a.khachhang_id  <> 9798610          ---khong tinh giao 30 ngay, chi tinh 60 ngay theo vb 3116/TTr-NVC-KHDN2 (folder loai tru ghtt) cho het nam 2023
          ...
          --     and exists (select 1 from x_tmp_30day where thuebao_id = a.thuebao_id)
...
select* from ct_Bsc_tratruoc_moi_30day where tien_Thanhtoan < 20000 and thang =  202401;
--
update 
select* from ttkd_bsc.ct_bsc_tratruoc_moi_30day where thang = 202401
-- DO VAO BANG CHINH
insert into ttkd_Bsc.ct_Bsc_tratruoc_moi_30day
select* from ct_Bsc_tratruoc_moi_30day where thang = 202401
--
set nhomtb_id = 2854435
where thang = 202312 and thuebao_id = 8067986
commit;
select rkm_id from ct_Bsc_tratruoc_moi_30day where thang = 202401 group by rkm_id having count(rkm_id) > 1
            ...
            insert into ct_bsc_tratruoc_moi_trudan;
                        ...
                        where thang = 202308 and congvan_id = 190 and nhom_datcoc_id in (1) and TIEN_DC_CU <> TIEN_THANHTOAN + VAT
                            ...
                            ;
                                    ...
                                    order by 6
            ;
             select * from  ttkd_bsc.ct_bsc_tratruoc_moi_30day a  where thang = 202310 and TIEN_THANHTOAN < 100000;
            ;
