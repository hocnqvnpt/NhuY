...
select distinct HT_TRA_ID from hocnq_ttkd.tmp3_30ngay
select distinct HT_TRA_ID from tmp3_ts -- where ngay_Nganhang is not null
...
select* from tmp3_60ngay--css_hcm.hinhthuc_tra where ht_tra_id in 
...
, gh as (select khachhang_id, thuebao_id, duan_id, ma_tb, ma_tt, loaitb_id, thang_kt 
                from ttkdhcm_ktnv.ghtt_giao_688 where tratruoc = 1 and km = 1 and loaibo = 0  and thang_kt = 202401)             ---change n-1
...
, kq_ghtt as (select  gh.khachhang_id, gh.thuebao_id, gh.duan_id, gh.ma_tb, gh.ma_tt, gh.loaitb_id, gh.thang_kt, 5 as lan
                        --, dbdc.thang_bd b, hdttdc.thang_bd c, hdttdc.*
                         ...
                         where a.kenhthu_id not in (6) and a.trangthai = 1
                                        and ((a.ht_tra_id <> 2 and to_number(to_char(a.ngay_tt, 'yyyymmdd')) between 20231201 and 20240302)                     ----change--3 thang- ngay 2
                                                    or (a.ht_tra_id in (2, 4, 7) and to_number(to_char(ct.ngay_nganhang, 'yyyymmdd')) between 20231201 and 20240302))                     ----change--3 thang- ngay 2
...
--                    and not exists (select 1 from ttkd_bsc.ct_bsc_tratruoc_moi where rkm_id = a.rkm_id and thang >=202312)
...
-- kiem tra trung 
SELECT * FROM nhuy.ct_bsc_tratruoc_moi_30day WHERE thang = 202401 and tien_khop is  null and ht_Tra_id is not null
select rkm_id from tmp3_ts group by rkm_id having count(rkm_id) > 1
...
select 202402 thang, a.gh_id, a.pbh_ql_id, a.donvi_giao pbh_giao_id, a.tbh_giao_id
...
from ds a
                 --           left join ttkd_bct.db_thuebao_ttkd db on a.thuebao_id = db.thuebao_id and db.trangthaitb_id is not null     ---n-1---
                           ...
                           left join ttkd_bsc.nhanvien_202402 nvtp on c.MANV_THUYETPHUC = nvtp.ma_nv
...
--5865861
                               ...
                               */                                                            
where  a.thang_kt = 202401 --and rkm_id != 3369696 --a.ma_tb  in ('')
         ...
         */     --   and a.thangdc < 3 and c.so_thangdc>= 3 
         ...
         --      and exists (select * from tmp3 where lan = 2618 and thuebao_id = a.thuebao_id )
             --   and rownum < 3
--select * from css_hcm.toanha where duan_id in (345, 1382)
; 
select rkm_id from tmp3_60ngay
rollback;
select rkm_id from ttkd_Bsc.ct_bsc_tratruoc_moi_30day where thang  = 202402 group by rkm_id having count(rkm_id) > 1
...
commit;
select rkm_id from ttkd_bsc.ct_bsc_tratruoc_moi where thang = 202402 group by rkm_id having count(rkm_id) > 1
select* from bcss_hcm.ctkmtc_int_20230901
---Buoc 3---Check and except tra truoc khong bang cuoc PS
...
commit;
    select * from ttkd_bsc.ct_bsc_tratruoc_moi where thang = 202401
--    update 
    ct_bsc_tratruoc_moi a  
--    set tien_khop = 0 
...
select * from  ttkd_bsc.ct_bsc_tratruoc_moi a  where thang = 202309; and TIEN_THANHTOAN < 100000; and ma_tb = 'hcmtuananh1';
