update ttkd_Bsc.nhanvien set thang = 202411 , donvi = 'POT' where thang is null;
commit;

    /* -------------- UP LAI TEN PHONG BAN TEN_PB ------------------ */
update nhanvien a set ten_pb=(select distinct ten_pb from ttkd_bsc.nhanvien_202405 where ma_pb=a.ma_pb);
rollback;
commit ;
...
select distinct loai_ld from ttkd_Bsc.nhanvien_202411 where nhomld_id is null;
rollback;
commit;
...
SET MANV_NNL=UPPER(trim(MANV_NNL)),MANV_HRM=UPPER(trim(MANV_HRM)),MA_NV=UPPER(trim(MA_NV)),TEN_NV=initcap(trim(TEN_NV)),MA_VTCV=UPPER(trim(MA_VTCV)), TEN_VTCV=initcap(trim(TEN_VTCV)),
   ...
   where thang = 202411 and donvi = 'POT';
...
/* -------------- UP USER_IPCC ----------------- */
update ttkd_Bsc.nhanvien set user_ipcc='' where thang = 202411 and donvi = 'POT';

update  ttkd_Bsc.nhanvien set user_ipcc=REPLACE(MAIL_VNPT,'@vnpt.vn','') where user_ipcc is null and  thang = 202411 and donvi = 'POT';
  ;
SELECT* FROM ADMIN.NHANVIEN@DATAGUARD WHERE MA_NV='CTV029044';
commit;
...
update  ttkd_Bsc.nhanvien a set (a.nhanvien_id,a.user_dhsxkd)=(select nhanvien_id, user_dhsxkd 
            from ttkd_Bsc.nhanvien where thang = '202410' and donvi = 'POT' AND ma_nv=a.ma_nv)
            where thang = 202411 and donvi = 'POT';

;
commit ;    
rollback;
UPDATE  ttkd_Bsc.nhanvien SET MAnV_HRM = MA_NV where thang = 202411 and donvi = 'POT';
select* from ttkd_Bsc.nhanvien where thang = 202411 and donvi = 'POT';
;
...
where a.nhanvien_id is null  and thang = 202411 and donvi = 'POT';
 ;
commit;
...
and thang = 202411 and donvi = 'POT'; thang = 202411 and donvi = 'POT';
select* from ttkd_bsc.userld_202403_goc;
select* from ttkd_Bsc.nhanvien where nhanvien_id  is null and thang = 202411 and donvi = 'POT';
select* from admin.nhanvien@dataguard where ma_Nv='CTV088333';
commit;
select* from ttkd_Bsc.dm_ccos;
-- update user ccos
update ttkd_Bsc.dm_ccos set thang = 202411 where thang is null;
/* ------------------ UP USER CCOS TU ANH KHANG CUNG CAP ----------------- */
update ttkd_Bsc.nhanvien A SET (a.user_ccos,trangthai_ccos)=(select user_ccos, trangthai from dm_ccos where manv_hrm=a.ma_nv and thang='202411' )
where thang = 202411 and donvi = 'POT';
;--where a.user_ccos is null 
;
select* from dm_ccos;
select* from userld_202411_goc;
commit ;
--- 
update ttkd_Bsc.nhanvien A SET a.user_ccos=lower(a.user_ccos) where thang = 202411 and donvi = 'POT';;
commit;
/* ------------ UP USER_CCBS ------------- */
update ttkd_Bsc.nhanvien  a SET (a.USER_CCBS,a.tr_thai)=(SELECT USER_CCBS,tr_thai FROM TTKD_bSC.nhanvien WHERE THANG =202410 AND DONVI = a.donvi
    AND MANV_HRM=A.MANV_HRM) 
where a.user_ccbs is null and thang = 202411 and donvi = 'POT';;
COMMIT ;
select* from ttkd_Bsc.nhanvien WHERE USER_CCBS IS NULL;
...
and a.user_ccbs is null and thang = 202411 and donvi = 'POT';
;
commit ;
update ttkd_Bsc.nhanvien a set USER_CCBS = (Select user_ld from userld_202411_goc where mail_vnpt = email )
where thang = 202411 and donvi ='POT' and USER_CCBS is null;
update ttkd_Bsc.nhanvien A SET (A.USER_CCBS,a.tr_thai,a.trangthai_ccos)
                            =(SELECT USER_CCBS,tr_thai,user_ccos,trangthai_ccos 
                            FROM ttkd_Bsc.nhanvien where thang = 202410 and MANV_HRM=A.MANV_HRM and donvi = a.donvi) 
                            where thang = 202411 and donvi = 'POT';;
COMMIT ;

update ttkd_Bsc.nhanvien a set a.tr_thai=(select tr_thai from userld_202411_goc b where a.user_ccbs=b.user_ld)     
 where thang = 202411 and donvi = 'POT';
...
/* KIEM TRA TRUNG MANV */
select manv_hrm,count(*)sl from ttkd_Bsc.nhanvien  WHERE THANG = 202411 AND DONVI = 'POT' group by manv_hrm having count(*) > 1 ;
...
NGAYSINH, ttkd_Bsc.nhanvien_ID, GIOITINH, CMND, USER_DHSXKD, MANV_HRM, USER_CCOS, TRANGTHAI_CCOS, USER_IPCC, PHAN_LOAI, LOAI_LAODONG, NHOMLD_ID
from ttkd_Bsc.nhanvien_202406_lan1 ;
select* from ttkd_Bsc.nhanvien where thang = 202406 and donvi = 'POT';
select * from ttkd_Bsc.nhanvien_202406_lan2 a;
select* from ttkd_Bsc.nhanvien;
create table nhanvien_POT as select* from nhanvien_POT where 1=2;
...
WHERE thang = 202411 and donvi = 'TTKD' AND NOT EXISTS(SELECT 1 FROM TTKD_bSC.DM_TO B WHERE A.MA_TO=B.MA_TO and hieuluc=1) AND A.MA_TO IS NOT NULL GROUP BY A.MA_TO, A.TEN_TO, A.MA_PB, A.TEN_PB  
; 
select* from DM_TO;
COMMIT ;
...
FROM ttkd_Bsc.nhanvien a
WHERE thang = 202411 and donvi = 'TTKD' AND NOT EXISTS (SELECT 1 FROM TTKD_bSC.DM_VTCV B WHERE A.MA_VTCV=B.MA_VTCV) AND A.MA_VTCV IS NOT NULL GROUP BY A.MA_VTCV, A.TEN_VTCV
; 
commit;
SELECT * FROM DM_VTCV ;
select* from nhanvien_POT where thang = 202404;
...
FROM ttkd_Bsc.nhanvien a
WHERE thang = 202411 and donvi = 'TTKD' AND NOT EXISTS(SELECT 1 FROM TTKD_bSC.DM_PHONGBAN B WHERE A.MA_PB=B.MA_PB) AND A.MA_PB IS NOT NULL GROUP BY A.MA_PB, A.TEN_PB
; 
commit;
...
select 202411 THANG,'TTKD' DONVI, MANV_NNL, MA_NV, TEN_NV, MA_VTCV, TEN_VTCV, MA_TO, TEN_TO, MA_PB, TEN_PB, LOAI_LD, USER_CCBS, TR_THAI, USER_CCBS2, MAIL_VNPT, SDT, NGAYSINH,
NHANVIEN_ID, GIOITINH, CMND, USER_DHSXKD, MANV_HRM, USER_CCOS, TRANGTHAI_CCOS, USER_IPCC, PHAN_LOAI, LOAI_LAODONG, NHOMLD_ID
from nhanvien;
delete from ttkd_bsc.nhanvien where thang = 202411 and donvi = 'TTKD';
commit;
select* from ttkd_bsc.nhanvien where thang = 202409 and donvi = 'POT'; AND TEN_nV LIKE '%Nga';
...
UPDATE NHANVIEN_202411 SET NHOMLD_ID = 2 WHERE NHOMLD_ID IS NULL AND MA_nV LIKE 'HCM%';