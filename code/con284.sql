...
select* from ttkd_Bsc.ct_dongia_Tratruoc where thang = 
...
from css_hcm.db_thuebao a
                ...
                left join css_hcm.diachi_tb_sub i on a.thuebao_id = i.thuebao_id
                ...
                left join (select distinct km.thuebao_id, km.loaitb_id
                                        ...
                                        where --Thay doi thang
                                                    202409 between thang_bddc and least(thang_ktdc, nvl(thang_kt_dc, 999999), nvl(thang_huy, 999999))
                                    ) km on a.thuebao_id = km.thuebao_id and km.loaitb_id = 61
               left join (select thuebao_id, loaitb_id, nvl(tyle_sd, tyle_tb) tyle_tb, nvl(cuoc_sd, cuoc_tb) cuoc_tb, congvan_id from v_thongtinkm_all
                                                where ((tyle_sd <> 100 and tyle_sd + cuoc_sd > 0) or (tyle_tb <> 100 and tyle_tb + cuoc_tb > 0))
                                                        and 202409 between thang_bd_mg and least(thang_kt_mg, nvl(thang_kt_dc, 99999999), nvl(thang_huy, 99999999))
...
where a.trangthaitb_id not in (7, 8, 9) and a.loaitb_id = 61 and a.doituong_id not in (35, 57) and chuquan_id in (266, 145, 264) and to_number(to_char(a.ngay_sd, 'yyyymm')) <= 202409
