...
rollback;
    ----tao du lieu view tong quan cac gia KPI
            ...
            delete from backup_tonghop_dongia where thang = 202401;
            commit;
...
set HCM_LUONG_QLDB_062 = case when exists (select * from ttkd_bsc.bangluong_dongia_qldb where thang = a.thang and HCM_LUONG_QLDB_008  >= 0.9 and manv = a.manv)
            then greatest(0.5 * (select nvl(HCM_LUONG_QLDB_054, 0) from TEMP_QLDB where thang = a.thang and manv = a.manv)
                                            , (SELECT GIATRI
                                                  FROM backup_tonghop_dongia
        ...
        when exists (select 1 from ttkd_bsc.bangluong_dongia_qldb where thang = a.thang and HCM_LUONG_QLDB_008  < 0.9 and manv = a.manv)
                then greatest(0.2 * (select nvl(HCM_LUONG_QLDB_054, 0) from TEMP_QLDB where thang = a.thang and manv = a.manv)
                                            , (SELECT GIATRI
                                                  FROM backup_tonghop_dongia
                                                  WHERE thang = a.thang and ma_kpi = 'HCM_LUONG_QLDB_062' and ma_nv = a.manv)
                                        )
