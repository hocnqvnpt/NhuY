...
, ct as (select min(aa.NGAY_NGANHANG) NGAY_NGANHANG, bb.phieu_id
    ...
    , kq_ghtt as (select 
              ...
              , p.thungan_tt_id, p.kenhthu_id, p.ht_tra_id, hdtb.tthd_id, hdkh.ngay_yc, p.thungan_hd_id, tp.ma_nv, tp.ma_To,
               kt.kenhthu , nh.ten_nh ten_nganhang ,ht.ht_tra ten_ht_tra, 3 lan
                     from  css_hcm.hd_khachhang hdkh
													...
													join css_hcm.ct_tienhd a on hdtb.hdtb_id = a.hdtb_id
                                                       ...
                                                       join admin_hcm.nhanvien_onebss nv on hdkh.ctv_id = nv.nhanvien_id
                                                       join ttkd_Bsc.nhanvien_202406 tp on nv.ma_nv = tp.ma_nv
				    
                     where  a.khoanmuctt_id = 11 and  tp.ma_pb = 'VNP0701400' and
                     hdtb.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224,136) and a.tien > 0 
                                      and ( to_number(to_char(p.ngay_tt, 'yyyymmdd')) between 20240601 and 20240701)  --    and p.ma_Gd = 'HCM-TT/02814791'            
)  
