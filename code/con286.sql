...
from tmp3_30ngay a
    join giao_202409 b on a.thuebao_id = b.thuebao_id
    );
...
-- insert into nhuy.ct_bsc_tratruoc_moi_30day-create table t4 as
    ...
    with t0 as (select c0.thang_kt, c0.thuebao_id, c0.phieutt_id, 'matt' ma_tt, c0.ma_gd, c0.rkm_id, c0.ngay_bd_moi, c0.tien_thanhtoan, c0.vat
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
            ...
            , ds as (select ten_pbh, thuebao_id,202408 thang_kt
                    from giao_202409 where thang_kt = 202408
                    )
    ...
    select 202409 thang, 0, 0 pbh_ql_id, 0 pbh_giao_id, 0 tbh_giao_id
    ...
    from ds a
                                ...
                                left join ttkd_bsc.nhanvien nvtp on c.MANV_THUYETPHUC = nvtp.ma_nv and nvtp.thang = 202409 -- change
                                left join goi on a.thuebao_id = goi.thuebao_id and rn = 1
                        
    where  a.thang_kt = 202408 --and rkm_id != 3369696 --a.ma_tb  in ('') -- change
           --      and a.gh_id is null
