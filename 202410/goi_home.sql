...
create table CT_HOMECOMBO_202410 as 
select* from  (
...
hdmoi as (
        ...
        and trunc(hdkh.ngay_yc) >= to_date('01/10/2024','dd/mm/yyyy') -- CHANGE
        )
select distinct
        a.*,  
...
from cttb_dk_goi_home a  
            ...
            left join (select a.ma_nv, a.ten_nv, a.ma_to, a.ma_pb, b.ten_to, a.ten_pb, c.ten_kenhbh
                        from TTKD_BSC.NHANVIEN  a join ttkd_bsc.dm_to b on a.ma_to = b.ma_to and a.thang = 202410 -- CHANGE
            ...
            left join ttkd_bsc.NHANVIEN  e -- CHANGE
                    on a.nguoi_th = e.user_ccbs        and e.thang = 202410           
)
;
...
update ct_homecombo_202410 set ten_kenhbh = 'VTTP', DONVI = 'VTT'
where mnv_tt in (select ma_nv from ttkd_bsc.nhanvien where thang = 202410 and donvi = 'VTTP');
rollback;
commit;
...
-- 
update ct_homecombo_202410 a 
set donvi_tiepthi = (select bo_dau(ten_pb)
from ttkd_Bsc.nhanvien_Vttp where thang = 202404 and ma_nv = a.mnv_tt) ,
ten_nv =  (select ten_nv from ttkd_Bsc.nhanvien where thang = 202410 and ma_nv = a.mnv_tt),
to_tiep_thi = (select ten_to from ttkd_Bsc.nhanvien where thang = 202410 and ma_nv = a.mnv_tt),
ten_kenhbh = 'VTTP',
ma_to = (select ma_to from ttkd_Bsc.nhanvien where thang = 202410 and ma_nv = a.mnv_tt),
ma_pb = (select ma_pb from ttkd_Bsc.nhanvien where thang = 202410 and ma_nv = a.mnv_tt)
where  DONVI = 'VTT'       ;
--
...
create table ct_homecombo_2024 as
select * from CT_HOMECOMBO_202401-- where thang = 202312;
...
insert into ct_homecombo_2024
    ...
    , TEN_KENHBH, DONVI_TIEPTHI, PHONG_DK_GOI, MA_TO, MA_PB)
        (select 202410 thang, CHU_NHOM, THANHVIEN, MNP, LOAI_GOI, LOAI_TB, ACCOUNT_FIBER, TRANGTHAI_FIBER, NGAYHC_FIBER, ACCOUNT_MYTV, TRANGTHAI_MYTV, NGAYHC_MYTV, NGAY_ACTIVE_VNP, TEN_KH, DIACHI_KH
                        , NGAY_DK, TRANGTHAI_GOI, GOI_MOI_HIENHUU, MNV_TT, NGUOI_TH, DONVI, NGAY_SD_FIBER, TOCDO_ID, MA_TD, hdtb_id, NGAY_YC_FIBER, NGAY_HT_FIBER, TRANGTHAI_HD, GHICHU, TEN_NV, TO_TIEP_THI
                        , TEN_KENHBH, DONVI_TIEPTHI, PHONG_DK_GOI, MA_TO, MA_PB
        from ct_homecombo_202410)  ----change---
;
commit;
rollback;
...
-------Append vao table nam----------
               ...
               --  create table CT_GDVP_HUY_2023 as
                 select * from CT_GDVP_HUY_2024 --where thang = 202311-- and  LOAI = 'HOME';
            create table CT_GDVP_HUY_2024 as select* from hocnq_ttkd.CT_GDVP_HUY_2023 where 1=2
                    insert into CT_GDVP_HUY_2024
                                                            (select distinct 202410, CHU_NHOM, 'HOME' LOAI, LOAI_GOI
