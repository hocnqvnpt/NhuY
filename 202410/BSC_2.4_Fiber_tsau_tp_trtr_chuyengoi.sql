   ...
   -- insert into tmp3_ts
                ...
                from (select  --gh.khachhang_id, gh.thuebao_id, gh.duan_id, gh.ma_tb, gh.ma_tt, gh.loaitb_id, gh.thang_kt
                             ...
                             from css_hcm.phieutt_hd a, css_hcm.ct_phieutt b
                                        , (select distinct hdtb_id, nvl(h.rkm_id, g.rkm_id) rkm_id, nvl(h.ngay_bddc, g.ngay_bddc) ngay_bddc, nvl(h.ngay_ktdc, g.ngay_ktdc) ngay_ktdc from css_hcm.hdtb_datcoc g 
                                        left join css_hcm.db_datcoc h on g.thuebao_dc_id = h.thuebao_dc_id
                                        ...
                                        , (select distinct thuebao_id, ma_tb 
                                              ...
                                              where thang_kt=0 and to_number(to_char(ngay_cn, 'yyyymm')) = 202404 
--                                              and thuebao_id= 1730769  ---change n
                                                            ) gh                                             
                             ...
                             where  a.kenhthu_id not in (6) and a.trangthai = 1 and hdtb.ma_tb = 'hung655'
...
--                                            and ((a.ht_tra_id <> 2 and to_number(to_char(a.ngay_tt, 'yyyymm')) <= 202403)                     ----change thang n---
--                                                            or (a.ht_tra_id in (2,7,4) and to_number(to_char(ct.ngay_nganhang, 'yyyymm')) <=202403))                     ----change thang n---
                                           -- and gh.ma_tb in ('fibervanloi')   ----loai taykhi can---
                                        --   and hdttdc.hdtb_id not in (11189895, 11110732)    ----loai taykhi can---
                            ) a
                            where ngay_bd_moi is not null  and a.ma_tb = 'hung655'
...
create table ct_bsc_trasau_tp_tratruoc as
select a.*, loaitb_id
from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a
    left join css_hcm.db_Thuebao b on a.thuebao_id = b.thuebao_id;
    commit;
    
    select thuebao_id from tmp3_ts_t where thuebao_id not in (select thuebao_id from tmp3_ts);
    ------Chay theo vb moi 86
...
, PHIEUTT_ID, TIEN_KHOP, MA_CHUNGTU, HT_TRA_ID, KENHTHU_ID, loaitb_id)
...
select 202405 thang,  -1 gh_id, dbtb.ma_tb, a.thuebao_id
           ...
           , case when a.rkm_id is null then null
                        when a.ht_tra_id in (1, 7) then 1
...
--                left join c on a.thuebao_id = c.thuebao_id
            left join ttkd_bsc.nhanvien_202406 nv on a.MANV_THUYETPHUC = nv.ma_nv  ; ---change thang n----
where a.rnk = 1 ; and a.ma_tb ='ctylekhuong34';
commit;
select rkm_id from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202403 group by rkm_id having count(rkm_id) > 1
...
---check-
---- loai tru  ptm n-1, n
delete from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202403 and thuebao_id in (
select a.thuebao_id from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a join css_hcm.db_thuebao b on a.thuebao_id = b.thuebao_id where thang = 202403 and to_number(to_char(b.ngay_sd, 'yyyymm')) >= 202402);
select b.ngay_sd, a.* from ttkd_bsc.ct_bsc_trasau_tp_tratruoc a join css_hcm.db_thuebao b on a.thuebao_id = b.thuebao_id where thang = 202403 and to_number(to_char(b.ngay_sd, 'yyyymm')) >= 202402;
commit;
...
where rkm_id is not null and thang = 202403 and exists (select 1 from v_thongtinkm_all where cuoc_dc > 0 
                                                              ...
                                                              --      and to_date('01/07/2023', 'dd/mm/yyyy') between ngay_bddc and least(NGAY_ktdc, nvl(NGAY_THOAI -1, sysdate + INTERVAL '50' YEAR), nvl(NGAY_huy -1, sysdate + INTERVAL '50' YEAR))
                                                                    and ngay_bddc <= to_date('29/02/2024', 'dd/mm/yyyy')
...
delete from ttkd_bsc.ct_bsc_trasau_tp_tratruoc where thang = 202402 and thuebao_id in (
            ...
            where thang = 202403 and to_number(to_char(b.ngay_sd, 'yyyymm')) >= 202402);
            --
...
where rkm_id is not null and thang = 202403 and exists (select 1 from v_thongtinkm_all where cuoc_dc > 0 
                                               -- and 202306 between thang_bddc and least (thang_ktdc, nvl(thang_kt_dc, 999999), nvl(thang_huy, 999999))
                                          --      and to_date('01/07/2023', 'dd/mm/yyyy') between ngay_bddc and least(NGAY_ktdc, nvl(NGAY_THOAI -1, sysdate + INTERVAL '50' YEAR), nvl(NGAY_huy -1, sysdate + INTERVAL '50' YEAR))
                                                and ngay_bddc <= to_date('30/02/2024', 'dd/mm/yyyy')
                                                and least(NGAY_ktdc, nvl(NGAY_THOAI -1, sysdate + INTERVAL '50' YEAR), nvl(NGAY_huy -1, sysdate + INTERVAL '50' YEAR))
                                                                    >= to_date('01/02/2024', 'dd/mm/yyyy')
                                                and a.thuebao_id = thuebao_id
                                                )
...
commit;
select* from ttkd_Bsc.ct_bsc_Trasau_tp_tratruoc where thang = 202407;