...
delete from ttkd_bsc.ct_bsc_tratruoc_moi_30day where thang = 202410;
commit;
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
            ...
            , ds as (select ten_pbh, thuebao_id, thang_kt
                    from giao_oneb where thang_kt =202410
                    )
...
commit;
rollback;
select distinct phong_cs , ma_pb from ttkd_Bsc.ct_Bsc_Tratruoc_moi where thang = 202410; and ma_pb is null;
update  ttkd_Bsc.ct_Bsc_Tratruoc_moi a 
set ma_pb = case when phong_Cs ='Phòng Bán Hàng Doanh Nghi?p 2' then  'VNP0702400'
                when phong_Cs ='Phòng Bán Hàng Doanh Nghi?p 1' then  'VNP0702300'
                when phong_Cs ='Phòng Bán Hàng Doanh Nghi?p 3' then  'VNP0702500'
                end
where thang = 202410 and ma_pb is null;
update ttkd_Bsc.ct_Bsc_Tratruoc_moi a 
set ma_pb = (select distinct ma_pb from ttkd_Bsc.nhanvien where thang = 202410 and ten_pb = phong_cs)
where thang = 202410 and ma_pb is null;
...
--****C.4(BHKV), C.4(KHDN) Ty le thue bao ghtt khong thanh cong tren tap KH thuoc BHKV, BHDN giao ds het han----
                select thang, ma_pb,'HCM_TB_GIAHA_022' loai_tinh, 'KPI_PB', phong_giao
...
;
commit;
delete from  ttkd_Bsc.tl_giahan_Tratruoc where thang = 202410 and ma_kpi ='';