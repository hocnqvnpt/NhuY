...
, tt1 as (
    ...
    from dc 
    where 202412 between to_number(to_char(ngay_bddc, 'yyyymm')) and to_number(to_char(ngay_kt_mg,'yyyymm'))
) 
, tt2 as 
(
    select thuebao_id, rkm_id,ROW_NUMBER() OVER (PARTITION BY dc.thuebao_id ORDER BY dc.rkm_id DESC) AS rn
...
--													   left join ct on p.phieutt_id = ct.phieu_id
                     ...
                     hdtb.loaitb_id in (18, 58, 59, 61, 171, 210, 222, 224,136) and a.tien > 0 
                                      and ( ( to_number(to_char(p.ngay_tt, 'yyyymmdd')) between 20250101 and 20250131)  --    and p.ma_Gd = 'HCM-TT/02814791'            
                                      or ( to_number(to_char(hdtb.ngay_ht, 'yyyymmdd')) between 20250101 and 20250131) and p.ngay_tt is null ) --    and p.ma_Gd = 'HCM-TT/02814791'            
)  
select *
...
create table tmp_ipcc as  select a.ob_id, ngay_tao, a.thuebao_id, ma_tb, LOAITB_ID, TOCDO_ID, a.KHACHHANG_ID, THANHTOAN_ID
                                ...
                                to_char(to_date(a.TD_TH,'dd/mm/yyyy'),'yyyymm') in ('202412','202501','202501');
...
insert into ttkd_bsc.nhuy_ct_ipcc_obghtt
        ...
        select 202501 thang, a.ob_id, ngay_tao, a.thuebao_id, ma_tb, LOAITB_ID, TOCDO_ID, a.KHACHHANG_ID, THANHTOAN_ID
...
delete from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202501;
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
    ...
    select 
            202501 thang, a.ob_id, a.ngay_ob, x.thuebao_id, dbtb.ma_tb,a.ngay_bddc ngay_bddc_cu, 
           ...
           , x.NVGIAOPHIEU_ID, x.DVGIAOPHIEU_ID ,   m.nhomtb_id , dbtb.khachhang_id, x.phieutt_id, x.ht_tra_id, x.kenhthu_id, x.loaitb_id, dbtb.thanhtoan_id, nvxp.ma_nv nhanvien_xuathd
                , case when x.rkm_id is null then null
                                when x.ma_gd in (select ma_gd from qrcode where thang = 202501) then 2
                                when x.ht_Tra_id in (216) then 2
                                when x.ht_tra_id in (2,4,5) then 0 else 4 end tien_khop
...
--                                    on a.pbh_id_th = pb_th.pbh_id
                                ...
                                left join ttkd_bsc.nhanvien nvtp on x.MANV_THUYETPHUC = nvtp.ma_nv and nvtp.thang = 202501 and nvtp.donvi = 'TTKD'
                                left join ttkd_bsc.nhanvien nvob on a.ma_nv = nvob.ma_nv and nvob.thang = 202501 and nvob.donvi = 'TTKD'
                                ...
                                left join ttkd_bsc.nhanvien nvxp on xp.ma_nv = nvxp.ma_nv and nvxp.thang = 202501 and nvxp.donvi = 'TTKD'
where  not exists (select rkm_id from ttkd_bsc.nhuy_ct_bsc_ipcc_obghtt where  nvl(tien_khop,0) > 0 and thang = 202412 and rkm_id = x.rkm_id )
                                ;
select* from ttkd_bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202501;
commit;
-- phi thu tai nha
...
where thang = 202501 and ma_Tb in (select ma_Tb from phithu_tainha where thang = 202501);

commit;
--update chung tu

select distinct ht_Tra_id from  ttkd_bsc.nhuy_ct_bsc_ipcc_obghtt  where thang= 202501 and tien_khop = 0;
update  ttkd_bsc.nhuy_ct_bsc_ipcc_obghtt set tien_khop = 1 where thang= 202501 and tien_khop = 0 and ht_Tra_id = 214;
...
--                              
                        where a.rkm_id is not null and thang = 202501 --and c.ma_pb = 'VNP0701400'--and a.thang = 202403 and to_number(to_char(ngay_tt,'yyyymm'))  = 202403---        CHANGE
...
where thang = 202501 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where  ma_kpi in ('HCM_SL_ORDER_001')
    and thang = 202501 and MA_VTCV = a.MA_VTCV)
and ma_kpi = 'HCM_SL_ORDER_001';
commit;
select* from TTKD_BSC.bangluong_kpi a where thang = 202501 and ma_kpi = 'HCM_SL_ORDER_001'
