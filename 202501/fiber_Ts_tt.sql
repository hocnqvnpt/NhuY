...
--                left join c on a.thuebao_id = c.thuebao_id
            ...
            select* from tmp3_tp;
            select* from ttkd_bsc.ct_bsc_Trasau_tp_tratruoc where thang = 202410;
            rollback;
            commit;
...
--                                            left join  km on a.rkm_id = km.rkm_id
                        where a.thang = 202410-- and --a.rkm_id is not null AND -- don gia TP 
...
where ma_kpi = 'DONGIA' --and loaitb_id in (58,59)
            and loai_tinh in ('DONGIA_TS_TP_TT') and a.thang = 202410 and b.ma_pb not in ('VNP0702300','VNP0702400','VNP0702500')
...
select* from  ttkd_bsc.tl_giahan_Tratruoc where loai_tinh in ('DONGIA_TS_TP_TT') and thang = 202408;
