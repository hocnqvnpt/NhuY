select a.*, b.ten_pb from giao_khoi_bh a
    left join ttkd_Bsc.nhanvien b on a.ma_nv_am =b.ma_nv and b.thang = 202501;
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
            ...
            , ds as (select b.ten_pb ten_pbh, b.ma_nv, b.ma_to, b.ma_pb, thuebao_id,202412 thang_kt
                    from giao_khoi_Bh a
                        left join ttkd_Bsc.nhanvien b on a.ma_nv_am =b.ma_nv and b.thang = 202501
    ...
    select 202501 thang, 0, 0 pbh_ql_id, 0 pbh_giao_id, 0 tbh_giao_id
...
UPDATE TTKD_bSC.ct_Bsc_Tratruoc_moi_1tr set tien_khop = 1 WHERE THANG = 202501 AND tien_khop = 0 and ma_Gd in (
select ma from ttkdhcm_ktnv.ds_chungtu_Nganhang_phieutt_Hd_1);
commit;
                 
UPDATE TTKD_bSC.ct_Bsc_Tratruoc_moi_1tr set thang = 202501;
select* from TTKD_bSC.ct_Bsc_Tratruoc_moi_1tr where thang = 202501;;
...
commit;
 insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TONG, DA_GIAHAN_DUNG_HEN
                            ...
                            where thang = 202501 and MA_KPI in ('HCM_TB_GIAHA_031')
...
with pb as (
    select  ma_Nv, ma_pb
    from  ttkd_bsc.blkpi_dm_to_pgd where thang = 202501 AND MA_KPI = 'HCM_TB_GIAHA_031' and phutrach='Phong'
)
, t_to as (
    select  ma_Nv, ma_pb, ma_to
    from  ttkd_bsc.blkpi_dm_to_pgd where thang = 202501 AND MA_KPI = 'HCM_TB_GIAHA_031' and phutrach='To'
)
...
where a.thang = 202501 and loai_tinh = 'KPI_TO' and a.MA_KPI in ('HCM_TB_GIAHA_031') 
group by a.THANG, a.MA_KPI, c.ma_nv, a.MA_PB, b.ma_nv
;
SELECT* FROM ttkd_bsc.tl_giahan_tratruoc  WHERE THANG = 202501 AND MA_KPI ='HCM_TB_GIAHA_031' and loai_tinh ='KPI_PB';
---
 insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TONG, DA_GIAHAN_DUNG_HEN
                                        , DTHU_DUYTRI, DTHU_THANHCONG_DUNG_HEN, TYLE, heso_giao)
with t_to as (
    select  ma_Nv, ma_pb, ma_to
    from  ttkd_bsc.blkpi_dm_to_pgd where thang = 202501 AND MA_KPI = 'HCM_TB_GIAHA_030' and phutrach='To'
)
...
set 
 ...
 ,tyle_thuchien = (select round(sum(DA_GIAHAN_DUNG_HEN)*100/sum(TONG), 2) 
                                                    from ttkd_bsc.tl_giahan_tratruoc where thang = to_char(trunc(sysdate, 'month')-1, 'yyyymm') 
                                                                    and ma_nv = a.ma_nv and ma_kpi = 'HCM_TB_GIAHA_031')
where thang = 202501 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_TB_GIAHA_031') and thang = 202501 and MA_VTCV = a.MA_VTCV)
...
, tyle_thuchien  = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
                                                            where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_TO' 
                                                                                and ma_to = a.ma_to and ma_pb = a.ma_pb and ma_kpi = 'HCM_TB_GIAHA_031')
where thang = 202501 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where to_truong_pho = 1 and ma_kpi in ( 'HCM_TB_GIAHA_031') 
                                and thang = 202501 and MA_VTCV = a.MA_VTCV) and ma_kpi = 'HCM_TB_GIAHA_031';
--- pho giam doc
...
--- 
update ttkd_Bsc.bangluong_kpi a
set diem_cong = case when tyle_thuchien >= 80 then 2 else 0 end
where  thang = 202501 and  exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where  ma_kpi in ('HCM_TB_GIAHA_031')  
                                and thang = 202501 and MA_VTCV = a.MA_VTCV)and ma_kpi = 'HCM_TB_GIAHA_031';

update ttkd_Bsc.bangluong_kpi a
set diem_tru = case when tyle_thuchien < 75 then 2 else 0 end
where  thang = 202501 and  exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_TB_GIAHA_031')  
                                and thang = 202501 and MA_VTCV = a.MA_VTCV) and ma_kpi = 'HCM_TB_GIAHA_031';
                                
update  ttkd_Bsc.bangluong_kpi a set mucdo_hoanthanh = case when tyle_Thuchien >= 80 then 100+2*(tyle_Thuchien-80)
                            when tyle_thuchien >= 75 and tyle_thuchien < 80 then 50 + 8*(tyle_thuchien-75)
                            when tyle_thuchien < 75 then 0 end
where thang = 202501 and ma_Kpi = 'HCM_TB_GIAHA_031'
                           ;
commit;
--- do bang luong 030
update TTKD_BSC.bangluong_kpi a 
set tyle_thuchien = (select TYLE from ttkd_bsc.tl_giahan_tratruoc 
                where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_PB' 
                and ma_PB = a.MA_PB and ma_kpi = 'HCM_TB_GIAHA_031')
        ,thuchien = (select DA_GIAHAN_DUNG_HEN from ttkd_bsc.tl_giahan_tratruoc 
                where thang =  to_char(trunc(sysdate, 'month')-1, 'yyyymm')  and loai_tinh ='KPI_PB' 
                and MA_PB = a.MA_PB and ma_kpi = 'HCM_TB_GIAHA_031')
        , giao = (select tong from ttkd_bsc.tl_giahan_tratruoc 
...
where thang = 202501 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where giamdoc_phogiamdoc = 1 
                                            ...
                                            and thang = 202501 and MA_VTCV = a.MA_VTCV)and ma_kpi = 'HCM_TB_GIAHA_030';
        ROLLBACK;                                
update ttkd_Bsc.bangluong_kpi a
set mucdo_hoanthanh = case 
                            when tyle_thuchien >= 80  then 100 + 2*(tyle_thuchien-80)
                            when tyle_thuchien >= 75 and tyle_thuchien < 80 then 50 + 8*(tyle_thuchien-75)
                            when tyle_thuchien < 75 then 0 end
where thang = 202501 and ma_Kpi = 'HCM_TB_GIAHA_030'

;
select* from ttkd_Bsc.blkpi_dm_to_pgd where thang =202501 and ma_kpi ='HCM_TB_GIAHA_031';
commit;
select* from ttkd_Bsc.nhanvien where thang = 202501 and ten_Nv like '%Trinh';
select* from ttkd_Bsc.bangluong_kpi a where thang = 202501 and ma_Kpi = 'HCM_TB_GIAHA_030';
select* from ttkd_Bsc.tl_giahan_tratruoc a where thang = 202501 and ma_Kpi = 'HCM_TB_GIAHA_031' and loai_Tinh = 'KPI_PB';
