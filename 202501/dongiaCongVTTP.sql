...
select 202501 THANG, LOAI_TINH, MA_KPI, MA_NV, THUEBAO_ID, MA_TB, TIEN_DC_CU, MANV_GIAO, MA_NV_CN, MANV_THUYETPHUC, SOTHANG_DC, HT_TRA_ONLINE, KHUVUC, -DONGIA,
...
rollback;
rollback;
select * from ttkd_bsc.nhanvien where thang = 202501 and donvi = 'POT';-- and donvi_id = 283468;
;
delete from ttkd_Bsc.dongia_vttp where thang = 202501;
commit; 
-- code moi
...
with ds as (
    select* from ttkd_bsc.ct_dongia_tratruoc a where thang =  202501 and loai_tinh = 'DONGIATRA_OB' and 
        (exists (select 1 from ttkd_bsc.nhanvien where thang = 202501 and donvi IN( 'VTTP','POT') and ma_nv = a.ma_nv) or 
        exists (select 1 from ttkd_bsc.nhanvien where thang = 202501 and donvi  IN( 'VTTP','POT') and ma_nv = a.nhanvien_xuathd))
)
    select 
    ds.THANG, LOAI_TINH, MA_KPI, ds.MA_NV, THUEBAO_ID, MA_TB,ds.MA_NV MANV_THUYETPHUC, SOTHANG_DC,DONGIA, DTHU, NGAY_TT,KHACHHANG_ID, HESO_dichvu, TIEN_KHOP,
            round(nvl(tien_thuyetphuc*heso_chuky*heso_dichvu,0)) tien_tp
            , b.ten_nv, b.ten_to dv_cap1, b.ten_pb dv_cap2, NVL(dv1.donvi_cha_id,283451), heso_chuky, nhanvien_xuathd,  round(nvl(tien_xuathd*heso_chuky*heso_dichvu,0)) tien_xp
    from ds
...
;
COMMIT;
 select* from ttkd_bsc.nhanvien where thang = 202501 AND MA_NV ='HCM001525';
update ttkd_Bsc.dongia_vttp 
set TIEN_DONGIA = 0
where ma_nv not in (Select ma_nv from ttkd_bsc.nhanvien where thang = 202501 and donvi IN('POT', 'VTTP')) and thang = 202501;
commit;
update ttkd_Bsc.dongia_vttp 
set TIEN_XHDON = 0
where nvl(MANV_XHDON,'a') not in (Select ma_nv from ttkd_bsc.nhanvien where thang =  202501 and donvi IN ('POT', 'VTTP')) and thang = 202501 and TIEN_XHDON > 0;
...
-- tien thu hoi
insert into ttkd_Bsc.dongia_Vttp(THANG, LOAI_TINH, MA_KPI, MA_NV, THUEBAO_ID, MA_TB, MANV_THUYETPHUC, SOTHANG_DC,DONGIA, DTHU, NGAY_TT,KHACHHANG_ID, HESO, TIEN_KHOP, TIEN_DONGIA, 
TEN_NV, DV_CAP1, DV_CAP2, DONVI_ID, HESO_CHUKY, MANV_XHDON)
with ds as(
     select b.* from thuhoi_Bsc_dongia a 
            ...
            where a.thang = 202412 and a.ma_pb is null
)
  select ds.THANG+1, LOAI_TINH, MA_KPI, ds.MA_NV, THUEBAO_ID, MA_TB,ds.MA_NV MANV_THUYETPHUC, SOTHANG_DC,DONGIA, DTHU, NGAY_TT,KHACHHANG_ID, HESO_dichvu, TIEN_KHOP,
    ...
    from ds
        join ttkd_bsc.nhanvien b on ds.ma_nv= b.ma_Nv and ds.thang = b.thang and donvi = 'VTTP'
        left join admin_hcm.nhanvien_onebss nv on ds.ma_nv = nv.ma_nv
        left join admin_hcm.donvi dv1 on nv.donvi_id = dv1.donvi_id
    where loai_tinh = 'DONGIATRA_OB'
;
SELECT MA_tB FROM ttkd_Bsc.dongia_Vttp WHERE THANG = 202501 AND LOAI_TINH = 'DONGIATRA_OB' GROUP BY MA_tB,MA_NV HAVING COUNT(MA_tB)>1;
delete ttkd_Bsc.dongia_Vttp WHERE THANG = 202501 AND LOAI_TINH = 'DONGIATRA_OB' AND MA_tB ='thuy114-3';IN ('huynhthumai','thichinh1002','hcm_thichinh1002','thuy114-3');
rollback;
select* from admin_hcm.nhanvien_onebss where ma_nv='CTV021801';
commit;
-- 
select distinct loai_tinh from ttkd_Bsc.dongia_Vttp where  thang = 202501;
--
with tien as (
...
from tien a
    join ttkd_bsc.nhanvien b on a.ma_nv = b.ma_nv and a.thang = b.thang and donvi   IN( 'VTTP','POT')
group by a.thang,a.ma_nv,b.ten_nv,b.ten_to, b.ten_pb
;
select * from ttkdhcm_ktnv.dm_quanly_daily_moi where hieuluc=1 and daily_id in (4,5);
...
select* from ttkd_bsc.dongia_Vttp a where thang = 202501; and tien_dongia > 0;
...
with tien as (
    ...
    from ttkd_bsc.dongia_vttp where thang = 202501
--    and tien_dongia > 0
    union all
...
,sl as (
    ...
    from  ttkd_Bsc.dongia_vttp 
    where thang = 202501 and manv_xhdon is not null and manv_xhdon != ma_nv
)
,abc as (
...
having sum(tien_dongia) > 0;
select* from ttkd_bsc.dongia_vttp where thang = 202501
...
with tien as (
    select thang,ma_Nv, tien_dongia
    from ttkd_Bsc.dongia_vttp 
    where thang = 202501  --and tien_dongia > 0
    union all
    select thang,MANV_XHDON,  TIEN_XHDON
    from ttkd_Bsc.dongia_vttp 
    where thang = 202501 
