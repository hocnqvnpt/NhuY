...
select * from cttb_dk_goi_home a where account_Fiber in ('ducthoi2407','ndv.1','vxh-2');;
...
group by chu_nhom having count(distinct ngay_active_vnp) >1)
;
select* from cttb_dk_goi_home;
---Buoc 2 thong tin ghep goi, huy goi
create table CT_HOMECOMBO_202412 as 
select* from  (
...
hdmoi as (
        ...
        and trunc(hdkh.ngay_yc) >= to_date('01/12/2024','dd/mm/yyyy') -- CHANGE
        )
select distinct
        a.*,  
...
from cttb_dk_goi_home a  
            ...
            left join (select a.ma_nv, a.ten_nv, a.ma_to, a.ma_pb, b.ten_to, a.ten_pb, c.ten_kenhbh
                        from TTKD_BSC.NHANVIEN  a join ttkd_bsc.dm_to b on a.ma_to = b.ma_to and a.thang = 202412 -- CHANGE
            ...
            left join ttkd_bsc.NHANVIEN  e -- CHANGE
                    on a.nguoi_th = e.user_ccbs        and e.thang = 202412           
)
;
select* from   cttb_dk_goi_home; not in (
select ACCOUNT_FIBER from CT_HOMECOMBO_202412  );
delete from CT_HOMECOMBO_202412 where chu_nhom is null;
commit;
-- update phong ban TTVT
-- 
update ct_homecombo_202412 a 
set donvi_tiepthi = (select bo_dau(ten_pb)
from ttkd_Bsc.nhanvien_Vttp where thang = 202404 and ma_nv = a.mnv_tt) ,
ten_nv =  (select ten_nv from ttkd_Bsc.nhanvien where thang = 202412 and ma_nv = a.mnv_tt),
to_tiep_thi = (select ten_to from ttkd_Bsc.nhanvien where thang = 202412 and ma_nv = a.mnv_tt),
ten_kenhbh = 'VTTP',
ma_to = (select ma_to from ttkd_Bsc.nhanvien where thang = 202412 and ma_nv = a.mnv_tt),
ma_pb = (select ma_pb from ttkd_Bsc.nhanvien where thang = 202412 and ma_nv = a.mnv_tt)
where  DONVI = 'VTT'       ;
--

commit;
select* from ct_homecombo_2024 
where thang = 202412 and to_Char(ngay_dk,'yyyymm')  = '202412' and account_fiber not in (select ma_Tb from ttkd_bsc.ct_Bsc_homecombo where thang = 202412)
and loai_goi in  ('HOMECHAT4','HOMECHAT2','HOMECHAT6','HOMESANH1','HOMESANH2','HOMESANH3','HOMESANH4');
insert into ct_homecombo_2024
    ...
    , TEN_KENHBH, DONVI_TIEPTHI, PHONG_DK_GOI, MA_TO, MA_PB)
        (select 202412 thang, CHU_NHOM, THANHVIEN, MNP, LOAI_GOI, LOAI_TB, ACCOUNT_FIBER, TRANGTHAI_FIBER, NGAYHC_FIBER, ACCOUNT_MYTV, TRANGTHAI_MYTV, NGAYHC_MYTV, NGAY_ACTIVE_VNP, TEN_KH, DIACHI_KH
                        , NGAY_DK, TRANGTHAI_GOI, GOI_MOI_HIENHUU, MNV_TT, NGUOI_TH, DONVI, NGAY_SD_FIBER, TOCDO_ID, MA_TD, hdtb_id, NGAY_YC_FIBER, NGAY_HT_FIBER, TRANGTHAI_HD, GHICHU, TEN_NV, TO_TIEP_THI
                        , TEN_KENHBH, DONVI_TIEPTHI, PHONG_DK_GOI, MA_TO, MA_PB
        from ct_homecombo_202412)  ----change---
;
commit;
...
(select distinct 202412, CHU_NHOM, 'HOME' LOAI, LOAI_GOI
...
select 202412 thang, 'Fiber_moi' loai_KPI, account_fiber, loai_goi, a.ngay_dk ngay_dk_goi, hd.ngay_sd ngay_sd_fiber, hd.thuonghieu, mnv_tt, ma_to, ma_pb, hd.hdtb_id, hd.thuebao_id, hd.ctv_id, hd.hdkh_id, hd.tocdo_id, hd.goi_id
...
where a.thang = 202411 and trangthai_goi <> 'Huy' and a.account_fiber = hd.ma_tb      --change n, n-1------
        and trangthai_fiber = 'Moi' and loai_goi in  ('HOMECHAT4','HOMECHAT2','HOMECHAT6','HOMESANH1','HOMESANH2','HOMESANH3','HOMESANH4')
        and not exists (select 1 from ttkd_bsc.ct_bsc_homecombo where thuebao_id = hd.thuebao_id and thang >=202411)     ---change n-1------
;
commit;
delete 
from  ttkd_bsc.ct_bsc_homecombo a
where thang = 202412 and exists (Select 1 from ttkd_Bsc.ct_Bsc_homecombo where thang = 202411 and ma_Tb = a.ma_Tb and ma_nv = a.ma_nv and ngay_dk_goi= a.ngay_dk_goi)
;
---
insert into ttkd_bsc.ct_bsc_homecombo
select 202412 thang, 'Fiber_hh' loai_KPI, account_fiber, loai_goi, a.ngay_dk ngay_dk_goi, hd.ngay_sd ngay_sd_fiber, hd.thuonghieu, mnv_tt, ma_to, ma_pb, hd.hdtb_id, hd.thuebao_id, hd.ctv_id, hd.hdkh_id, hd.tocdo_id, hd.goi_id
    , null ghichu                                                                                           ----1-change----
--    , 'bsung n-1 ket goi, hoan cong n' ghichu                                                                                           ---or 2-change----
from ct_homecombo_2024 a
, (select hdgoi.ctv_id, hdgoi.goi_id, hdtb.hdtb_id, hdtb.thuebao_id, hdtb.ma_tb, hdtb.hdkh_id, td.tocdo_id, td.thuonghieu, db.ngay_sd
...
where a.thang = 202411 and trangthai_goi <> 'Huy' and a.account_fiber = hd.ma_tb      --change n, n-1------
        and trangthai_fiber != 'Moi' and loai_goi in  ('HOMECHAT4','HOMECHAT2','HOMECHAT6','HOMESANH1','HOMESANH2','HOMESANH3','HOMESANH4')
        and not exists (select * from ttkd_bsc.ct_bsc_homecombo where thuebao_id = hd.thuebao_id and thang >=202411)     ---change n-1------
;
commit;
rollback;
select* from ttkd_Bsc.ct_Bsc_Homecombo a 
where thang = 202412 and ma_pb is not null;
delete from ttkd_Bsc.tl_giahan_tratruoc where thang = 202412 and ma_kpi  ='HCM_SL_COMBO_006';
select * f
--- tinh bsc
...
--    select *
    ...
    where loai_kpi in ('Fiber_hh','Fiber_moi') and thang = 202412 
...
insert into ttkd_bsc.tl_giahan_tratruoc (thang, loai_tinh, ma_kpi, ma_nv, ma_to, ma_pb, DA_GIAHAN_DUNG_HEN)
    ...
    where thang = 202412 and LOAI_TINH = 'KPI_NV' and  ma_kpi = 'HCM_SL_COMBO_006' 
...
insert into ttkd_bsc.tl_giahan_tratruoc (thang, loai_tinh, ma_kpi,  ma_to, ma_pb, DA_GIAHAN_DUNG_HEN)
    ...
    from ttkd_bsc.tl_giahan_tratruoc a 
        left join (select distinct ma_nv, ma_to, ma_kpi, thang, ma_pb , dichvu from ttkd_bsc.blkpi_dm_to_pgd 
    where ma_kpi ='HCM_SL_COMBO_006' AND THANG = 202412) b on a.thang = b.thang 
    and a.ma_to = b.ma_to and a.ma_pb = b.ma_pb
    where a.thang = 202412 and LOAI_TINH = 'KPI_TO'  and  a.ma_kpi = 'HCM_SL_COMBO_006' 
...
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where ma_kpi in ('HCM_SL_COMBO_006') and thang = 202412 and MA_VTCV = a.MA_VTCV)
...
and ma_kpi = 'HCM_SL_COMBO_006')
where thang = 202412 and exists (select * from TTKD_BSC.blkpi_danhmuc_kpi_vtcv where to_truong_pho = 1 and ma_kpi in ('HCM_SL_COMBO_006')
    and thang = 202412 and MA_VTCV = a.MA_VTCV)
and ma_kpi = 'HCM_SL_COMBO_006';
...
and ma_kpi in ('HCM_SL_COMBO_006') and thang = 202412 and MA_VTCV = a.MA_VTCV)
and ma_kpi = 'HCM_SL_COMBO_006' and giao is not null;
commit;

select* from  TTKD_BSC.bangluong_kpi a where ma_kpi = 'HCM_SL_COMBO_006' and ten_vtcv like 'Phó%' and giao is not null and thang = 202412;
select* from ttkd_Bsc.ct_Bsc_homecombo where thang = 202412 and ma_pb is null;
delete  from TTKD_BSC.bangluong_kpi a where ma_kpi = 'HCM_SL_COMBO_006' and ten_vtcv like 'Phó%' and thang = 202412 
