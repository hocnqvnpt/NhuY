...
select distinct ma_pb, ten_pb from ttkd_bsc.nhanvien where thang = 202409;
...
----------------TT giam cuoc or thang tang tren 2 dong-------------
        ...
        , goi_cu as (select nhomtb_id, thuebao_id, row_number() over (partition by thuebao_id order by nhomtb_id desc) rnk
                  ...
                  where trangthai = 1 and KYHOADON = 20240701
        )
    ...
    select 
            202409 thang, a.ob_id, a.ngay_ob, x.thuebao_id, dbtb.ma_tb,a.ngay_bddc ngay_bddc_cu, 
           ...
           , x.NVGIAOPHIEU_ID, x.DVGIAOPHIEU_ID ,   m.nhomtb_id , dbtb.khachhang_id, x.phieutt_id, x.ht_tra_id, x.kenhthu_id, x.loaitb_id, dbtb.thanhtoan_id, nvxp.ma_nv nhanvien_xuathd
                , case when x.rkm_id is null then null
                                when x.ma_gd in (select ma_gd from qrcode where thang = 202409) then 2
...
--                                    on a.pbh_id_th = pb_th.pbh_id
                                ...
                                left join ttkd_bsc.nhanvien nvtp on x.MANV_THUYETPHUC = nvtp.ma_nv and nvtp.thang = 202409 and nvtp.donvi = 'TTKD'
                                left join ttkd_bsc.nhanvien nvob on a.ma_nv = nvob.ma_nv and nvob.thang = 202409 and nvob.donvi = 'TTKD'
                                ...
                                left join ttkd_bsc.nhanvien nvxp on xp.ma_nv = nvxp.ma_nv and nvxp.thang = 202409 and nvxp.donvi = 'TTKD'
where not exists (select rkm_id from ttkd_bsc.nhuy_ct_bsc_ipcc_obghtt where  nvl(tien_khop,0) > 0 and thang = 202407 and rkm_id = x.rkm_id )
                                ;
update qrcode set thang = 202409 where thang is null;
drop table ttin_chungtu;
delete from qrcode where thang = 202409;
;
select *  from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202409;
delete from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202409;
...
--                              
                        where a.rkm_id is not null and thang = 202409 --and c.ma_pb = 'VNP0701400'--and a.thang = 202403 and to_number(to_char(ngay_tt,'yyyymm'))  = 202403---        CHANGE
...
--			and nvl(a.bosung, 999) <> 1
	...
	;
commit;
drop table ttin_chungtu_t9;
flashback table ttin_chungtu to before drop;
select* from ttin_chungtu_t8 ;where manv_tt = manv_hd;
create table ttin_chungtu_t9 as 
select a.* , c.ma_nv manv_tt, d.ma_nv manv_hd from ttin_ctu a 
...
rollback;
delete from ttkd_bsc.ct_dongia_Tratruoc where thang = 202409 and loai_tinh = 'DONGIATRA_OB';
commit;
...
--create table tmp_Dg_obghtt as;
      ...
      manv_Tt NHANVIEN_tt, DONGIA, TYLE_THANHCONG, GHICHU--, manv_Tt, manv_hd, nhanvien_capnhat , thungan_tt_id, thungan_Hd_id
            , case when dongia = 10000 then 10000*nvl(TYLE_THANHCONG,0.4) else 0 end tien_xuathd , ipcc
      from 
      (
      select 
     ...
     '1' ghichu--, 'bo sung cho 1 thue bao gia han 2 hop dong' ghichu,
        ...
        ,case 
         when tien_khop = 1 and to_number(to_char(to_date(b.ngay_tt,'dd/mm/yyyy') ,'yyyymmdd'))  <= to_number(to_char(c.ngay_insert,'yyyymmdd'))+1 then 0.4 
...
--
                        ...
                        from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt a 
                                            ...
                                            left join ttin_Chungtu_t9 b on a.ma_Gd = b.ma_gd
                        ...
                        where  A.THANG = 202409-- and a.thuebao_id in (select thuebao_id from temp_chot)--and a.thang = 202403 and to_number(to_char(ngay_tt,'yyyymm'))  = 202403---        CHANGE
...
--            join ttkd_bsc.nhanvien_202404 b on a.MANV_THUYETPHUC = b.ma_nv 
            ...
            and not exists (select 1 from ttkd_Bsc.ct_dongia_Tratruoc where thang in  (202408) and tien_khop >0 and thuebao_id = a.thuebao_id and ma_nv = a.MANV_THUYETPHUC and
...
where thang = 202409 and tien_khop = 1 and loai_tinh = 'DONGIATRA_OB' and exists (
...
where
b.thang = 202409 and a.chungtu_id=c.chungtu_id
...
update ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt set tien_khop = 3 where ma_Tb in (Select ma_tb from phithu_Tainha where thang = 202409);
update ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt set ipcc = 1 where ma_Nd_ob is not null and thang = 202409; and ipcc !=1;
select* from ttkd_Bsc.ct_Dongia_tratruoc where thang = 202409;
commit;
...
;
update ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt set ipcc = 1 where ma_nd_ob is not null and thang = 202409 and ipcc is null;
commit;
select* from  ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt a where thang = 202409 and ma_nd_ob is null and ipcc is null; and exists (select 1 from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202409
and khachhang_id = a.khachhang_id and nvtuvan_id = a.nvtuvan_id and ma_gd = a.ma_Gd and ma_nd_ob is not null);
select thuebao_id, manv_Thuyetphuc from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt a where thang = 202409 and tien_khop = 1 
and not exists (select 1 from ttkd_Bsc.ct_dongia_Tratruoc where thang in  (202407) and tien_khop >0 and thuebao_id = a.thuebao_id and ma_nv = a.MANV_THUYETPHUC and
            loai_tinh = 'DONGIATRA_OB') group by thuebao_id, manv_Thuyetphuc;

-- update 
update ttkd_Bsc.ct_dongia_tratruoc set tien_thuyetphuc = dongia where dongia!= 10000 and loai_tinh = 'DONGIATRA_OB' AND THANG = 202409;
update ttkd_Bsc.ct_dongia_tratruoc set tien_thuyetphuc = (dongia-tien_xuathd) where dongia= 10000 and loai_tinh = 'DONGIATRA_OB' AND THANG = 202409;
commit;
select distinct tien_khop, tien_xuathd, tien_thuyetphuc from  ttkd_Bsc.ct_dongia_tratruoc WHERE loai_tinh = 'DONGIATRA_OB' AND THANG = 202409 ; and dongia != 7500 ;
rollback;
select* from  ttkd_Bsc.ct_dongia_tratruoc WHERE loai_tinh = 'DONGIATRA_OB' AND THANG = 202409 and tien_khop!= 1;
select* from ttkd_Bsc.ct_bsc_homecombo where thang = 202409;
select* from (
select THANG, LOAI_KPI, MA_TB, HOMECOMBO, to_char(NGAY_DK_GOI,'dd-mm-yyyy') NGAY_DK_GOI, to_char(NGAY_SD_FIBER,'dd-mm-yyyy') NGAY_SD_FIBER, FIBER, MA_NV, MA_TO, MA_PB, HDTB_ID, THUEBAO_ID, CTV_ID_GOI, HDKH_ID, TOCDO_ID, GOI_ID, GHICHU from ttkd_bsc.ct_bsc_homecombo 
    where loai_kpi in ('Fiber_hh','Fiber_moi') 
);
select* from ttkd_Bsc.ct_Dongia_Tratruoc where thang = 202409 and loai_tinh = 'DONGIA_TS_TP_TT';
select A.MA_GD, B.CHUNGTU_ID from  ttkd_Bsc.NHUY_cT_BSC_IPCC_OBGHTT a
    left join nv_capnhat b on a.ma_gd = b.ma_gd
where a.thang = 202406 AND TIEN_KHOP = 1;
...
from  ttkd_Bsc.tl_giahan_Tratruoc WHERE THANG = 202409 AND LOAI_TINH IN ('DONGIATRA_OB','DONGIA_TS_TP_TT')
GROUP BY THANG,MA_NV,MA_TO,MA_PB;
rollback;
select* from ttkd_Bsc.ct_dongia_tratruoc where thang = 202409 and ma_Tb ='bht210726';
COMMIT;

delete from ttkd_Bsc.tl_giahan_tratruoc where thang = 202409 and ma_pb in (select ma_pb from ttkd_Bsc.nhanvien where thang = 202409 and ten_pb like '%Khu%') and loai_Tinh = 'DONGIATRA_OB';
...
with tien as (
    ...
    where thang = 202409 and loai_tinh = 'DONGIATRA_OB' and ipcc = 1 --and ghichu = 'bo sung TB co chot, chua chi don gia, nhan vien xuat hoa don la nhan vien cap nhat chung tu, ty le thanh cong la ty le tien don gia xuat HD (TTCK)'
--    and ma_nv = 'VNP017247'
    union all
    select thang,nhanvien_xuathd,loai_tinh, ma_kpi,ma_tb,  heso_chuky*heso_dichvu*NVL(tien_xuathd,0) TIEN,ma_pb
    from ttkd_Bsc.ct_dongia_tratruoc 
    where thang = 202409 and loai_tinh = 'DONGIATRA_OB' --and ghichu= 'bo sung TB co chot, chua chi don gia, nhan vien xuat hoa don la nhan vien cap nhat chung tu, ty le thanh cong la ty le tien don gia xuat HD (TTCK)'
)
...
WHERE  a.ma_pb in (select ma_pb from ttkd_Bsc.nhanvien where thang = 202409 and ten_pb like '%Khu%') 
--where b.ma_pb not in ('VNP0701800','VNP0701500','VNP0701200','VNP0701300','VNP0701600','VNP0701400','VNP0701100','VNP0702200','VNP0702100') and b.donvi = 'TTKD'  and b.ma_pb not in ('VNP0702300','VNP0702500','VNP0702400','VNP0700600')
--;
--select* from ttkd_bsc.ct_dongia_tratruoc where ma_nv = 'VNP017254' and ma_Tb= 'hcmdson16' and thang = 202406;
group by  a.thang,a.ma_nv, b.ma_to, b.ma_pb, a.loai_tinh, a.ma_kpi,b.ten_to, b.ten_pb, b.ma_Vtcv, b.ten_vtcv, ten_pb;
commit;
select distinct loai_Tinh, ma_kpi from ttkd_Bsc.tl_giahan_Tratruoc where thang = 202406;
...
select distinct ghichu from  ttkd_Bsc.ct_dongia_tratruoc 
    ...
    select distinct ma_pb , ten_pb from ttkd_Bsc.nhanvien_202406 where ten_pb like 'Phòng Bán Hàng Khu V?c%';
rollback;
