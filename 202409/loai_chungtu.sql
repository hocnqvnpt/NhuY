...
select * from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202409 and ma_Tb in ( 'vanduy54','vnpt290523-1');
select* from css_hcm.phieutt_hd where ma_Gd = 'HCM-TT/02963655';
update ttkd_bsc.ct_dongia_tratruoc a
set nhanvien_xuathd = (select distinct a.nhanvien_xuathd from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202409 and a.ma_Tb = ma_Tb and manv_thuyetphuc = a.ma_nv)
where thang = 202409 and tien_khop = 1 and tyle_thanhcong is null and loai_Tinh ='DONGIATRA_OB' and ghichu !='gach no tu dong';
rollback;
COMMIT;
select* from ttkd_bsc.ct_dongia_Tratruoc where thang = 202409 and tien_khop > 0 and tien_thuyetphuc is null and loai_Tinh = 'DONGIATRA_OB';
update ttkd_bsc.ct_dongia_tratruoc c
set tien_thuyetphuc = 6000
--select* from ttkd_bsc.ct_dongia_tratruoc c
where thang = 202409 and tien_khop = 1 and loai_Tinh ='DONGIATRA_OB' 
...
select* from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where thang = 202409 and ma_Gd in ('HCM-TT/02961381','HCM-TT/02960610');
(select ma_Tb from ttkd_Bsc.ct_dongia_tratruoc where thang = 202409 and tien_khop = 1 and tyle_thanhcong is null and loai_Tinh ='DONGIATRA_OB' and ghichu !='gach no tu dong');
update ttkd_Bsc.ct_dongia_tratruoc set nhanvien_xuathd = 'CTV087446'
where thang = 202409 and tien_khop = 1 and tyle_thanhcong is null and loai_Tinh ='DONGIATRA_OB' and ghichu !='gach no tu dong' and ma_Tb ='hcmbpb-02.01';
...
nvl(chungtu_id,0), nguoi_cnt nv_Gach, 1 tinh_dongia, 1 tinh_bsc, 202409 thang
from ct_Trsau where thang = 202409  
union all
select ten_nganhang, ma_ct, tien_ct, ten_Ht_Tra, ngay_tt, null ma_tt, ma_Tb, ma_Gd, loaihinh_tb, ghichu, manv_tt, phongban_tt, tien_hopdong, vat_hopdong, tongtien, null httt_id
,1 tra_truoc, chungtu_id, manv_tt, 0 tinh_dongia, 1 tinh_bsc, 202409 thang
from ct_trtruoc where thang = 202409;
commit;
select* from ct_Trtruoc;
select* from ct_Bsc_Chungtu where thang = 202409;and tra_Truoc = 1; and tinh_dongia = 1;
--delete  from ct_Bsc_Chungtu where thang = 202409; and tra_Truoc = 1;
rollback;
update ct_Bsc_Chungtu set tinh_dongia = 1 where tra_Truoc = 1 and thang = 202409;
commit;
rollback;
---- bo don gia
...
where thang = 202409 and tra_truoc = 1 and exists (select 1 from ttkd_bsc.ct_dongia_tratruoc where thang in ( 202409) and loai_tinh = 'DONGIATRA_OB'
and a.ma_Tb = ma_Tb and a.nv_gach = nhanvien_xuathd) and tinh_Dongia = 1;
;
update ct_Bsc_Chungtu a 
set tinh_dongia = 0
--select* from ct_Bsc_Chungtu a 
where thang = 202409 and tra_truoc = 1 and exists (select 1 from ttkd_bsc.ct_dongia_tratruoc where thang in ( 202408,202409) and loai_tinh = 'DONGIATRA_OB'
...
and a.fcheck=0
and EXISTS (select 1 from ttkdhcm_ktnv.ds_chungtu_nganhang_sub_oneb where ma_gd=b.ma_gd) and b.thang = 202409
...
and a.fcheck=0
and EXISTS (select 1 from ttkdhcm_ktnv.ds_chungtu_nganhang_sub_oneb where ma_tt=b.ma_tt) and b.thang = 202409
and ct.chungtu_id = b.chungtu_id);
...
using (
        ...
        where a.chungtu_id=c.chungtu_id
                ...
                and c.nhandien_thanhtoan is not null --1574 989
                and a.fcheck=0
                and EXISTS (select 1 from ttkdhcm_ktnv.ds_chungtu_nganhang_sub_oneb where ma_tt=b.ma_tt) and b.thang = 202409 ) tb 
...
rollback;
commit;
select * from ct_bsc_chungtu where thang = 202409 and tinh_bsc =  0;
...
select distinct  ma_Gd from ct_Bsc_Chungtu where thang = 202409 and tinh_dongia = 1 and tinh_bsc  = 1 and tra_Truoc = 1
and ma_Tb in (select ma_Tb from ttkd_Bsc.ct_Dongia_tratruoc where tien_khop = 1 and heso_chuky!= 0 and thang = 202409)) ;
select* from ttkd_Bsc.ct_Dongia_tratruoc where tien_khop = 1 and heso_chuky = 0 and thang = 202409;
select* from ttkd_Bsc.nhuy_ct_Bsc_ipcc_obghtt where ma_Gd ='HCM-TT/02982144';
select* from ttkd_Bsc.ct_dongia_Tratruoc where thang = 202409 and tyle_Thanhcong is null and tien_khop =1 and loai_Tinh = 'DONGIATRA_OB';